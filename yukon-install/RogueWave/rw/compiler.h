 ...
 *** RW_NO_IOSSTREAM:  Adds certain functionality needed for iostream
        support.
 *** RW_NO_IOS_HEADER: Defined if the header <iosfwd> is used instead of <ios>
 ...
 *** RW_NO_PUBSYNC: Defined if iostream library has the public method
        sync() instead of pubsync()
 ...
 *** RW_NO_NEW_TEMPLATE_SYNTAX:  Defined if your compiler does not
		support using the "template<>" syntax for template specialization.
 ...
 *** RW_NO_LOCALTIME_R: defined if the system does not have localtime_r()
        for multithreading.
 ...
 *** RW_DECLARE_GLOBAL_FRIENDS_FIRST: For compilers that need friend functions 
        declared globally before class declarations.
 ...
 *** RW_WRAP_C: Defined if your header files must be wrapped in the
        'extern "C" { ... }' construct.
 *** RW_FALSE_NEW_DELETE_MISMATCH: defined if your compiler doesn't
        recognize the classic way to overload the new and delete operators
 *** RW_BREAK_LINK_COMPATIBILITY: This macro may be defined if maintaining
        link compatibility is not an issue.  If defined, bug fixes that 
        necessitate breaking link compatibility are turned on.
 *** RW_ATOMIC_INT_OPS: This macro may be defined if your operating system
	supports atomic integer operations.  This will allow the RWCString
	and RWWString classes to do faster reference count updates in a 
	multithreaded program.  Currently, these operations are only 
	supported with Windows NT and Windows 98.
 *** RW_STD_COLLECTION_PROBLEM: defined if your compiler has a problem 
        returning a reference to the underlying C++-standard collection that
        serves as the implementation for a given collection, 
        ( ie. collection.std() ). 
 ***/
...
/* #define RW_NO_NONCLASS_ARROW_RETURN 1 */
/* #define RW_WRAP_C 1 */
...
/* #define RW_RWV20_STDLIB 1 */
/* #define RW_NO_STL 1 */

#define RWSTDLIBTYPE 2
#define RW_NAMESPACE_REQUIRED 1

/* #define RW_NO_RELOPS_NAMESPACE 1 */
...
#define RW_SOCKETS_ARE_ALWAYS_INET 1
#define RWNET_WINSOCK 1
#define RW_MUST_CALL_BIND_EARLY 1

/* #define RW_NO_NEW_TEMPLATE_SYNTAX 1 */
...
#    endif
#    if __TURBOC__ >= 0x540
#      define RW_BCB_NEW_IOSTREAMS         1
#      define RW_BCB_CTYPE_NEEDED          1
#      define RW_BCB_NAMESPACE             1
#      define RW_NO_LOCALTIME_R            1
#      ifndef RW_NO_STL 
#        define RW_BROKEN_NAME_RESOLUTION  1  
#        define RW_ALLOCATOR_NO_REBIND     1
#        undef RW_ALLOC_INTERFACE_STLV2X_HACK
#        undef RW_BROKEN_TEMPLATE_TYPEDEFS
#        undef RW_ITER_NO_TAGS
#        undef RW_NO_TYPENAME 
#      endif
#    endif            
#  endif
#endif  /* __TURBOC__ */
...
#      undef  RW_MSVC_DLL_LOCALE_FACET
...
/********************** CYGNUS C++ *********************************/
#if defined(__GNUG__)
#define RW_NO_IOS_HEADER               1
#define RW_NO_PUBSYNC                  1
#define RW_WCTYPEH_NEEDED              1
...
/******** Turn off C++ exceptions by default with Objectstore ******/
