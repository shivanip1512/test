...
public class ProgramTableModel extends javax.swing.table.AbstractTableModel implements
        javax.swing.event.TableModelListener, IProgramTableModel {
    ...
    public static String[] columnNames = { "Program Name", "Status", "Start Date/Time", "Stop Date/Time",
        "Current Gear", "Priority",
    ...
    static {
        ...
        try {
            showRedCol =
                Boolean.valueOf(ClientSession.getInstance().getRolePropertyValue(YukonRoleProperty.LC_REDUCTION_COL));
        } catch (Exception e) {}
    ...
    public Object getValueAt(int row, int col) {
        ...
        if (row <= getRowCount() && isProgramValid((LMProgramBase) getRowAt(row))) {
            ...
            return TdcLoadcontrolUtils.getProgramValueAt(prg, col, new SystemUserContext());
        } else {
            return null;
        }
    }
    ...
    private boolean isProgramValid(LMProgramBase prg) {
        if (prg instanceof com.cannontech.loadcontrol.data.LMProgramDirect
            ...
            throw new IllegalArgumentException("Found an instance of " + prg.getClass().getName() + " in our model : "
    ...
    public synchronized void setCurrentControlArea(com.cannontech.loadcontrol.data.LMControlArea newCurrentControlArea) {
        ...
        if (getCurrentControlArea() == null || getCurrentControlArea().getLmProgramVector() == null) {
            ...
            java.util.Collections.sort(getRows(), PROGRAM_NAME_COMPARATOR);
        }
    ...
    public void tableChanged(javax.swing.event.TableModelEvent event) {
        if (event instanceof LCGenericTableModelEvent) {
            if (((LCGenericTableModelEvent) event).getType() == LCGenericTableModelEvent.TYPE_CLEAR) {
                clear();
            }
