...
public abstract class MeterReadBase implements ReadableDevice {
    ...
    public void populateWithPointData(int deviceID) {
        List<LitePoint> litePoints =
            (YukonSpringHook.getBean("pointDao", PointDao.class)).getLitePointsByPaObjectId(deviceID);
        ...
        for (LitePoint litePoint : litePoints) {
            ...
            if (pointData != null && pointData.getPointQuality() != PointQuality.Uninitialized)
                ...
                populate(PointIdentifier.createPointIdentifier(litePoint), pointData.getPointDataTimeStamp(),
                    pointData.getValue());
