...
public class MultispeakMeterServiceImpl implements MultispeakMeterService, MessageListener {
    ...
    private static Map<Long, MultispeakEvent> eventsMap =
        Collections.synchronizedMap(new HashMap<Long, MultispeakEvent>());
    ...
    public void initialize() throws Exception {
        ...
        supportedEventTypes =
            ImmutableSet.of(OutageEventType.OUTAGE, OutageEventType.NO_RESPONSE, OutageEventType.RESTORATION,
                OutageEventType.POWER_OFF, OutageEventType.POWER_ON, OutageEventType.INSTANTANEOUS,
        ...
        for (OutageEventType eventType : supportedEventTypes) {
            String valueStr =
                configurationSource.getString("MSP_OUTAGE_EVENT_TYPE_CONFIG_" + eventType.value().toUpperCase());
            if (valueStr != null) {
                int[] errorCodes =
            ...
            if (event != null) {
                ...
                Runnable eventRunner = new Runnable() {
                    ...
                    public void run() {
                        ...
                        log.info("Message Received [ID:" + returnMsg.getUserMessageID() + " DevID:"
                            + returnMsg.getDeviceID() + " Command:" + returnMsg.getCommandString() + " Result:"
                            + returnMsg.getResultString() + " Status:" + returnMsg.getStatus() + " More:"
                            + returnMsg.getExpectMore() + "]");

                        if (returnMsg.getExpectMore() == 0) {

                            log.info("Received Message From ID:" + returnMsg.getDeviceID() + " - "
    ...
    @Override
    public LoadActionCode CDMeterState(final MultispeakVendor mspVendor, final YukonMeter meter)
    ...
    public MeterRead getLatestReadingInterrogate(MultispeakVendor mspVendor, YukonMeter meter, String responseUrl) {
        ...
        multispeakEventLogService.initiateMeterRead(meterNumber, meter, "N/A", "getLatestReadingByMeterNo",
            mspVendor.getCompanyName());
        ...
        synchronized (event) {
            ...
            if (timeout) {
                mspObjectDao.logMSPActivity("getLatestReadingByMeterNo", "MeterNumber (" + meterNumber
                    + ") - Reading Timed out after " + (mspVendor.getRequestMessageTimeout() / 1000)
    ...
    public synchronized ErrorObject[] odEvent(final MultispeakVendor mspVendor, String[] meterNumbers,
        ...
        if (StringUtils.isBlank(responseUrl)) { // no need to go through all the
                                                // work if we have no one to
            // respond to.
        ...
        Map<String, YukonMeter> meterNumberToMeterMap =
            meterDao.getMetersMapForMeterNumbers(Lists.newArrayList(meterNumbers));
            ...
            new CommandCompletionCallbackAdapter<CommandRequestDevice>() {
                ...
                public void receivedLastResultString(CommandRequestDevice command, String value) {
                    ...
                    OutageDetectionEvent outageDetectionEvent =
                        buildOutageDetectionEvent(yukonMeter, outageEventType, now, value);
                ...
                public void receivedLastError(CommandRequestDevice command, SpecificDeviceErrorDescription error) {
                    ...
                    OutageDetectionEvent outageDetectionEvent =
                        buildOutageDetectionEvent(yukonMeter, outageEventType, now, error.getPorter());
    ...
    private void doRfnOutagePing(final List<YukonMeter> meters, final MultispeakVendor mspVendor,
        ...
        if (isChannelReadForPing) {
            DeviceAttributeReadCallback callback = new DeviceAttributeReadCallback() {
                ...
                public void receivedLastValue(PaoIdentifier pao, String value) { // success
                    ...
                    if (meters.contains(yukonMeter)) { // meter may have already been handled and removed by
                        ...
                        OutageEventType outageEventType = getForStatusCode(0);
                        OutageDetectionEvent outageDetectionEvent =
                            buildOutageDetectionEvent(yukonMeter, outageEventType, now, "");
                        sendODEventNotification(yukonMeter, mspVendor, transactionId, responseUrl, outageDetectionEvent);
                    }
                }
            ...
            for (PointValueQualityHolder pointValue : pointValues) {
                ...
                OutageDetectionEvent outageDetectionEvent =
                    buildOutageDetectionEvent(yukonMeter, outageEventType, pointValue.getPointDataTimeStamp(),
                        "Last Known Status");
                sendODEventNotification(yukonMeter, mspVendor, transactionId, responseUrl, outageDetectionEvent);
            }
        }
    ...
    public synchronized ErrorObject[] meterReadEvent(final MultispeakVendor mspVendor, String[] meterNumbers,
        ...
        multispeakEventLogService.initiateMeterReadRequest(meterNumbers.length, "initiateMeterReadByMeterNumber",
            mspVendor.getCompanyName());
        ...
        for (String meterNumber : meterNumbers) {
            ...
            multispeakEventLogService.initiateMeterRead(meterNumber, meter, transactionID,
                "initiateMeterReadByMeterNumber", mspVendor.getCompanyName());
        }
        ...
        DeviceAttributeReadCallback callback = new DeviceAttributeReadCallback() {
            ...
            public void receivedLastValue(PaoIdentifier pao, String value) {
                ...
                if (updater != null) {
                    ...
                    try {
                        log.info("Sending ReadingChangedNotification (" + responseUrl + "): Meter Number "
                            + meterRead.getObjectID());
                        final ReadingChangedNotification readingChangedNotification =
                                objectFactory.createReadingChangedNotification();
                        ...
                        log.info("Sending ReadingChangedNotification (" + responseUrl + "): Meter Number "
                            + meterRead.getObjectID());
                        ReadingChangedNotificationResponse readingChangedNotificationResponse =
                            cbClient.readingChangedNotification(mspVendor, responseUrl, readingChangedNotification);
                        ErrorObject[] errObjects = null;
                        if (readingChangedNotificationResponse != null) {
                            ArrayOfErrorObject arrOfErrorObject =
                                readingChangedNotificationResponse.getReadingChangedNotificationResult();
                            List<ErrorObject> errorObjects = arrOfErrorObject.getErrorObject();
                            if (errorObjects != null) {
                                errObjects = mspObjectDao.toErrorObject(errorObjects);
                            }
                        }
                        multispeakEventLogService.notificationResponse("readingChangedNotification", transactionID,
                            meterRead.getObjectID(), meterRead.getPosKWh().toString(),
                            ArrayUtils.getLength(errObjects), responseUrl); // just picked PoskWh to
                                                                            // log...because nothing better
    ...
    public synchronized ErrorObject[] blockMeterReadEvent(final MultispeakVendor mspVendor, String meterNumber,
        ...
        multispeakEventLogService.initiateMeterReadRequest(1, "initiateMeterReadByMeterNoAndType",
            mspVendor.getCompanyName());

        final YukonMeter paoToRead;
        try {
            paoToRead = mspMeterDao.getMeterForMeterNumber(meterNumber);
            multispeakEventLogService.initiateMeterRead(meterNumber, paoToRead, transactionId,
                "initiateMeterReadByMeterNoAndType", mspVendor.getCompanyName());
        } catch (NotFoundException e) {
            multispeakEventLogService.meterNotFound(meterNumber, "initiateMeterReadByMeterNoAndType",
                mspVendor.getCompanyName());
            ErrorObject err =
                mspObjectDao.getNotFoundErrorObject(meterNumber, "MeterNumber", "Meter", "MeterReadEvent",
                    mspVendor.getCompanyName());
        ...
        DeviceAttributeReadCallback callback = new DeviceAttributeReadCallback() {
             ...
             * This is different than the meterReadEvent which may have multiple meterNumbers it is processing
             * and
             * therefore fires notifications for each read that comes in (basically using this same layout of
             * code, only
            ...
            public void complete() {
                ...
                final FormattedBlockNotification formattedBlockNotification =
                        objectFactory.createFormattedBlockNotification();
                ...
                try {
                    FormattedBlockNotificationResponse formattedBlockNotificationResponse =
                        cbClient.formattedBlockNotification(mspVendor, responseUrl, formattedBlockNotification);
                    ErrorObject[] errObjects = null;
                    if (formattedBlockNotificationResponse != null) {
                        ArrayOfErrorObject arrOfErrorObject =
                            formattedBlockNotificationResponse.getFormattedBlockNotificationResult();
                        List<ErrorObject> errorObjects = arrOfErrorObject.getErrorObject();
                        if (errorObjects != null) {
                            errObjects = mspObjectDao.toErrorObject(errorObjects);
                        }
                    ...
                    multispeakEventLogService.notificationResponse("formattedBlockNotification", transactionId,
                        block.getObjectId(), "", ArrayUtils.getLength(errObjects), responseUrl);
                    if (!ArrayUtils.isEmpty(errObjects)) {
    ...
    @Override
    public synchronized ErrorObject[] cdEvent(final MultispeakVendor mspVendor, ConnectDisconnectEvent[] cdEvents,
            final String transactionId, final String responseUrl) throws MultispeakWebServiceException {
        ...
        multispeakEventLogService.initiateCDRequest(cdEvents.length, "initiateConnectDisconnect",
            mspVendor.getCompanyName());
        ...
        for (ConnectDisconnectEvent cdEvent : cdEvents) {
            ...
            try {
                ...
                if (isRfnDisconnect) {
                    ...
                    doRfnConnectDisconnect(rfnMeter, mspLoadActionCode.getRfnState().getType(), mspVendor,
                ...
                if (!canInitiatePorterRequest || StringUtils.isBlank(mspLoadActionCode.getPlcCommandString())) {
                    ...
                    plcCommandRequests.add(request);
                    multispeakEventLogService.initiateCD(meter.getMeterNumber(), meter, mspLoadActionCode.toString(),
                        (cdEvent.getReasonCode() != null ? cdEvent.getReasonCode().value() : "unknown"),
    ...
    private void doPlcConnectDisconnect(List<CommandRequestDevice> plcCommandRequests,
        ...
        commandRequestDeviceExecutor.execute(plcCommandRequests, callback, DeviceRequestType.GROUP_CONNECT_DISCONNECT,
    ...
    private void sendCDEventNotification(SimpleMeter yukonMeter, LoadActionCode loadActionCode,
        ...
        log.info("Sending CDStateChangedNotification (" + responseUrl + "): Meter Number "
            + yukonMeter.getMeterNumber() + " Code:" + loadActionCode);
        ...
        try {
            ...
            multispeakEventLogService.notificationResponse("CDStateChangedNotification", transactionId,
                yukonMeter.getMeterNumber(), loadActionCode.value(), -1, responseUrl);
    ...
    @Override
    public ErrorObject[] meterAdd(final MultispeakVendor mspVendor, Meter[] addMeters)
            throws MultispeakWebServiceException {
        ...
        for (final Meter mspMeter : addMeters) {
            try {
                transactionTemplate.execute(new TransactionCallbackWithoutResult() {
                    ...
                    protected void doInTransactionWithoutResult(TransactionStatus status) {
                        ...
                        ServiceLocation mspServiceLocation =
                            mspObjectDao.getMspServiceLocation(mspMeter.getMeterNo(), mspVendor);
                        ...
                        if (mspServiceLocation != null) {
                            ...
                            updateBillingCyle(billingCycle, newMeter.getMeterNumber(), newMeter, METER_ADD_STRING,
                                mspVendor);
                            updateAltGroup(mspServiceLocation, newMeter.getMeterNumber(), newMeter, METER_ADD_STRING,
                                mspVendor);
                        } else {
                            multispeakEventLogService.objectNotFoundByVendor(mspMeter.getMeterNo(),
                                "getServiceLocationByMeterNo", METER_ADD_STRING, mspVendor.getCompanyName());
                        }
                ...
                ErrorObject err =
                    mspObjectDao.getErrorObject(mspMeter.getMeterNo(), "X Exception: (MeterNo:" + mspMeter.getMeterNo()
                        + ")-" + ex.getMessage(), "Meter", METER_ADD_STRING, mspVendor.getCompanyName());
                errorObjects.add(err);
                multispeakEventLogService.errorObject(err.getErrorString(), METER_ADD_STRING,
                    mspVendor.getCompanyName());
                log.error(ex);
            } catch (Error ex) {
                // Transactional code threw error -> rollback
                ErrorObject err =
                    mspObjectDao.getErrorObject(mspMeter.getMeterNo(), "X Error: (MeterNo:" + mspMeter.getMeterNo()
                        + ")-" + ex.getMessage(), "Meter", METER_ADD_STRING, mspVendor.getCompanyName());
                errorObjects.add(err);
                multispeakEventLogService.errorObject(err.getErrorString(), METER_ADD_STRING,
                    mspVendor.getCompanyName());
    ...
    private YukonMeter addNewMeter(Meter mspMeterToAdd, MultispeakVendor mspVendor) throws MspErrorObjectException {
            ...
            if (templateMeter.getPaoType().isRfn()) {
                ...
                newDevice =
                    deviceCreationService.createRfnDeviceByTemplate(templateMeter.getName(), newPaoName,
                        newMeterRfnIdentifier.getSensorModel(), newMeterRfnIdentifier.getSensorManufacturer(),
                        newMeterRfnIdentifier.getSensorSerialNumber(), true);
                ...
                ErrorObject errorObject =
                    mspObjectDao.getErrorObject(mspMeterToAdd.getObjectID(), "Error: Invalid template type ["
                        + templateMeter.getPaoType() + "].", "Meter", METER_ADD_STRING, mspVendor.getCompanyName());
                throw new MspErrorObjectException(errorObject);
            }
        } catch (DeviceCreationException | BadConfigurationException e) {
            log.error(e);
            ErrorObject errorObject =
                mspObjectDao.getErrorObject(mspMeterToAdd.getObjectID(), "Error: " + e.getMessage(), "Meter",
                    METER_ADD_STRING, mspVendor.getCompanyName());
            throw new MspErrorObjectException(errorObject);
        }
        ...
        multispeakEventLogService.meterCreated(newMeter.getMeterNumber(), newMeter, METER_ADD_STRING,
            mspVendor.getCompanyName());
    ...
    private RfnIdentifier buildNewMeterRfnIdentifier(RfnMeter templateMeter, String serialNumber) {
        ...
        if (StringUtils.isBlank(manufacturer) || StringUtils.isBlank(model)) {
            ...
            String templatePrefix =
                configurationSource.getString(MasterConfigStringKeysEnum.RFN_METER_TEMPLATE_PREFIX, "*RfnTemplate_");
    ...
    private YukonMeter checkForExistingMeterAndUpdate(Meter mspMeter, MultispeakVendor mspVendor)
        ...
        multispeakEventLogService.meterFound(meter.getMeterNumber(), meter, METER_ADD_STRING,
            mspVendor.getCompanyName());
    ...
    private void updateExistingMeter(Meter mspMeter, YukonMeter existingMeter, YukonMeter templateMeter,
        ...
        String newSerialOrAddress = mspMeter.getNameplate().getTransponderID().trim(); // this should be the
                                                                                       // sensorSerialNumber
     ...
     */
    private void verifyAndUpdateType(YukonMeter templateMeter, YukonMeter existingMeter, String mspMethod,
            MultispeakVendor mspVendor) throws MspErrorObjectException {
        ...
        if (templateMeter.getPaoType() != originalType) {
            ...
            try {
                ...
                existingMeter.setPaoIdentifier(new PaoIdentifier(existingMeter.getDeviceId(),
                    templateMeter.getPaoType())); // update local object with new type
                ...
                if (originalRouteId != initialRoute.getId()) {
                    ...
                    if (routes.size() == 1) { // no need to run route discovery if we only have one route.
                        multispeakEventLogService.routeUpdated(initialRoute.getName(), meterNumber, mspMethod,
                            mspVendor.getCompanyName());
                        systemLog(mspMethod, "MeterNumber(" + meterNumber + ") - Only one route associated with "
                            + "substation, assigned route and skipping route discovery.", mspVendor);
                    }
                }
    ...
    public ErrorObject[] serviceLocationChanged(final MultispeakVendor mspVendor, ServiceLocation[] serviceLocations) {
        ...
        for (final ServiceLocation mspServiceLocation : serviceLocations) {
            try {
                transactionTemplate.execute(new TransactionCallbackWithoutResult() {
                    ...
                    protected void doInTransactionWithoutResult(TransactionStatus status) {
                        ...
                        if (paoAlias == MspPaoNameAliasEnum.SERVICE_LOCATION) {
                            ...
                            if (meters.isEmpty()) {
                                multispeakEventLogService.meterNotFoundByPaoName(serviceLocationStr,
                                    SERV_LOC_CHANGED_STRING, mspVendor.getCompanyName());
                                ErrorObject err =
                                    mspObjectDao.getNotFoundErrorObject(serviceLocationStr, "ServiceLocation",
                                ...
                                for (YukonMeter meter : meters) {
                                    if (!meter.isDisabled()) { // check if disabled here, since we are
                                                               // searching Yukon for potential service
                                                               // location matches (which _could_ be
                                                               // disabled?)
                                        // update the billing group from CIS billingCyle
                                        String billingCycle = mspServiceLocation.getBillingCycle();
                                        ...
                                        // TODO log something about meter not updated because disabled? Never
                                        // did before in 6.0 either
                                    }
                                }
                            }
                        } else {
                            // Must get meters from MSP CB call to process.
                            List<Meter> mspMeters =
                            ...
                            if (!mspMeters.isEmpty()) {
                                ...
                                for (Meter mspMeter : mspMeters) {
                                    try {
                                        ...
                                        // MeterNumber should not have changed, nor transponderId...only
                                        // paoName possibly
                                        String newPaoName = getPaoNameFromMspMeter(mspMeter, mspVendor);
                                        ...
                                        verifyAndUpdateSubstationGroupAndRoute(meter, mspVendor, mspMeter,
                                            mspServiceLocation, METER_ADD_STRING);
                                    } catch (NotFoundException e) {
                                        multispeakEventLogService.meterNotFound(mspMeter.getMeterNo(),
                                            SERV_LOC_CHANGED_STRING, mspVendor.getCompanyName());
                                        ErrorObject err =
                                            mspObjectDao.getNotFoundErrorObject(mspMeter.getMeterNo(), "MeterNumber",
                                                "Meter", SERV_LOC_CHANGED_STRING, mspVendor.getCompanyName());
                                        errorObjects.add(err);
                                ...
                                multispeakEventLogService.objectNotFoundByVendor(mspServiceLocation.getObjectID(),
                                    "getMeterByServLoc", SERV_LOC_CHANGED_STRING, mspVendor.getCompanyName());
                                ErrorObject err =
                ...
                // Transactional code threw application exception -> rollback
                ErrorObject err =
                    mspObjectDao.getErrorObject(mspServiceLocation.getObjectID(), "X Exception: (ServLoc:"
                        + mspServiceLocation.getObjectID() + ")-" + ex.getMessage(), "ServiceLocation",
                        SERV_LOC_CHANGED_STRING, mspVendor.getCompanyName());
                errorObjects.add(err);
                multispeakEventLogService.errorObject(err.getErrorString(), SERV_LOC_CHANGED_STRING,
                    mspVendor.getCompanyName());
                ...
                // Transactional code threw error -> rollback
                ErrorObject err =
                    mspObjectDao.getErrorObject(mspServiceLocation.getObjectID(), "X Error: (ServLoc:"
                        + mspServiceLocation.getObjectID() + ")-" + ex.getMessage(), "ServiceLocation",
                        SERV_LOC_CHANGED_STRING, mspVendor.getCompanyName());
                errorObjects.add(err);
                multispeakEventLogService.errorObject(err.getErrorString(), SERV_LOC_CHANGED_STRING,
    ...
    public ErrorObject[] meterChanged(final MultispeakVendor mspVendor, Meter[] changedMeters)
        ...
        for (final Meter mspMeter : changedMeters) {
            try {
                transactionTemplate.execute(new TransactionCallbackWithoutResult() {
                    ...
                    protected void doInTransactionWithoutResult(TransactionStatus status) {
                        ...
                        try {
                            ...
                            YukonMeter templateMeter = getYukonMeterForTemplate(mspMeter, mspVendor);
                            ...
                            multispeakEventLogService.meterNotFound(mspMeter.getMeterNo(), METER_CHANGED_STRING,
                                mspVendor.getCompanyName());
                            ErrorObject err =
                                mspObjectDao.getNotFoundErrorObject(mspMeter.getObjectID(),
                                    "MeterNumber: " + mspMeter.getMeterNo(), "Meter", METER_CHANGED_STRING,
                                    mspVendor.getCompanyName());
                            errorObjects.add(err);
                            multispeakEventLogService.errorObject(err.getErrorString(), METER_CHANGED_STRING,
                                mspVendor.getCompanyName());
                            log.error(e);
                        }
                    };
                ...
                errorObjects.add(e.getErrorObject());
                multispeakEventLogService.errorObject(e.getErrorObject().getErrorString(), METER_CHANGED_STRING,
                    mspVendor.getCompanyName());
                log.error(e);
            } catch (RuntimeException ex) {
                // Transactional code threw application exception -> rollback
                ErrorObject err =
                    mspObjectDao.getErrorObject(mspMeter.getMeterNo(), "X Exception: (MeterNo:" + mspMeter.getMeterNo()
                        + ")-" + ex.getMessage(), "Meter", METER_CHANGED_STRING, mspVendor.getCompanyName());
                errorObjects.add(err);
                multispeakEventLogService.errorObject(err.getErrorString(), METER_CHANGED_STRING,
                    mspVendor.getCompanyName());
                log.error(ex);
    ...
    public ErrorObject[] addMetersToGroup(MeterGroup meterGroup, String mspMethod, MultispeakVendor mspVendor) {
        ...
        if (meterGroup.getMeterList() != null) {
            for (String meterNumber : meterGroup.getMeterList().getMeterID()) {
                try {
                    ...
                    try {
                        ...
                        ErrorObject errorObject =
                            mspObjectDao.getNotFoundErrorObject(meterNumber, "MeterNumber", "Meter",
                                "removeMetersFromGroup", mspVendor.getCompanyName());
                        errorObjects.add(errorObject);
                        log.error(e);
                    }
                }
    ...
    public ErrorObject deleteGroup(String groupName, MultispeakVendor mspVendor) {
        try {
            ...
            ErrorObject errorObject =
                mspObjectDao.getNotFoundErrorObject(groupName, "meterGroupId", "MeterGroup", "deleteGroup",
                    mspVendor.getCompanyName());
            return errorObject;
        }
    ...
    public boolean updateAltGroup(ServiceLocation mspServiceLocation, String meterNumber, YukonDevice yukonDevice,
        ...
        boolean updateAltGroup =
            configurationSource.getBoolean(MasterConfigBooleanKeysEnum.MSP_ENABLE_ALTGROUP_EXTENSION);
        if (updateAltGroup) {
            List<ExtensionsItem> extensionItems = mspServiceLocation.getExtensionsList().getExtensionsItem();
            String extensionName =
                configurationSource.getString(MasterConfigStringKeysEnum.MSP_ALTGROUP_EXTENSION, "altGroup");
     ...
     */
    private boolean updatePrefixGroup(String groupName, String meterNumber, YukonDevice yukonDevice, String mspMethod,
            MultispeakVendor mspVendor, StoredDeviceGroup deviceGroupParent) {
        ...
        if (!alreadyInGroup) {
            ...
            if (numAffected > 0) {
                multispeakEventLogService.addMeterToGroup(meterNumber, deviceGroup.getFullName(), mspMethod,
                    mspVendor.getCompanyName());
                systemLog(mspMethod, "MeterNumber(" + meterNumber + ") - Added to Group: " + deviceGroup.getFullName()
                    + ".", mspVendor);
            }
        }
    ...
    private ErrorObject[] addToGroup(String[] meterNos, SystemGroupEnum systemGroup, String mspMethod,
        ...
        for (String meterNumber : meterNos) {
            try {
                ...
                ErrorObject err =
                    mspObjectDao.getNotFoundErrorObject(meterNumber, "MeterNumber", "Meter", "addToGroup",
                        mspVendor.getCompanyName());
                errorObjects.add(err);
                log.error(e);
            }
    ...
    private ErrorObject[] removeFromGroup(String[] meterNos, SystemGroupEnum systemGroup, String mspMethod,
        ...
        for (String meterNumber : meterNos) {
            try {
                ...
                multispeakEventLogService.meterNotFound(meterNumber, mspMethod, mspVendor.toString());
                ErrorObject err =
                    mspObjectDao.getNotFoundErrorObject(meterNumber, "MeterNumber", "Meter", "removeFromGroup",
                        mspVendor.getCompanyName());
                errorObjects.add(err);
                log.error(e);
            }
    ...
    private String getSubstationNameFromMspObjects(Meter mspMeter, ServiceLocation mspServiceLocation,
        ...
        boolean useExtension =
            configurationSource.getBoolean(MasterConfigBooleanKeysEnum.MSP_ENABLE_SUBSTATIONNAME_EXTENSION);
        if (useExtension) { // custom for DEMCO/SEDC integration
            String extensionName =
                configurationSource.getString(MasterConfigStringKeysEnum.MSP_SUBSTATIONNAME_EXTENSION, "readPath");
            String extensionValue;

            if (mspMeter != null) {
                log.debug("Attempting to load extension value for substation name from multispeak _meter_.");
                extensionValue =
                    getExtensionValue(mspMeter.getExtensionsList().getExtensionsItem(), extensionName, null);
                if (StringUtils.isNotBlank(extensionValue)) {
                    return extensionValue;
                }

                log.debug("Not found in meter. Attempting to load extension value for substation name "
                    + "from multispeak _serviceLocation_.");
                if (mspServiceLocation == null) {
            ...
            if (mspServiceLocation != null) {
                extensionValue =
                    getExtensionValue(mspServiceLocation.getExtensionsList().getExtensionsItem(), extensionName, null);
                if (StringUtils.isNotBlank(extensionValue)) {
                    return extensionValue;
                }
            }

            log.debug("Extension value for substation name NOT found for meter or service location, "
                + "returning empty substationName.");
            return "";

        } else { // Normal loading
            if (mspMeter == null || mspMeter.getUtilityInfo() == null
                || StringUtils.isBlank(mspMeter.getUtilityInfo().getSubstationName())) {
                return null;
            } else {
    ...
    private YukonMeter getYukonMeterForTemplate(Meter mspMeter, MultispeakVendor mspVendor)
        ...
        try {
            ...
            ErrorObject err =
                mspObjectDao.getErrorObject(mspMeter.getObjectID(), "Error: Meter (" + mspMeter.getMeterNo()
                    + ") - does not contain a valid template meter: Template[" + templateName
    ...
    private void validateMspMeter(Meter mspMeter, MultispeakVendor mspVendor, String method)
        ...
        if (StringUtils.isBlank(mspMeter.getMeterNo())) {
            ...
            ErrorObject errorObject =
                mspObjectDao.getErrorObject(mspMeter.getMeterNo(),
                    "Error: MeterNo is invalid (empty or null).  No updates were made.", "Meter", method,
                    mspVendor.getCompanyName());
            throw new MspErrorObjectException(errorObject);
        }
        ...
        if (mspMeter.getNameplate() == null || StringUtils.isBlank(mspMeter.getNameplate().getTransponderID())) {
            ...
            ErrorObject errorObject =
                mspObjectDao.getErrorObject(mspMeter.getMeterNo(), "Error: MeterNumber(" + mspMeter.getMeterNo()
                    + ") - SerialNumber nor TransponderId are valid.  No updates were made.", "Meter", method,
                    mspVendor.getCompanyName());
            throw new MspErrorObjectException(errorObject);
        }
    }
