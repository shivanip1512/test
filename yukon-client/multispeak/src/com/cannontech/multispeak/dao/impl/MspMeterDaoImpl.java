...
public final class MspMeterDaoImpl implements MspMeterDao {
    ...
    public MspMeterReturnList getAMRSupportedMeters(String lastReceived, int maxRecords) {
        ...
        CollectionRowCallbackHandler<Meter> crcHandler =
            new CollectionRowCallbackHandler<Meter>(mspMeterRowMapper, mspMeters);
    ...
    private static Meter createMeter(YukonResultSet rset) throws SQLException {
        ...
        meter.setMeterType(paoIdentifier.getPaoType().getPaoTypeName()); // Meter type/model. Always use
                                                                         // paoType
        ...
        // meter.setAMRDeviceType(paoType); //This is the Yukon Template Field, Yukon doesn't store what this
        // meter used as it's template.
        ...
        if (StringUtils.isNotBlank(carrierAddress)) {
            ...
            meter.setSerialNumber(rfnSerialNumber); // Meter serial number. This is the original number
                                                    // assigned to the meter by the manufacturer.
        }

        // meter.setSealNumberList(null); //List of seals applied to this meter.
        // meter.setUtilityInfo(null); //This information relates the meter to the account with which it is
        // associated
        ...
        // MSPDevice
        // meter.setDeviceClass(null); //A high-level description of this type of object (e.g., "kWh meter",
        // "demand meter", etc.).
