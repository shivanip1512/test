...
public class MspRawPointHistoryDaoImpl implements MspRawPointHistoryDao {
    ...
    public MspMeterReadReturnList retrieveMeterReads(ReadBy readBy, String readByValue, Date startDate, Date endDate,
        ...
        for (BuiltInAttribute attribute : attributesToLoad) {
            ...
            ListMultimap<PaoIdentifier, PointValueQualityHolder> resultsForAttribute =
                rawPointHistoryDao.getAttributeData(meters, attribute, startDate, endDate, false,
                    Clusivity.INCLUSIVE_INCLUSIVE, Order.FORWARD, null);

            resultsPerAttribute.put(attribute, resultsForAttribute);
            estimatedSize += resultsForAttribute.size();
        }

        // build the actual MeterRead objects in the order they are to be output getPaoList returns the meters
        // in
     ...
     * @return
     */
    private List<YukonMeter> getPaoList(ReadBy readBy, String readByValue, String lastReceived, int maxRecords) {
        final Date timerStart = new Date();
