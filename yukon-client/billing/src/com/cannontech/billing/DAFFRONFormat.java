 ...
 * @author: 
 */
public class DAFFRONFormat extends FileFormatBase 
{
...
private void removeDuplicateMeters() 
{
	...
	for( int i = itemPos; i >= 0; i--, j-- )
	{
		...
		if( j >=0 )
		{
			...
			{
				if( ((com.cannontech.billing.record.DAFFRONRecord)getRecordVector().get(i)).getReadingKWH().doubleValue() >=
					    ((com.cannontech.billing.record.DAFFRONRecord)getRecordVector().get(j)).getReadingKWH().doubleValue() )
...
*/
				...
				if( currentPointID != lastPointID )	//just getting max time for each point
				{
					...
					if( currentDeviceID == lastDeviceID)
					{
						if (ptOffset == 1 || isKWH(ptOffset))
						{
							...
							com.cannontech.billing.record.DAFFRONRecord lastRecord =
								(com.cannontech.billing.record.DAFFRONRecord)getRecordVector().get(recCount -1);
					...
					{
						com.cannontech.billing.record.DAFFRONRecord daffronRec = 
							new com.cannontech.billing.record.DAFFRONRecord(meterNumber);
						if (ptOffset == 1 || isKWH(ptOffset))
						{
							if( tsDate.compareTo((Object)getBillingDefaults().getEnergyStartDate()) <= 0) //ts <= mintime, fail!
								break inValidTimestamp;
							
							daffronRec.setReadingKWH(reading);
							daffronRec.setTime(ts);
							daffronRec.setDate(ts);
						...
						else if (isKW(ptOffset))
						{
							...
							daffronRec.setPeakReadingKW(reading);
							daffronRec.setPeakTimeKW(ts);
							daffronRec.setPeakDateKW(ts);
						}
						lastDeviceID = currentDeviceID;
						getRecordVector().addElement(daffronRec);
						recCount++;
					}
				}
			}
	...
	com.cannontech.clientutils.CTILogger.info(" @DAFFRON Data Collection : Took " + (System.currentTimeMillis() - timer));
	return true;
}
}
