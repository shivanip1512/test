...
public class UpdateThermostatScheduleAction implements ActionBase {
	...
	public SOAPMessage build(HttpServletRequest req, HttpSession session) {
		try {
			...
			if (req.getParameter("ApplyToWeekend") != null)
				session.setAttribute(ServletUtils.TRANSIENT_ATT_LEADING + ServletUtils.ATT_APPLY_TO_WEEKEND, "checked");
			else
				session.removeAttribute(ServletUtils.TRANSIENT_ATT_LEADING + ServletUtils.ATT_APPLY_TO_WEEKEND);
			if (req.getParameter("ApplyToWeekdays") != null)
				session.setAttribute(ServletUtils.TRANSIENT_ATT_LEADING + ServletUtils.ATT_APPLY_TO_WEEKDAYS, "checked");
			else
				session.removeAttribute(ServletUtils.TRANSIENT_ATT_LEADING + ServletUtils.ATT_APPLY_TO_WEEKDAYS);
				...
				for (int j = 0; j < accountInfo.getStarsInventories().getStarsInventoryCount(); j++) {
					...
					if (inv.getInventoryID() == invIDs[i]) {
						...
						if (settings.getStarsThermostatDynamicData() == null)
							parseResponse( resp, settings.getStarsThermostatProgram() );
		...
		try {
			...
			for (int i = 0; i < updateSched.getStarsThermostatSeasonCount(); i++) {
				...
				for (int j = 0; j < starsSeason.getStarsThermostatScheduleCount(); j++) {
					...
					if (starsSched.getDay().getType() == StarsThermoDaySettings.ALL_TYPE)
					{
						if (hwTypeDefID == YukonListEntryTypes.YUK_DEF_ID_DEV_TYPE_ENERGYPRO) {
						...
						hwTypeDefID == YukonListEntryTypes.YUK_DEF_ID_DEV_TYPE_ENERGYPRO)
					{
	...
	public static void parseResponse(StarsUpdateThermostatScheduleResponse resp, StarsThermostatProgram schedule) {
		for (int i = 0; i < resp.getStarsThermostatSeasonCount(); i++) {
			...
			for (int j = 0; j < schedule.getStarsThermostatSeasonCount(); j++) {
				StarsThermostatSeason season = schedule.getStarsThermostatSeason(j);
			...
			if (oldSeason == null) {
				...
				schedule.addStarsThermostatSeason( oldSeason );
			}
