...
public class UpdateThermostatScheduleAction implements ActionBase {
	...
	public SOAPMessage build(HttpServletRequest req, HttpSession session) {
		try {
			...
			if (req.getParameter("ApplyToWeekend") != null)
				user.setAttribute(
					ServletUtils.TRANSIENT_ATT_LEADING
						+ ServletUtils.ATT_APPLY_TO_WEEKEND,
					"checked");
			else
			...
			if (req.getParameter("ApplyToWeekdays") != null)
				user.setAttribute(
					ServletUtils.TRANSIENT_ATT_LEADING
						+ ServletUtils.ATT_APPLY_TO_WEEKDAYS,
					"checked");
			else
				user.removeAttribute(
					ServletUtils.TRANSIENT_ATT_LEADING
		...
		try {
			...
			if (liteAcctInfo == null) {
				respOper.setStarsFailure(
					StarsFactory.newStarsFailure(
						...
						"Cannot find customer account information, please login again"));
				return SOAPUtil.buildSOAPMessage(respOper);
			}

			int energyCompanyID = user.getEnergyCompanyID();
			LiteStarsEnergyCompany energyCompany =
				SOAPServer.getEnergyCompany(energyCompanyID);

			StarsUpdateThermostatSchedule updateSched =
				reqOper.getStarsUpdateThermostatSchedule();
			...
			for (int i = 0; i < invIDs.length; i++) {
				LiteStarsLMHardware liteHw =
					(LiteStarsLMHardware) energyCompany.getInventory(
						invIDs[i],
						true);

				if (liteHw.getDeviceStatus()
					== YukonListEntryTypes.YUK_DEF_ID_DEV_STAT_UNAVAIL) {
					String errorMsg =
						ServerUtils.isOperator(user)
							? ((invIDs.length == 1)
				...
				if (liteHw.getManufacturerSerialNumber().trim().length()
					== 0) {
					String errorMsg =
						ServerUtils.isOperator(user)
				...
				int hwTypeDefID =
					YukonListFuncs
						.getYukonListEntry(liteHw.getLmHardwareTypeID())
						.getYukonDefID();
				LiteStarsThermostatSettings dftSettings =
					energyCompany
						.getDefaultLMHardware(hwTypeDefID)
						.getThermostatSettings();
				LiteStarsThermostatSettings liteSettings =
					liteHw.getThermostatSettings();

				int mondayID =
					energyCompany
						.getYukonListEntry(
							YukonListEntryTypes.YUK_DEF_ID_TOW_MONDAY)
						.getEntryID();
				int weekdayID =
					energyCompany
						.getYukonListEntry(
							YukonListEntryTypes.YUK_DEF_ID_TOW_WEEKDAY)
						.getEntryID();
				...
				for (int j = 0;
					j < updateSched.getStarsThermostatSeasonCount();
					j++) {
					StarsThermostatSeason starsSeason =
						updateSched.getStarsThermostatSeason(j);

					for (int k = 0;
						k < starsSeason.getStarsThermostatScheduleCount();
						k++) {
						StarsThermostatSchedule starsSched =
							starsSeason.getStarsThermostatSchedule(k);
						int towID =
							ECUtils.getThermSeasonEntryTOWID(
								starsSched.getDay(),
								energyCompany);

						ArrayList oldSched = new ArrayList();
						for (int l = 0;
							l < liteSettings.getThermostatSeasons().size();
							l++) {
							LiteLMThermostatSeason liteSeason =
								(LiteLMThermostatSeason) liteSettings
									.getThermostatSeasons()
									.get(
									l);
							if (liteSeason.getWebConfigurationID()
								== ECUtils.YUK_WEB_CONFIG_ID_COOL
								&& starsSeason.getMode().getType()
									== StarsThermoModeSettings.COOL_TYPE
								|| liteSeason.getWebConfigurationID()
									== ECUtils.YUK_WEB_CONFIG_ID_HEAT
								&& starsSeason.getMode().getType()
									== StarsThermoModeSettings.HEAT_TYPE) {
								for (int m = 0;
									m < liteSeason.getSeasonEntries().size();
									m++) {
									LiteLMThermostatSeasonEntry liteEntry =
										(LiteLMThermostatSeasonEntry) liteSeason
											.getSeasonEntries()
											.get(
											m);
						...
						for (int l = 0;
							l < dftSettings.getThermostatSeasons().size();
							l++) {
							LiteLMThermostatSeason liteSeason =
								(LiteLMThermostatSeason) dftSettings
									.getThermostatSeasons()
									.get(
									l);
							if (liteSeason.getWebConfigurationID()
						...
						String dayStr = null;
						if (starsSched.getDay().getType()
							== StarsThermoDaySettings.ALL_TYPE)
							dayStr = "all";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.WEEKDAY_TYPE)
							dayStr = "weekday";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.WEEKEND_TYPE)
							dayStr = "weekend";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.SATURDAY_TYPE)
							dayStr = "sat";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.SUNDAY_TYPE)
							dayStr = "sun";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.MONDAY_TYPE)
							dayStr = "mon";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.TUESDAY_TYPE)
							dayStr = "tue";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.WEDNESDAY_TYPE)
							dayStr = "wed";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.THURSDAY_TYPE)
							dayStr = "thu";
						else if (
							starsSched.getDay().getType()
								== StarsThermoDaySettings.FRIDAY_TYPE)
							dayStr = "fri";
						else
							throw new Exception(
								"Invalid thermostat schedule attribute: day = "
									+ starsSched.getDay().toString());
						...
						boolean oldSkip1 = false,
							oldSkip2 = false,
							oldSkip3 = false,
							oldSkip4 = false;
						if (oldSched.size() == 4) {
							oldSkip1 =
								((LiteLMThermostatSeasonEntry) oldSched.get(0))
									.getTemperature()
									== -1;
							oldSkip2 =
								((LiteLMThermostatSeasonEntry) oldSched.get(1))
									.getTemperature()
									== -1;
							oldSkip3 =
								((LiteLMThermostatSeasonEntry) oldSched.get(2))
									.getTemperature()
									== -1;
							oldSkip4 =
								((LiteLMThermostatSeasonEntry) oldSched.get(3))
									.getTemperature()
									== -1;
						}
						...
						String time1 =
							(skip1)
								? "HH:MM"
								: starsSched.getTime1().toString().substring(
									0,
									5);
						String time2 =
							(skip2)
								? "HH:MM"
								: starsSched.getTime2().toString().substring(
									0,
									5);
						String time3 =
							(skip3)
								? "HH:MM"
								: starsSched.getTime3().toString().substring(
									0,
									5);
						String time4 =
							(skip4)
								? "HH:MM"
								: starsSched.getTime4().toString().substring(
									0,
									5);
						...
						if (isCool) {
							if (!skip1) {
								temp1C =
									String.valueOf(
										starsSched.getTemperature1());
								if (oldSkip1)
									temp1H =
										String.valueOf(
											(
												(LiteLMThermostatSeasonEntry) dftOtherSched
												.get(0))
												.getTemperature());
							}
							if (!skip2) {
						...
						cmd
							...
							.append(temp4C)
							.append(" serial ")
							.append(liteHw.getManufacturerSerialNumber());
						...
						if (hwTypeDefID
							== YukonListEntryTypes
								.YUK_DEF_ID_DEV_TYPE_COMM_EXPRESSSTAT) {
							logMsg += "Occupied:"
								+ starsSched.getTime4().toString().substring(
									0,
									5)
						...
						ActivityLogger.logEvent(
							user.getUserID(),
							liteAcctInfo.getAccountID(),
							energyCompany.getLiteID(),
							liteAcctInfo.getCustomer().getCustomerID(),
				...
				com
					...
					.stars
					.event
					.LMCustomerEventBase eventBase =
					event.getLMCustomerEventBase();

				eventDB.setInventoryID(new Integer(invIDs[i]));
				eventBase.setEventTypeID(
					new Integer(
						energyCompany
							.getYukonListEntry(
								YukonListEntryTypes
									.YUK_DEF_ID_CUST_EVENT_LMHARDWARE)
							.getEntryID()));
				eventBase.setActionID(
					new Integer(
						energyCompany
							.getYukonListEntry(
								YukonListEntryTypes
									.YUK_DEF_ID_CUST_ACT_PROGRAMMING)
							.getEntryID()));
				...
				event =
					(com
						...
						.stars
						.event
						.LMHardwareEvent) Transaction
						.createTransaction(Transaction.INSERT, event)
						.execute();

				LiteLMCustomerEvent liteEvent =
					(LiteLMCustomerEvent) StarsLiteFactory.createLite(event);
				...
				if (resp == null) {
					resp =
						updateThermostatSchedule(
							...
							liteHw,
							energyCompany);
					StarsLMHardwareEvent starsEvent =
						new StarsLMHardwareEvent();
					StarsLiteFactory.setStarsLMCustomerEvent(
						starsEvent,
						liteEvent);
			...
			if (hasTwoWay) {
				try {
					Thread.sleep(3 * 1000);
					// Wait a while for the new settings to be reflected in the table
			...
			try {
				respOper.setStarsFailure(
					StarsFactory.newStarsFailure(
						StarsConstants.FAILURE_CODE_OPERATION_FAILED,
						"Cannot update thermostat schedules"));
				return SOAPUtil.buildSOAPMessage(respOper);
			} catch (Exception e2) {
	...
	public int parse(
		...
		try {
			...
			StarsUpdateThermostatScheduleResponse resp =
				operation.getStarsUpdateThermostatScheduleResponse();
			if (resp == null)
			...
			for (int i = 0; i < invIDs.length; i++) {
				for (int j = 0;
					j
						< accountInfo
							.getStarsInventories()
							.getStarsInventoryCount();
					j++) {
					StarsInventory inv =
						accountInfo.getStarsInventories().getStarsInventory(j);
					if (inv.getInventoryID() == invIDs[i]) {
						if (resp.getStarsLMHardwareEvent() != null)
							inv
								.getStarsLMHardwareHistory()
								.addStarsLMHardwareEvent(
								resp.getStarsLMHardwareEvent());

						StarsThermostatSettings settings =
							inv.getLMHardware().getStarsThermostatSettings();
			...
			session.setAttribute(
				ServletUtils.ATT_CONFIRM_MESSAGE,
	...
	public static StarsOperation getRequestOperation(HttpServletRequest req)
		...
		StarsUpdateThermostatSchedule updateSched =
			new StarsUpdateThermostatSchedule();

		String[] invIDStrs = req.getParameterValues("InvIDs");
		if (invIDStrs == null || invIDStrs.length == 0) {
			updateSched.setInventoryID(
				Integer.parseInt(req.getParameter("InvID")));
		} else {
		...
		StarsThermoModeSettings mode =
			StarsThermoModeSettings.valueOf(req.getParameter("mode"));
		StarsThermoDaySettings day =
			StarsThermoDaySettings.valueOf(req.getParameter("day"));
		...
		if (req.getParameter("time1") != null) {
			Date time1 =
				ServletUtils.parseTime(
					req.getParameter("time1"),
					TimeZone.getDefault());
			if (time1 == null)
				throw new WebClientException(
					"Invalid time format '" + req.getParameter("time1") + "'");

			cal.setTime(time1);
			schedule.setTime1(
				new org.exolab.castor.types.Time(
					(cal.get(Calendar.HOUR_OF_DAY) * 3600
						+ cal.get(Calendar.MINUTE) * 60)
						* 1000));
			schedule.setTemperature1(
				(noScript)
		...
		if (req.getParameter("time2") != null) {
			Date time2 =
				ServletUtils.parseTime(
					req.getParameter("time2"),
					TimeZone.getDefault());
			if (time2 == null)
				throw new WebClientException(
					"Invalid time format '" + req.getParameter("time2") + "'");
		...
		if (req.getParameter("time3") != null) {
			Date time3 =
				ServletUtils.parseTime(
					req.getParameter("time3"),
					TimeZone.getDefault());
			if (time3 == null)
				throw new WebClientException(
					"Invalid time format '" + req.getParameter("time3") + "'");
		...
		if (req.getParameter("time4") != null) {
			Date time4 =
				ServletUtils.parseTime(
					req.getParameter("time4"),
					TimeZone.getDefault());
			if (time4 == null)
				throw new WebClientException(
					"Invalid time format '" + req.getParameter("time4") + "'");
		...
		if (type != null
			&& type.equalsIgnoreCase(StarsThermostatTypes.ENERGYPRO.toString())) {
			...
			if (applyToWeekdaysStr != null && applyToWeekendStr != null) {
				...
				if (ServletUtils.isWeekday(day)) {
					...
					StarsThermostatSchedule schedule2 =
						StarsFactory.newStarsThermostatSchedule(schedule);
	...
	public static StarsUpdateThermostatScheduleResponse updateThermostatSchedule(
		...
		throws Exception {
		StarsUpdateThermostatScheduleResponse resp =
			new StarsUpdateThermostatScheduleResponse();

		int hwTypeDefID =
			YukonListFuncs
				.getYukonListEntry(liteHw.getLmHardwareTypeID())
				.getYukonDefID();
		LiteStarsThermostatSettings liteDftSettings =
			energyCompany
				.getDefaultLMHardware(hwTypeDefID)
				.getThermostatSettings();
		LiteStarsThermostatSettings liteSettings =
		...
		try {
			...
			for (int i = 0;
				i < updateSched.getStarsThermostatSeasonCount();
				i++) {
				StarsThermostatSeason starsSeason =
					updateSched.getStarsThermostatSeason(i);
				int webConfigID =
					(starsSeason.getMode().getType()
						== StarsThermoModeSettings.COOL_TYPE)
						? ECUtils.YUK_WEB_CONFIG_ID_COOL
				...
				for (int j = 0;
					j < liteSettings.getThermostatSeasons().size();
					j++) {
					LiteLMThermostatSeason lSeason =
						(LiteLMThermostatSeason) liteSettings
							.getThermostatSeasons()
							.get(
							j);
				...
				for (int j = 0;
					j < liteDftSettings.getThermostatSeasons().size();
					j++) {
					LiteLMThermostatSeason lSeason =
						(LiteLMThermostatSeason) liteDftSettings
							.getThermostatSeasons()
							.get(
							j);
					if (lSeason.getWebConfigurationID() == webConfigID)
				...
				if (liteSeason == null) {
					...
					season =
						(LMThermostatSeason) Transaction
							.createTransaction(Transaction.INSERT, season)
							.execute();

					liteSeason =
						(LiteLMThermostatSeason) StarsLiteFactory.createLite(
							season);
				...
				if (liteSeason2 == null) {
					...
					season2.setInventoryID(
						new Integer(liteHw.getInventoryID()));
					season2.setWebConfigurationID(
						new Integer(liteDftSeason2.getWebConfigurationID()));
					season2.setStartDate(
						new Date(liteDftSeason2.getStartDate()));
					season2 =
						(LMThermostatSeason) Transaction
							.createTransaction(Transaction.INSERT, season2)
							.execute();
				...
				for (int j = 0;
					j < resp.getStarsThermostatSeasonCount();
					j++) {
					if (resp.getStarsThermostatSeason(j).getMode().getType()
						== starsSeason.getMode().getType())
				...
				if (starsRespSeason2 == null) {
					...
					starsRespSeason2.setMode(
						ECUtils.getThermSeasonMode(
							liteSeason2.getWebConfigurationID()));
					resp.addStarsThermostatSeason(starsRespSeason2);
				}

				for (int j = 0;
					j < starsSeason.getStarsThermostatScheduleCount();
					j++) {
					StarsThermostatSchedule starsSched =
						starsSeason.getStarsThermostatSchedule(j);
					int[] towIDs = null;
							...
							towIDs =
								new int[] {
									energyCompany
										.getYukonListEntry(
											YukonListEntryTypes
												.YUK_DEF_ID_TOW_MONDAY)
										.getEntryID(),
									energyCompany
										.getYukonListEntry(
											YukonListEntryTypes
												.YUK_DEF_ID_TOW_TUESDAY)
										.getEntryID(),
									energyCompany
										.getYukonListEntry(
											YukonListEntryTypes
												.YUK_DEF_ID_TOW_WEDNESDAY)
										.getEntryID(),
									energyCompany
										.getYukonListEntry(
											YukonListEntryTypes
												.YUK_DEF_ID_TOW_THURSDAY)
										.getEntryID(),
									energyCompany
										.getYukonListEntry(
											YukonListEntryTypes
												.YUK_DEF_ID_TOW_FRIDAY)
										.getEntryID(),
									energyCompany
										.getYukonListEntry(
											YukonListEntryTypes
												.YUK_DEF_ID_TOW_SATURDAY)
										.getEntryID(),
									energyCompany
										.getYukonListEntry(
											YukonListEntryTypes
												.YUK_DEF_ID_TOW_SUNDAY)
										.getEntryID()};
						} else {
					...
					int[] times =
						{
							starsSched.getTime1().getHour() * 3600
								+ starsSched.getTime1().getMinute() * 60
								+ starsSched.getTime1().getSeconds(),
							starsSched.getTime2().getHour() * 3600
								+ starsSched.getTime2().getMinute() * 60
								+ starsSched.getTime2().getSeconds(),
							starsSched.getTime3().getHour() * 3600
								+ starsSched.getTime3().getMinute() * 60
								+ starsSched.getTime3().getSeconds(),
							starsSched.getTime4().getHour() * 3600
								+ starsSched.getTime4().getMinute() * 60
								+ starsSched.getTime4().getSeconds()};
					int[] temps =
						{
							starsSched.getTemperature1(),
							starsSched.getTemperature2(),
					...
					for (int k = 0; k < towIDs.length; k++) {
						...
						if (liteSeason.getSeasonEntries() != null) {
							for (int l = 0;
								l < liteSeason.getSeasonEntries().size();
								l++) {
								LiteLMThermostatSeasonEntry liteEntry =
									(LiteLMThermostatSeasonEntry) liteSeason
										.getSeasonEntries()
						...
						if (liteSeason2.getSeasonEntries() != null) {
							for (int l = 0;
								l < liteSeason2.getSeasonEntries().size();
								l++) {
								LiteLMThermostatSeasonEntry liteEntry =
									(LiteLMThermostatSeasonEntry) liteSeason2
										.getSeasonEntries()
										.get(
										l);
								if (liteEntry.getTimeOfWeekID() == towIDs[k])
						...
						if (liteEntries.size() > 0
							&& liteEntries.size() != 4) {
							// Currently this should not happen, so remove these entries
							throw new Exception(
								"Invalid number of thermostat season entries: "
									+ liteEntries.size()
									+ ", for season id = "
									+ liteSeason.getSeasonID());
						} else if (liteEntries.size() == 4) {
							// Update the season entries
							for (int l = 0; l < 4; l++) {
								LiteLMThermostatSeasonEntry liteEntry =
									(
										LiteLMThermostatSeasonEntry) liteEntries
											.get(
										l);
								liteEntry.setStartTime(times[l]);
								liteEntry.setTemperature(temps[l]);

								LMThermostatSeasonEntry entry =
									(
										LMThermostatSeasonEntry) StarsLiteFactory
											.createDBPersistent(
										liteEntry);
							...
							for (int l = 0; l < 4; l++) {
								LMThermostatSeasonEntry entry =
									new LMThermostatSeasonEntry();
								entry.setSeasonID(
									new Integer(liteSeason.getSeasonID()));
								...
								LiteLMThermostatSeasonEntry liteEntry =
									(
										LiteLMThermostatSeasonEntry) StarsLiteFactory
											.createLite(
										entry);
						...
						if (liteEntries2.size() > 0
							&& liteEntries2.size() != 4) {
							throw new Exception(
								"Invalid number of thermostat season entries: "
									+ liteEntries2.size()
									+ ", for season id = "
									+ liteSeason2.getSeasonID());
						} else if (liteEntries2.size() == 4) {
							// Update the season entries for the other season
							for (int l = 0; l < 4; l++) {
								LiteLMThermostatSeasonEntry liteEntry =
									(
										LiteLMThermostatSeasonEntry) liteEntries2
											.get(
										l);
								liteEntry.setStartTime(times[l]);
							...
							for (int l = 0;
								l < liteDftSeason2.getSeasonEntries().size();
								l++) {
								LiteLMThermostatSeasonEntry liteEntry =
									(LiteLMThermostatSeasonEntry) liteDftSeason2
										.getSeasonEntries()
										.get(
										l);
								if (liteEntry.getTimeOfWeekID() == towIDs[k])
							...
							if (liteEntries2.size() != 4)
								throw new Exception(
									"Invalid number of thermostat season entries: "
										+ liteEntries2.size()
										+ ", for season id = "
										+ liteDftSeason2.getSeasonID());

							for (int l = 0; l < 4; l++) {
								LMThermostatSeasonEntry entry =
									new LMThermostatSeasonEntry();
								entry.setSeasonID(
									new Integer(liteSeason2.getSeasonID()));
								entry.setTimeOfWeekID(new Integer(towIDs[k]));
								entry.setStartTime(new Integer(times[l]));
						...
						// Build response message
						StarsThermostatSchedule starsRespSched =
							StarsLiteFactory.createStarsThermostatSchedule(
								towIDs[k],
								liteEntries);
						starsRespSeason.addStarsThermostatSchedule(
							starsRespSched);
						StarsThermostatSchedule starsRespSched2 =
							StarsLiteFactory.createStarsThermostatSchedule(
								towIDs[k],
								liteEntries2);
						starsRespSeason2.addStarsThermostatSchedule(
							starsRespSched2);
					}
				}
			}
	...
	public static void parseResponse(
		...
		for (int i = 0; i < resp.getStarsThermostatSeasonCount(); i++) {
			...
			for (int j = 0;
				j < settings.getStarsThermostatSeasonCount();
				j++) {
				StarsThermostatSeason season =
					settings.getStarsThermostatSeason(j);
				if (season.getMode().getType()
					== newSeason.getMode().getType()) {
					oldSeason = season;
					break;
				}
			}
			...
			for (int j = 0;
				j < newSeason.getStarsThermostatScheduleCount();
				j++) {
				StarsThermostatSchedule newSched =
					newSeason.getStarsThermostatSchedule(j);
				StarsThermoDaySettings daySetting = newSched.getDay();

				StarsThermostatSchedule oldSched = null;
				for (int k = 0;
					k < oldSeason.getStarsThermostatScheduleCount();
					k++) {
					StarsThermostatSchedule sched =
						oldSeason.getStarsThermostatSchedule(k);
