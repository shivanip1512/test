...
public class PointForm extends DBEditorForm {
    ...
    public SelectItem[] getInitialStates() {
        ...
        if (initialStates == null) {
            initialStates = new SelectItem[] { new SelectItem(0, CtiUtilities.STRING_NONE) };
        }
     ...
     *
     */
    public List<SelectItem> getUofMs() {
        
        if (uofms == null) {
            UnitMeasureDao unitMeasureDao = YukonSpringHook.getBean(UnitMeasureDao.class);
            
            List<LiteUnitMeasure> allUnitMeasures = unitMeasureDao.getLiteUnitMeasures();
            
            uofms = new ArrayList<>();
            for (LiteUnitMeasure measure : allUnitMeasures) {
                uofms.add(new SelectItem(measure.getUomID(), measure.getUnitMeasureName()));
            }
        }
        return uofms;
    }
        ...
        if (alarmCategories == null) {
            ...
            for (int i = 0; i < allAlarmStates.size(); i++) {
                ...
                alarmCategories[i] = new SelectItem(// value, label
                    lac.getCategoryName(), lac.getCategoryName());
            }
        }
        ...
        if (notifGroups == null) {
            ...
            synchronized (cache) {
                ...
                for (int i = 0; i < liteNotifGrps.size(); i++) {
                    ...
                    notifGroups[i] = new SelectItem(liteGroup.getNotificationGroupID(), liteGroup.toString());
                }
            }
        }
    ...
    public SelectItem[] getStateGroups() {
        ...
        if (stateGroups == null) {
            ...
            for (int i = 0; i < allStateGroups.length; i++) {
                ...
                stateGroups[i] = new SelectItem(grp.getStateGroupID(), grp.getStateGroupName());
            }
        }
    ...
    public void initItem(int id) {
        ...
        PointBase pointDB =
            (PointBase) LiteFactory.createDBPersistent(YukonSpringHook.getBean(PointDao.class).getLitePoint(id));
    ...
    private void initPanels() {
        ...
        getVisibleTabs().put("General", true);
        getVisibleTabs().put("Alarming", true);
        getVisibleTabs().put("FDR", true);

        // all type specifc panels
        getVisibleTabs().put("PointAnalog", false);
        getVisibleTabs().put("PointStatus", false);
        getVisibleTabs().put("PointCalcStatus", false);
        getVisibleTabs().put("PointAccum", false);
        getVisibleTabs().put("PointCalc", false);

        int ptType = PointTypes.getType(getPointBase().getPoint().getPointType());
        switch (ptType) {

        case PointTypes.ANALOG_POINT:
            getVisibleTabs().put("PointAnalog", true);
            break;

        case PointTypes.STATUS_POINT:
            getVisibleTabs().put("PointStatus", true);
            break;

        case PointTypes.CALCULATED_STATUS_POINT:
            getVisibleTabs().put("PointCalcStatus", true);
            updateTypeChanged(new ValueChangeEvent(DUMMY_UI, null,
                ((CalcStatusPoint) getPointBase()).getCalcBase().getUpdateType()));

            break;

        case PointTypes.DEMAND_ACCUMULATOR_POINT:
            getVisibleTabs().put("PointAccum", true);
            break;
        ...
        case PointTypes.CALCULATED_POINT:
            getVisibleTabs().put("PointCalc", true);
            updateTypeChanged(new ValueChangeEvent(DUMMY_UI, null,
        ...
        stateGroupChanged(new ValueChangeEvent(DUMMY_UI, null, getPointBase().getPoint().getStateGroupID().intValue()));

        archiveTypeChanged(new ValueChangeEvent(// src, oldValue, newValue
            DUMMY_UI, null, getPointBase().getPoint().getArchiveType()));
    ...
    public void stateGroupChanged(ValueChangeEvent ev) {
        ...
        LiteState[] lStates = YukonSpringHook.getBean(StateDao.class).getLiteStates(((Integer) ev.getNewValue()));
        initialStates = new SelectItem[lStates.length];
        for (int i = 0; i < lStates.length; i++)
            initialStates[i] = new SelectItem(new Integer(lStates[i].getLiteID()), lStates[i].getStateText());
    ...
    public void archiveTypeChanged(ValueChangeEvent ev) {
        ...
        isArchiveInterEnabled =
            (newVal.equalsIgnoreCase(PointArchiveType.ON_TIMER.getPointArchiveTypeName()) ||
    ...
    private void initAlarmTable() {
        ...
        String alarmStates = getPointBase().getPointAlarming().getAlarmStates().length() != PointAlarming.ALARM_STATE_COUNT ?
        ...
        LiteStateGroup stateGroup = stateDao.getLiteStateGroup(getPointBase().getPoint().getStateGroupID());
            ...
            for (int j = 0; j < stateNames.length; j++, i++) {
                if (i >= alarmStates.length()) {
                    throw new ArrayIndexOutOfBoundsException("Trying to get alarmStates[" + i + "] while alarmStates.length()==" + alarmStates.length() + 
                        ", too many states for Status point "+ getPointBase().getPoint().getPointName() + " defined.");
                }
                AlarmTableEntry entry = new AlarmTableEntry();
                setAlarmGenNotif(entry, alarmStates.charAt(i), allAlarmStates, excludeNotifyStates.toUpperCase().charAt(i));
    ...
    public void paoClick(ActionEvent ae) {
        ...
        try {
            // go to the next page
            int itemId =
                Integer.valueOf((String) FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get(
                    "paoID"));
    ...
    protected void checkForErrors() throws InvalidPointOffsetException, InvalidPointLimits {
        ...
        LitePoint litePoint = pointDao.getLitePoint(getPointBase().getPoint().getPointID());
        Integer paoId = getWizData().getParentId();
        if (!checkPointLimits())
        ...
        if (!PointOffsetUtils.isValidPointOffset(offset, paoId, type)) {
            throw new InvalidPointOffsetException("The point offset " + offset + "" + " is already in use by another "
                + getPointBase().getPoint().getPointType() + " point on this device.");
        }
    }

    /**
     * @return true if the user has the CapControl Settings > Database Editing role property.
     */
        ...
        if (getPointBase() instanceof AnalogPoint) {
            ...
            if (limitOne != null && limitTwo != null) {
                if ((limitOne.getHighLimit() < limitOne.getLowLimit()) ||
                    (limitTwo.getHighLimit() < limitTwo.getLowLimit()))
                    return false;
            }
        }
        return true;
