...
public class RolePropertyController {
	...
	public void setupHelperLookup() {
		helperLookup = new MapMaker()
				.concurrencyLevel(1)
				.makeComputingMap(
						new Function<YukonRole, MappedPropertiesHelper<DescriptiveRoleProperty>>() {
							@Override
							public MappedPropertiesHelper<DescriptiveRoleProperty> apply(
									YukonRole from) {
	...
	public String view(YukonUserContext context, ModelMap map,
			FlashScope flashScope, int roleGroupId, int roleId)
			throws Exception {
		LiteYukonGroup liteYukonGroup = yukonGroupDao
				.getLiteYukonGroup(roleGroupId);
		YukonRole role = YukonRole.getForId(roleId);

		GroupRolePropertyValueCollection propertyValues = rolePropertyEditorDao
				.getForGroupAndRole(liteYukonGroup, role, true);
	...
	private void setupModelMap(YukonUserContext context, ModelMap map,
			LiteYukonGroup liteYukonGroup, YukonRole role) {
		final MessageSourceAccessor messageSourceAccessor = messageSourceResolver
				.getMessageSourceAccessor(context);
		MappedPropertiesHelper<DescriptiveRoleProperty> mappedPropertiesHelper = helperLookup
				.get(role);
		...
		Comparator<MappedPropertiesHelper.MappableProperty<DescriptiveRoleProperty, ?>> comparator = new Comparator<MappedPropertiesHelper.MappableProperty<DescriptiveRoleProperty, ?>>() {
			...
			@Override
			public int compare(
					MappedPropertiesHelper.MappableProperty<DescriptiveRoleProperty, ?> o1,
					MappedPropertiesHelper.MappableProperty<DescriptiveRoleProperty, ?> o2) {
				String o1Text = messageSourceAccessor.getMessage(o1.getExtra()
						.getKey());
				String o2Text = messageSourceAccessor.getMessage(o2.getExtra()
						.getKey());
				return o1Text.compareToIgnoreCase(o2Text);
			}
		};

		Collections.sort(mappedPropertiesHelper.getMappableProperties(),
				comparator);

		map.addAttribute("mappedPropertiesHelper", mappedPropertiesHelper);
		map.addAttribute("roleGroupName", liteYukonGroup.getGroupName());
		map.addAttribute("roleName",
				messageSourceAccessor.getMessage(role.getFormatKey()));
		map.addAttribute("roleId", role.getRoleId());
		map.addAttribute("roleGroupId", liteYukonGroup.getGroupID());
		map.addAttribute("showDelete", role.getCategory().isSystem() ? false
				: true);
	}

	private MappedPropertiesHelper<DescriptiveRoleProperty> getHelper(
			YukonRole yukonRole) {
		Map<YukonRoleProperty, DescriptiveRoleProperty> descriptiveRoleProperties = rolePropertyEditorDao
				.getDescriptiveRoleProperties(yukonRole);

		MappedPropertiesHelper<DescriptiveRoleProperty> mappedPropertiesHelper = new MappedPropertiesHelper<DescriptiveRoleProperty>(
				"values");
		for (DescriptiveRoleProperty descriptiveRoleProperty : descriptiveRoleProperties
				.values()) {
			YukonRoleProperty yukonRoleProperty = descriptiveRoleProperty
					.getYukonRoleProperty();
			mappedPropertiesHelper.add(yukonRoleProperty.name(),
					descriptiveRoleProperty, yukonRoleProperty.getType());
		}
	...
	public String save(HttpServletRequest request,
			@ModelAttribute("command") GroupRolePropertyEditorBean command,
			BindingResult result, YukonUserContext context, ModelMap map,
			FlashScope flashScope, int roleGroupId, int roleId)
			throws Exception {
		LiteYukonGroup liteYukonGroup = yukonGroupDao
				.getLiteYukonGroup(roleGroupId);
		...
		flashScope.setConfirm(new YukonMessageSourceResolvable(
				"yukon.web.modules.adminSetup.roles.loginGroupRoleUpdated"));
		setupModelMap(context, map, liteYukonGroup, role);
	...
	public String delete(HttpServletRequest request, ModelMap map,
			FlashScope flash, int roleGroupId, int roleId) {
		...
		flash.setConfirm(new YukonMessageSourceResolvable(
				"yukon.web.modules.adminSetup.roleGroupEditor.updateSuccessful"));
		return "redirect:/adminSetup/roleGroup/view";
	}

	@RequestMapping(value = "update", method = RequestMethod.POST, params = "cancel")
	public String cancel(ModelMap map, FlashScope flash, int roleGroupId,
			int roleId) {
		map.addAttribute("roleGroupId", roleGroupId);
		return "redirect:/adminSetup/roleGroup/view";
	}
	...
	public void initialize(WebDataBinder webDataBinder, int roleId) {
		...
		MappedPropertiesHelper<DescriptiveRoleProperty> helper = helperLookup
				.get(YukonRole.getForId(roleId));
		helper.register(webDataBinder);
	}

	@Autowired
	public void setRolePropertyEditorDao(
			RolePropertyEditorDao rolePropertyEditorDao) {
