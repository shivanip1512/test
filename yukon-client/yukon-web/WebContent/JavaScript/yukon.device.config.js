...
yukon.deviceConfig = (function() {
    ...
    var _determineDisplayItemAddButtonVisibility = function() {
        var totalElems = $('[data-display-item]'), visibleElems = totalElems
                .filter(":visible");
    ...
    _showSandwichedMidnightEntries = function(num) {
        var scheduleVisibles = $("td").filter(
                function() {
                    return this.id.match('^schedule' + num + '_time\\d+$')
                            && $(this.parentElement).is(':visible');
                });
    ...
    _hideSlotDisabledEntries = function() {
        var displayItems = $('[data-display-item]'), filter = function() {
    ...
    _handleVisibleElemsAndButtons = function() {
        var totalElems = $('[data-display-item]'), hiddenElems = totalElems
                .filter(":hidden"), i;
    ...
    _determineScheduleAddButtonVisibility = function(num) {
        ...
        }).length, scheduleVisibles = $("tr").filter(
                function() {
                    return $(this).attr("data-schedule-name") === 'schedule'
                            + num
                            && $(this).is(':visible');
                }), btn;
        ...
        if (scheduleVisibles.length < total) {
            ...
            btn.show();
            $(scheduleVisibles[scheduleVisibles.length - 1].lastElementChild)
                    .append(btn);
        } else {
    ...
    _registerScheduleButtons = function() {
        $(".js-addScheduleBtn")
                .click(
                        function() {
                            var button = $(this), num = button
                                    .attr('data-add-schedule'), hiddenElems = $(
                                    "tr")
                                    .filter(
                                            function() {
                                                return $(this).attr(
    ...
    _hidingMap = [ {
        ...
        valueHideMap : [
                {
                    value : 'ON_DEMAND',
                    hide : [ 'disconnectDemandInterval',
                            'disconnectDemandThreshold',
                            'disconnectLoadLimitConnectDelay',
                            'maxDisconnects', 'disconnectMinutes',
                            'connectMinutes' ],
                    show : [ 'reconnectParam' ]
                },
                {
                    ...
                    show : [ 'reconnectParam', 'disconnectDemandInterval',
                            'disconnectDemandThreshold',
                            'disconnectLoadLimitConnectDelay', 'maxDisconnects' ]
                },
                {
                    value : 'CYCLING',
                    hide : [ 'reconnectParam', 'disconnectDemandInterval',
                            'disconnectDemandThreshold',
                            'disconnectLoadLimitConnectDelay', 'maxDisconnects' ],
    ...
    _hideShowElement = function(hidingMapEntry, timeout) {
        var value = $('[data-field="' + hidingMapEntry.keyHolder + '"]').find(
                ':input').val(), currentEntry, ii, jj, elem;
    ...
    _hideThingsInMap = function() {
        var currentEntry, ii, input;

        for (ii = 0; ii < _hidingMap.length; ii += 1) {
            currentEntry = _hidingMap[ii];
            input = $('[data-field="' + currentEntry.keyHolder + '"]').find(
                    ':input');
    ...
    _openCategoryPopup = function(btn, url) {
        $('#category-popup').load(url, function() {
            var title = $('#popup-title').val(), buttons = yukon.ui.buttons({
                okText : yg.text.save,
                event : 'yukon.deviceConfigs.category.save'
            });
    ...
    mod = {
        ...
        /**
         * Initialize the module. Depends on DOM elements so call after page
         * load.
         */
        init : function() {

            var typesLink = $('[data-show-device-types]'), num, pipe, deviceType;
            ...
            $('.js-edit-category')
                    .click(
                            function(ev) {
                                ...
                                var btn = $(this), categoryId = $(
                                        '#categoryId_'
                                                + btn.data('categoryType'))
                                        .val(), configId = btn.data('configId'), url = yukon
                                        .url('/deviceConfiguration/category/editInPlace?categoryId='
                                                + categoryId
                                                + '&configId='
                                                + configId);
            ...
            /**
             * Save button click on cateogry edit or create popup. Post form and
             * handle results.
             */
            $('#category-popup').on('yukon.deviceConfigs.category.save',
                    function(ev) {
            ...
            $(".js-create-category")
                    .click(
                            function(ev) {
                                var btn = $(this), type = btn
                                        .data('categoryType'), configId = btn
                                        .data('configId'), url = yukon
                                        .url('/deviceConfiguration/category/createInPlace?categoryType='
                                                + type
                                                + '&configId='
                                                + configId);
            ...
            $(".js-categories").click(
                    function() {
                        ...
                        $(".pipe[data-device-type-" + deviceType + "]").css(
                                'visibility', 'visible');
                        $(".pipe[data-device-type=" + deviceType + "]").css(
                                'visibility', 'visible');
                    });
            ...
            $("#category-popup")
                    .on(
                            ...
                            function(event) {
                                if ($(event.target)
                                        .closest('#showNextFieldBtn').length > 0) {
                                    // Show the next hidden display item.
                                    var hiddenElems = $('[data-display-item]')
                                            .filter(":hidden"), visibleElems = $(
                                            '[data-display-item]').filter(
                                            ":visible");
