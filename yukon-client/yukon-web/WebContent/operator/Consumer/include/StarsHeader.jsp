...
<%@ page import="com.cannontech.stars.database.data.lite.LiteAccountInfo"%>
...
<%
     ...
     final RolePropertyDao rolePropertyDao =
     ...
     final SimpleDateFormat dateTimeExtFormat =
         new java.text.SimpleDateFormat("MM/dd/yy - HH:mm:ss");
     final SimpleDateFormat histDateFormat =
         new java.text.SimpleDateFormat("MM/dd/yy HH:mm z");
     ...
     final YukonUserContext userContext =
         YukonUserContextUtils.getYukonUserContext(pageContext);
         ...
         if (liteEC != null) {
             ...
             if (ecSettings != null) {
                 ...
                 if (ecSettings.getStarsCustomerSelectionLists() != null) {
                     ...
                     for (int i = 0; i < ecSettings.getStarsCustomerSelectionLists()
                         .getStarsCustSelectionListCount(); i++) {
                         StarsCustSelectionList value =
                             ecSettings.getStarsCustomerSelectionLists()
                                 .getStarsCustSelectionList(i);
                         String key = value.getListName();
                         selectionListTable.put(key, value);
                     }
                     session.setAttribute(ServletUtils.ATT_CUSTOMER_SELECTION_LISTS,
                                          selectionListTable);
                 }
             }
             ...
             if (accountInfo != null) { // ensure there is a new instance of StarsCustAccountInformation for each request
                 ...
                 if (accountInfo != null) {
                     ...
                     session.setAttribute(ServletUtils.TRANSIENT_ATT_CUSTOMER_ACCOUNT_INFO,
                                          accountInfo);
                     ...
                     try {
                         Customer cust =
                             (Customer) com.cannontech.database.data.lite.LiteFactory
                                 .createDBPersistent(
                                 new LiteCustomer(account.getCustomerID()));

                         Transaction<Customer> t =
                             Transaction.createTransaction(Transaction.RETRIEVE, cust);
                         cust = t.execute();
                     ...
                     //-------------------------------------------------------------------------------
                     final LMHardwareControlInformationService lmHardwareControlInformationService =
                         YukonSpringHook.getBean("lmHardwareControlInformationService",
                                                 LMHardwareControlInformationService.class);

                     LiteAccountInfo currentLiteAcctInfo =
                         starsCustAccountInformationDao.getById(account.getAccountID(),
                                                                liteEC.getEnergyCompanyId());
                     List<LiteStarsAppliance> currentAppList =
                         currentLiteAcctInfo.getAppliances();
                     ...
                     for (int x = 0; x < currentAppList.size(); x++) {
                         ...
                         List<Integer> inventoryNotOptedOut =
                             lmHardwareControlInformationService
                                 .getInventoryNotOptedOutForThisProgram(currentApp
                                     .getProgramID(), account.getAccountID());
                         for (Integer invenId : inventoryNotOptedOut) {
                             if (currentApp.getInventoryID() == invenId) {
                                 List<Integer> progInventory =
                                     partialOptOutMap.get(currentApp.getProgramID());
                                 if (progInventory == null) {
                                     partialOptOutMap.put(currentApp.getProgramID(),
             ...
             graphBean =
                 (com.cannontech.graph.GraphBean) session
                     .getAttribute(ServletUtil.ATT_GRAPH_BEAN);
             if (graphBean == null) {
                 session.setAttribute(ServletUtil.ATT_GRAPH_BEAN,
                                      new com.cannontech.graph.GraphBean());
                 graphBean =
                     (com.cannontech.graph.GraphBean) session
                         .getAttribute(ServletUtil.ATT_GRAPH_BEAN);
             }
