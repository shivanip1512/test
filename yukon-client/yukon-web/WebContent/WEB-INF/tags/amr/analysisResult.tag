...
<cti:uniqueIdentifier var="uniqueId" prefix="result-bar_${data.id.paoId}"/>

<c:set var="colorWidth" value="1"/>

<div id="${uniqueId}" style="width: ${width}px;" class="result-bar">
    
    <c:forEach var="readData" items="${data.timeline}">
        <cti:formatInterval var="title" value="${readData.range}" type="DATEHM"/>
        <div class="result ${readData.color}" style="width:${readData.width}px;" title="${title}"></div>
    </c:forEach>
