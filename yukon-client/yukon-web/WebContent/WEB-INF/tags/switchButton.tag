...
<%@ attribute name="onNameKey" required="true" description="i18n key used for the green button." %> 
<%@ attribute name="offNameKey" required="true" description="i18n key used for the green button." %> 
    ...
    <c:choose>
        <c:when test="${not empty name}">
            <input type="checkbox" name="${name}" id="${id}" class="switch-btn-checkbox" 
                <c:if test="${checked}">checked</c:if> data-toggle="${pageScope.toggleGroup}">
        </c:when>
        <c:otherwise>
