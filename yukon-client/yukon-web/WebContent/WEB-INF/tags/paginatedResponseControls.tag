...
<c:set var="numberOfPages" value="${response.totalItems / response.itemsPerPage}"/>

<c:if test="${numberOfPages >= 1}">
    <div class="compact-results-paging clearfix ${classes}">
        <div class="paging-area" data-current-page="${response.pageNumber + 1}" data-page-size="${response.itemsPerPage}">
            ...
            <span class="fl page-num-text">
                <c:set var="startIndex" value="${response.pageNumber * response.itemsPerPage + 1}"/>
            ...
            <span class="fl next-page">
                <c:choose>
                    <c:when test="${response.pageNumber <= numberOfPages - 1}">
                        <button class="button naked">
