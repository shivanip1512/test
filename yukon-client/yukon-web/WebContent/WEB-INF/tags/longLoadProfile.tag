...
<c:set var="lastLlpId" value="${lastLlpId + 1}" scope="request"/>
<c:set var="thisId" value="llp_${lastLlpId}"/>
...
<input id="${thisId}_deviceId" type="hidden" value="${deviceId}">
  ...
  <tags:hideReveal section="${thisId}_pendingList" styleClass="smallText">Pending requests in progress</tags:hideReveal>
