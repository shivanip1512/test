<%@ tag dynamic-attributes="attrs" description="Use this tag to create a dropdown menu option inside a dropdown.tag element." %>
    ...
    <a <c:if test="${not empty pageScope.href}">href="${href}"</c:if> <c:if test="${newTab}">target="_blank"</c:if> class="clearfix">
        <c:choose>
            <c:when test="${not empty pageScope.displayable}">
                ...
                <span class="${labelClasses} ${disabled}"><cti:formatObject value="${displayable}"/></span>
            </c:when>
