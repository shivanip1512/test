...
<c:if test="${not empty address}">
    <c:choose>
        <c:when test="${not empty pageScope.inLine && pageScope.inLine}">
            <tags:notNullDataLine value="${address.locationAddress1}" inLine="${pageScope.inLine}" ignore="${pageScope.ignore}"/>
            <tags:notNullDataLine value="&nbsp;${address.locationAddress2}" inLine="${pageScope.inLine}" ignore="${pageScope.ignore}"/>
        </c:when>
        <c:otherwise>
    ...
    <c:if test="${not empty address.cityName}">
        <tags:notNullDataLine value="${address.cityName}," inLine="true" ignore="${pageScope.ignore},"/>  
    </c:if>
