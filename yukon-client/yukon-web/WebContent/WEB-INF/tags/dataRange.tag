...
<c:choose>
    ...
    <c:when test="${value.dataRangeType == 'DAYS_PREVIOUS'}">
        <i:inline key="yukon.web.modules.tools.bulk.archivedValueExporter.dataRangeType.DAYS_PREVIOUS"/> (${value.daysPrevious})
    ...
    <c:when test="${value.dataRangeType == 'SINCE_LAST_CHANGE_ID'}">
        <i:inline key="yukon.web.modules.tools.bulk.archivedValueExporter.dataRangeType.SINCE_LAST_CHANGE_ID"/>
    </c:when>
    <c:otherwise>
