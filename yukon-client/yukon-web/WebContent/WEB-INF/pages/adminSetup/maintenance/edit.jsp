	...
	<cti:dataGrid cols="2" tableClasses="twoColumnLayout">
		<form action="update" method="post">
			...
			<input type="hidden" name="jobId" value="${job.id}"> <input
				type="hidden" name="cronUniqueId" value="${cronUniqueId}">
			<tags:formElementContainer nameKey="${job.beanName}">
				<tags:nameValueContainer2>
					<tags:nameValue2 nameKey=".status">
						<c:if test="${job.disabled}">
							<span class="fwb errorMessage"><i:inline
									key=".status.disabled" />
							</span>
						</c:if>
						<c:if test="${!job.disabled}">
							<span class="fwb successMessage"><i:inline
									key=".status.enabled" />
							</span>
						</c:if>
					</tags:nameValue2>
					...
					<tags:nameValue2 nameKey=".nextRun">
						<amr:scheduledGroupRequestExecutionJobNextRunDate
							jobId="${job.id}" />
					</tags:nameValue2>
				</tags:nameValueContainer2>
