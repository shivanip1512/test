...
<cti:standardPage title="Device Definitions" module="blank">
	...
	<c:url var="url" value="/spring/static/deviceDefinition.xml"/>
	...
	<table class="miniResultsTable" style="width:75%">
		...
		<tr>
			...
			<%-- feature select --%>
			<td>
				<select onchange="doDefinitionFilter(this, 'feature');">
					<option value="" >Any Feature</option>
					<c:forEach var="feature" items="${allFeatures}" >
						<c:choose>
							<c:when test="${featureParam == feature.name}">
								<option value="${feature.name}" selected>${feature.description}</option>
							</c:when>
							<c:otherwise>
								<option value="${feature.name}">${feature.description}</option>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</select>
		...
		<tags:sectionContainer title="${sectionName}">
			...
			<c:forEach var="deviceInfo" items="${info.value}">
				...
				<tags:boxContainer title="${deviceInfo.definition.displayName}" id="${deviceInfo.definition.type}" hideEnabled="true">
						...
						<table class="miniResultsTable" style="width:900px">
							...
							<c:forEach var="point" items="${deviceInfo.points}">
								<tr class="<tags:alternateRow odd="" even="altRow"/>">
									...
									<td>${point.pointTemplate.devicePointIdentifier.offset}</td>
					...
					<%-- FEATURES --%>
					<c:if test="${fn:length(deviceInfo.features) > 0}">
						<br>
						<table class="miniResultsTable" style="width:900px">
							<tr>
								<th>Features</th>
							</tr>
							<c:forEach var="feature" items="${deviceInfo.features}">
								<tr class="<tags:alternateRow odd="" even="altRow"/>">
