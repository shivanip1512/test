            ...
            <table class="compactResultsTable">
                ...
                <tbody>
                    <c:forEach var="jobWrapper" items="${jobWrappers}">
                        ...
                        <tr id="tr_${jobWrapper.job.id}" data-schedule-id="${jobWrapper.job.id}" data-schedule-name="${fn:escapeXml(jobWrapper.name)}">
                            <%-- name --%>    
                            <td>
                                <a href="${viewScheduleDetailsUrl}" 
                                    title="<cti:msg2 key=".edit.hoverText" arguments="${jobWrapper.name}"/>">${fn:escapeXml(jobWrapper.name)}</a>
                            </td>
                            ...
                            <td id="status_${jobWrapper.job.id}">
                                <div id="jobNotRunningSpan_${jobWrapper.job.id}" <c:if test="${jobWrapper.jobStatus eq 'RUNNING'}">style="display:none;"</c:if>>
                                    <cti:dataUpdaterValue type="JOB" identifier="${jobWrapper.job.id}/STATE_TEXT"/>
                                </div>
                                <div id="jobRunningSpan_${jobWrapper.job.id}" class="wsnw" style="height: 16px;<c:if test="${not (jobWrapper.jobStatus eq 'RUNNING')}">display:none;</c:if>">
                        ...
                        <cti:dataUpdaterCallback function="buildTooltipText('status_${jobWrapper.job.id}')" 
                            initialize="true"
