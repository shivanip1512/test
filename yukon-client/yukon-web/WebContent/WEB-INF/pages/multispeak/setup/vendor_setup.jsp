...
<cti:standardPage module="adminSetup" page="vendor.${mode}">
	...
	<cti:linkTabbedContainer mode="section" id="page_header_tab_container">
		<cti:linkTab tabId="deviceTab"
			selectorKey="yukon.web.modules.adminSetup.multispeak.home.tab.title">
			<c:url value="/multispeak/setup/home" />
		</cti:linkTab>

		<cti:linkTab tabId="vendorTab"
			selectorKey="yukon.web.modules.adminSetup.vendor.tab.title"
			initiallySelected="${true}">
			<c:url value="/multispeak/setup/vendorHome" />
		</cti:linkTab>

		<cti:checkGlobalSetting setting="MSP_LM_MAPPING_SETUP">
			<cti:linkTab tabId="loadMgtTab"
				selectorKey="yukon.web.modules.adminSetup.lmMappings.tab.title">
				<c:url value="/multispeak/setup/lmMappings/home" />
			</cti:linkTab>
		</cti:checkGlobalSetting>

		<cti:linkTab tabId="synchronizationTab"
			selectorKey="yukon.web.modules.adminSetup.multispeakSyncHome.tab.title">
			<c:url value="/multispeak/setup/multispeakSync/home" />
		</cti:linkTab>
	</cti:linkTabbedContainer>
	<c:set var="interfaceListLength"
		value="${fn:length(possibleInterfaces)}" />
	<cti:url var="saveUrl" value="/multispeak/setup/save" />
	<form:form modelAttribute="multispeak" id="mspForm" name="mspForm"
		...
		<c:set var="tableClass"
			value="${viewMode ? '' : 'with-form-controls'}" />
		...
		<input type="hidden" class="js-vendor-id"
			value="${mspVendor.vendorID}" />
						...
						<tags:nameValueContainer2 tableClass="natural-width ${tableClass}">
							...
							<tags:nameValue2 nameKey=".companyName" requiredField="true">
								<c:choose>
									...
									<c:otherwise>
										<select title="<cti:msg2 key=".selectVendor.title"/>"
											name="mspVendorId"
											onChange="yukon.admin.multispeak.vendorChanged();"
											id="mspVendorId">
							...
							<tags:nameValue2 nameKey=".reqMsgTimeout" requiredField="true">
								<tags:input path="mspVendor.requestMessageTimeout"
									maxlength="32" />
							</tags:nameValue2>

							<tags:nameValue2 nameKey=".initRequestTimeout"
								requiredField="true">
								<tags:input path="mspVendor.maxInitiateRequestObjects"
									maxlength="32" />
							</tags:nameValue2>
					...
					<div class="column two nogutter">
						...
						<tags:nameValueContainer2 tableClass="natural-width ${tableClass}">
							...
							<tags:nameValue2 nameKey=".password">
								<tags:password path="mspVendor.password" showPassword="true"
									includeShowHideButton="true"></tags:password>
							</tags:nameValue2>
						</tags:nameValueContainer2>
						...
						<tags:nameValueContainer2>
							...
							<tags:nameValue2 nameKey=".password">
								<tags:password path="mspVendor.outPassword" showPassword="true"
									includeShowHideButton="true"></tags:password>
							</tags:nameValue2>

							<tags:nameValue2 nameKey=".validateCertificate">
								<cti:displayForPageEditModes modes="EDIT,CREATE">
									<tags:switch path="mspVendor.validateCertificate"
										classes="toggle-sm" />
									<input type="hidden" class="js-create-or-edit-mode">
								</cti:displayForPageEditModes>
								<cti:displayForPageEditModes modes="VIEW">
									<tags:switch path="mspVendor.validateCertificate"
										classes="toggle-sm" disabled="true" />
										...
										<c:if
											...
											<tr data-id="${indexValue}">
												<td class="wsnw"><tags:checkbox
													...
													<span class="checkBoxLabel"> <cti:msg2
															key=".vendor.endpoint.${currentInterface.mspInterface}" />
												</span> <form:hidden
														path="mspInterfaceList[${indexValue}].vendorID" /> <form:hidden
												...
												<td><c:set var="versionOptions"
														value="${mspVersionList}" /> <c:if
														test="${currentInterface.mspInterface == 'NOT_Server_DR'}">
												...
												<td style="width: 120px;">
													<div class="button-group fr wsnw oh">
														...
														<cti:button icon="icon-application-view-columns"
															...
															onclick="yukon.admin.multispeak.executeRequest('${currentInterface.mspInterface}',this.name,'${currentInterface.version}');" />
														<tags:hidden
															path="mspInterfaceList[${indexValue}].useVendorAuth"
															id="js-msp-uservendorauth-${indexValue}" />
														<tags:hidden
															path="mspInterfaceList[${indexValue}].inUserName"
															id="js-msp-inusername-${indexValue}" />
														<tags:hidden
															path="mspInterfaceList[${indexValue}].inPassword"
															id="js-msp-inpassword-${indexValue}" />
														<tags:hidden
															path="mspInterfaceList[${indexValue}].outUserName"
															id="js-msp-outusername-${indexValue}" />
														<tags:hidden
															path="mspInterfaceList[${indexValue}].outPassword"
															id="js-msp-outpassword-${indexValue}" />
														<tags:hidden
															path="mspInterfaceList[${indexValue}].validateCertificate"
															id="js-msp-validatecertificate-${indexValue}" />
														<input type="hidden" id="js-page-mode" value="${mode}" />
														<cti:msg2 var="interfaceAuthTitle"
															key=".interfaceAuthPopupTitle" />
														<cti:button icon="icon-lock" renderMode="buttonImage"
															classes="js-endpoint-auth-btn" name="interfaceAuth"
							...
							<table>
								...
								<tbody>
									<tr>
										...
										<td rowspan='${interfaceListLength+3}'><textarea
												cols="40" rows="${interfaceListLength*3+1}" name="Results"
												id="results" readonly wrap="VIRTUAL"
		...
		<div class="page-action-area">
			<cti:displayForPageEditModes modes="VIEW">
				...
				<cti:button nameKey="create" busy="true" href="${createUrl}"
	...
	<cti:url var="url"
		value="/multispeak/setup/vendorHome/${mspVendor.vendorID}" />
