...
public class AdvancedOptionsPanel extends com.cannontech.common.gui.util.DataInputPanel
{
	...
	/**
	 * AdvancedOptionsPanel constructor comment.
	 * SetPanelsEnabled for format=(ExportFormatTypes.FORMATID).
	 */
	...
	/**
	 * Enable/Disable the associated interface pieces for format (format=ExportFormatTypes.FORMATID).
	 * @param format int
	 */
	...
	public void setValue(Object object)
	{
		...
		if( object instanceof ExportPropertiesBase)
		{
			...
			else if( ((ExportPropertiesBase)object).getFormatID() == ExportFormatTypes.CSVBILLING_FORMAT)
			{
				...
				getHeadingsCheckBox().setSelected(props.isShowColumnHeadings());
				getDelimiterTextBox().setText(String.valueOf(props.getDelimiter()));
				getFileDirectoryTextField().setText(props.getEnergyFileName().toString());
			}
		}
	}
	...
	public Object getValue(Object object)
	{
		...
		else if(getExportProperties().getFormatID() == ExportFormatTypes.CSVBILLING_FORMAT )
		{
			...
			date = getStopDateComboBox().getSelectedDate();
			if( date instanceof java.util.Date )
			{
				calTemp.setTime((java.util.Date) date);
			...
			exportProps.setShowColumnHeadings(getHeadingsCheckBox().isSelected());
	
			//Even if the delimiter textBox is longer than 1 character, we only take the first character.
			String delTemp = getDelimiterTextBox().getText().toString();
			if( delTemp.length() > 0)
				exportProps.setDelimiter(new Character(delTemp.charAt(0)));
					
			exportProps.setEnergyFileName(getFileDirectoryTextField().getText().toString().trim());
		}
