...
public class CommandDeviceBean implements DBChangeListener {
    ...
    public ArrayList<LiteYukonPAObject> getDeviceList() {
        ...
        if (deviceList == null || isChanged()) {
            if (isChanged()) { // clear out all maps, we're not sure what
            ...
            synchronized (cache) {
                ...
                for (LiteYukonPAObject lPao : allPaos) {
                    ...
                    if (getSortBy() == DeviceClasses.CARRIER) {
                        ...
                        if (!(getSortBy() == lPao.getPaoType().getPaoClass().getPaoClassId()))
                            isValid = false;
                    }

                    if (isValid) { // Filter the sortBy objects even more.
                        if (getFilterBy() == ROUTE_FILTER) {
                            if (!(String.valueOf(lPao.getRouteID()).equals(getFilterValue())))
                                isValid = false;
                        } else if (getFilterBy() == COMM_CHANNEL_FILTER) {
                            if (!(String.valueOf(lPao.getPortID()).equals(getFilterValue())))
                                isValid = false;
                        } else if (getFilterBy() == CBC_TYPE_FILTER) {
                            if (!(String.valueOf(lPao.getPaoType().getDbString()).equals(getFilterValue())))
                                isValid = false;
                        }
                    }
    ...
    public String getDeviceTableHTML() {
        ...
        html += "<table class='full-width' border='0' cellspacing='0' cellpadding='3'>" + LINE_SEPARATOR;
        html += "  <tr>" + LINE_SEPARATOR;
        html += "    <td>" + LINE_SEPARATOR;
        html += "      <table border='0' cellspacing='0' cellpadding='3' class='TableCell full-width'>";
        html += "        <tr>";
        ...
        html += "      <table class=\"results-table full-width\">" + LINE_SEPARATOR;
        html += "        <tr>" + LINE_SEPARATOR;
        ...
        for (int i = startIndex; i < endIndex; i++) {
            LiteYukonPAObject lPao = getDeviceList().get(i);
            html += "        <tr>" + LINE_SEPARATOR;
            for (int j = 0; j < columns.length; j++) {
                html += "          <td width=\'" + getColumnWidth(columns[j]) + "%\'>";
                if (j == 0)
                    html += "<a method='post' href='CommandDevice.jsp?deviceID=" + lPao.getYukonID() + "'>" + getDeviceAttributeValue(columns[j],
                                                                                                                                      lPao) + "</a>";
                else
        ...
        html += "  <tr>" + LINE_SEPARATOR;
        html += "    <td>" + LINE_SEPARATOR;
        html += "      <table border='0' cellspacing='0' cellpadding='3' class='TableCell full-width'>" + LINE_SEPARATOR;
        html += "        <tr>" + LINE_SEPARATOR;
        html += "          <td>" + (startIndex + 1) + "-" + endIndex + " of " + listSize + " | ";
    ...
    private void organizeDeviceList() {
        ...
        if (getOrderDir() == ORDER_DIR_DESCENDING)
            Collections.reverse(getDeviceList());
    }
    ...
    public List<LiteYukonPAObject> getValidRoutes() {
        if (validRoutes == null) {
            ...
            for (LiteYukonPAObject lPao : routes) {
    ...
    public List<LiteYukonPAObject> getValidCommChannels() {
        if (validCommChannels == null) {
            ...
            for (LiteYukonPAObject lPao : ports) {
    ...
    public ArrayList<String> getValidCBCTypes() {
        if (validCBCTypes == null) {
            try {
                ...
                for (String cbcType : valids) {
                    validCBCTypes.add(cbcType);
                }
    ...
    public final static String[] retrieveCBCTypes() throws java.sql.SQLException {
        ...
        SqlStatement stmt = new SqlStatement("SELECT DISTINCT TYPE FROM " + YukonPAObject.TABLE_NAME + 
    ...
    private Object getDeviceAttributeValue(String valueString, LiteYukonPAObject lPao) {
        if (valueString.equalsIgnoreCase(DEVICE_NAME_STRING) || valueString.equalsIgnoreCase(LOAD_GROUP_STRING)) {
            ...
            YCLiteCBC lcbc = getCBCIDToLiteCBCMap().get(new Integer(lPao.getYukonID()));
            if (lcbc != null) {
            ...
            YCLiteLoadGroup llg = getLoadGroupIDToLiteLoadGroupsMap().get(new Integer(lPao.getYukonID()));
            if (llg != null) {
    ...
    private int getColumnWidth(String columnString) {
        if (columnString.equalsIgnoreCase(DEVICE_NAME_STRING) || columnString.equalsIgnoreCase(LOAD_GROUP_STRING)) {
            return 25;
        }
        return 15;
    }
    ...
    public HashMap<Integer, YCLiteLoadGroup> getLoadGroupIDToLiteLoadGroupsMap() {
        if (loadGroupIDToLiteLoadGroupsMap == null) {
            ...
            final PaoAuthorizationService paoAuthorizationService = YukonSpringHook.getBean("paoAuthorizationService",
                                                                                            PaoAuthorizationService.class);
            ...
            List<YCLiteLoadGroup> filtered = paoAuthorizationService.filterAuthorized(unloadedLiteYukonUser,
                                                                                      liteLoadGroups,
        ...
        if (cbcIDToLiteCBCMap == null) {
            ...
            try {
                ...
                if (conn == null) {
                    ...
                    // Load all cbcs
                    StringBuffer sql = new StringBuffer(" SELECT DISTINCT DEVICEID, SERIALNUMBER, ROUTEID " + " FROM " + DeviceCBC.TABLE_NAME);
