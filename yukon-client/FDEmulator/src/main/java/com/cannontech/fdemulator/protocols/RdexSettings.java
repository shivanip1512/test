 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class RdexSettings extends JFrame implements ActionListener
{
	...
	private JTextField registerName = new JTextField();
	private JLabel regLabel = new JLabel("Register name:");
	...
	private JLabel defaultReg = new JLabel("SIM");
	...
	public RdexSettings(String title, RdexProtocol protocol)
	{
		super("RDEX Settings");
		proto = protocol;
		try
	...
	public void loadLastSettings()
	{
		...
		while (exit != 1)
		{
			try
			{
				...
				if (var.equalsIgnoreCase("RDEX"))
				{
					String regname = settingsFile.readLine().trim();
	...
	public void saveSettings()
	{
		...
		while (exit != 1)
		{
			try
			{
				...
				if (var.equalsIgnoreCase("RDEX"))
				{
					settingsFile.writeBytes(regname);
					for (int i = 0; i < 100 - regname.length(); i++)
					{
	...
	public void init() throws Exception
	{
		...
		regLabel.setBounds(new Rectangle(10, 5, 90, 20));
		registerName.setBounds(new Rectangle(110, 5, 90, 20));
		defaultReg.setBounds(new Rectangle(290, 5, 90, 20));
		...
		this.getContentPane().add(regLabel, null);
		this.getContentPane().add(registerName, null);
		...
		this.getContentPane().add(defaultReg, null);
