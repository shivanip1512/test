/*
 * Created on Jul 28, 2003 To change the template for this generated file go to
...
public class ImageExporter {
    ...
    public void exportAllImages(String dir, List<LiteYukonImage> images) {
        for (LiteYukonImage image : images) {
            if (image != null) {
                writeImage(dir + File.separatorChar + image.getImageName(),
                           image.getImageValue());
            } else {
                writeImage(dir + File.separatorChar + Util.DEFAULT_IMAGE_NAME,
                           Util.DEFAULT_IMAGE_BYTES);
            }
        }
    }
    ...
    public void exportImages(String dir, Drawing d) {
        ...
        for (int i = 0; i < c.length; i++) {
            ...
            if (c[i] instanceof YukonImageElement) {
                ...
                if (img != null) {
                    writeImage(dir + File.separatorChar + img.getImageName(),
                               img.getImageValue());
                } else {
                    writeImage(dir + File.separatorChar + Util.DEFAULT_IMAGE_NAME,
                               Util.DEFAULT_IMAGE_BYTES);
                }
            } else if (c[i] instanceof DynamicGraphElement) {
                DynamicGraphElement dge = (DynamicGraphElement) c[i];
                ByteArrayOutputStream bout = new ByteArrayOutputStream();
                try {
                    dge.getCTIGraph().encodePng(bout);
                    writeImage(dir + File.separatorChar + Util.genExportedGraphName(dge),
                               bout.toByteArray());
    ...
    private void writeImage(String fileName, byte[] buf) {
