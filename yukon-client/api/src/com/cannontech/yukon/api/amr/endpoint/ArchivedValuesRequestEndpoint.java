...
public class ArchivedValuesRequestEndpoint {
    ...
    public Element invoke(Element archivedValuesRequest, LiteYukonUser user) throws Exception {
        ...
        try {
            ...
            if (responseFields.contains(ResponseDescriptor.LAST_VALUE_ID)) {
                ...
                lastValueIdElement.setAttribute("value", Long.toString(maxValueId));
            }
    ...
    private Set<ResponseDescriptor> findResponseTypes(SimpleXPathTemplate template)
        ...
        for (Node node : responseTypeNodes) {
            ...
            if (rt == null) {
                throw new XmlValidationException("invalid response type " + node.getLocalName());
            }
        ...
        public ListMultimap<PaoIdentifier, PointValueQualityHolder> getPointValueMap(Set<PaoIdentifier> paos,
            ...
            if (selector.getNumberOfRows() != null) {
                return rawPointHistoryDao.getLimitedAttributeData(paos, attribute, dateRange, changeIdRange,
                    selector.getNumberOfRows(), false, selector.getOrder(), OrderBy.TIMESTAMP, null);
            } else {
                return rawPointHistoryDao.getAttributeData(paos, attribute, dateRange, changeIdRange, false,
                    selector.getOrder(), null, null);
            }
        }
