...
public class CentMaineDBConverter extends MessageFrameAdaptor {
	...
	public void run() {
		...
		if (s)
			s = processTransmitterFile();

		if (s)
			s = processRepeaterFile();

		if (s)
			s = processRouteFile();

		if (s)
			s = processRouteMacrosFile();
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			...
			// default path name
			CTILogger.info(
				"Please enter a directory path for the location of conversion files");
	...
	public boolean processPortFile() {
		...
		getIMessageFrame().addOutput(
			"Starting Port file process (" + commListFileName + ")...");
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			Integer portID =
				new Integer(
					pInt(portNum.toString()).intValue() + PORTID_OFFSET);

			//ingore this, we already have it
			if (portID.intValue() == PORTID_OFFSET
				|| commChannels.contains(portID.intValue()))
				continue;
			ts.nextToken();
			ts.nextToken();
		...
		if (success) {
			...
			getIMessageFrame().addOutput(
				" Port file processed and inserted Successfully");
		} else
	...
	public boolean processTransmitterFile() {
		...
		getIMessageFrame().addOutput(
			"Starting Transmitter file process ("
				+ transmitterFileName
				+ ")...");
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			Integer deviceID =
				new Integer(
					pInt((idNum).substring(0, 3)).intValue() + CCUID_OFFSET);
			...
			if (device.getDeviceDirectCommSettings().getPortID().intValue()
				> PORTID_OFFSET) {
				...
				StatusPoint commStatus =
					(StatusPoint) PointFactory.createPoint(
				...
				commStatus.getPoint().setStateGroupID(
					new Integer(
						com
							.cannontech
							.database
		...
		if (success) {
			CTILogger.info(
				" Transmitter file processed and inserted Successfully");
			getIMessageFrame().addOutput(
				" Transmitter file processed and inserted Successfully");
		} else
	...
	public boolean processRepeaterFile() {
		...
		getIMessageFrame().addOutput(
			"Starting Repeater file process (" + repeaterFileName + ")...");
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			Integer deviceID =
				new Integer(
					pInt((idNum).substring(3, 6)).intValue() + RPTID_OFFSET);
		...
		if (success) {
			CTILogger.info(
				" Repeater file processed and inserted Successfully");
			getIMessageFrame().addOutput(
				" Repeater file processed and inserted Successfully");
		} else
	...
	public boolean processRouteFile() {
		...
		getIMessageFrame().addOutput(
			"Starting Route file process (" + routeFileName + ")...");
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			Integer deviceID =
				new Integer(
					pInt((ccuID).substring(0, 3)).intValue() + CCUID_OFFSET);
		...
		if (success) {
			...
			getIMessageFrame().addOutput(
				" Route file processed and inserted Successfully");
		} else
	...
	public boolean processRouteMacrosFile() {
		...
		//create an object to hold all of our DBPersistant objects
		com.cannontech.database.data.multi.MultiDBPersistent multi =
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			for (int count = 0; count < MAX_DSM2_MACRO_COUNT; count++) {
				try {
					ts2 =
						new StringTokenizer(lines.get(i + 1 + count).toString(),"|");
				} catch (Exception e) {
					// no more lines
					break;
				}
		...
		if (success) {
			CTILogger.info(
				" Route Macro file was processed and inserted Successfully");
			getIMessageFrame().addOutput(
				"Route Macro file was processed and inserted Successfully");

			CTILogger.info(
				" " + addCount + " Macro Routes were added to the database");
			getIMessageFrame().addOutput(
				addCount + " Macro Routes were added to the database");
		} else
			getIMessageFrame().addOutput(
	...
	public boolean processMCTFile() {
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			if (dType.equalsIgnoreCase("MC210I")) {
				...
				dType.equalsIgnoreCase("MC240I")
					|| dType.equalsIgnoreCase("MC240P")) {
			...
			if (deviceType == DeviceTypes.STRING_DCT_501[0]) {
				...
				// set the MCT address
				device.getDeviceCarrierSettings().setAddress(
					new Integer(address));

				// set this devices route
				Object myRouteID = macroRouteIDsMap.get("@" + zone);
				if (myRouteID==null){
					myRouteID = routeIDsMap.get(zone);
				}
				...
				if (deviceType.equals(new String("MCT-240"))
					...
					device.getDeviceLoadProfile().setLoadProfileCollection(
						new String("YNNN"));
				} else {
					device.getDeviceLoadProfile().setLoadProfileCollection(
						new String("NNNN"));
				...
				AccumulatorPoint accumPoint =
					(AccumulatorPoint) PointFactory.createPoint(
				...
				if (String.valueOf(dType.charAt(5)).equalsIgnoreCase("I")) {
					accumPoint.getPointAccumulator().setMultiplier(
						new Double(0.01));
				} else
					accumPoint.getPointAccumulator().setMultiplier(
						new Double(1.0));
				...
				accumPoint.getPointAlarming().setAlarmStates(
					PointAlarming.DEFAULT_ALARM_STATES);
				accumPoint.getPointAlarming().setExcludeNotifyStates(
					PointAlarming.DEFAULT_EXCLUDE_NOTIFY);
				accumPoint.getPointAlarming().setNotifyOnAcknowledge(
					new String("N"));
				accumPoint.getPointAlarming().setNotificationGroupID(
					new Integer(PointAlarming.NONE_NOTIFICATIONID));
				...
				if (String.valueOf(dType.charAt(5)).equalsIgnoreCase("I")) {
					pulseAccumPoint.getPointAccumulator().setMultiplier(
						new Double(0.01));
				} else
					pulseAccumPoint.getPointAccumulator().setMultiplier(
						new Double(1.0));
				...
				// set default settings for point ALARMING
				pulseAccumPoint.getPointAlarming().setAlarmStates(
					PointAlarming.DEFAULT_ALARM_STATES);
				pulseAccumPoint.getPointAlarming().setExcludeNotifyStates(
					PointAlarming.DEFAULT_EXCLUDE_NOTIFY);
				pulseAccumPoint.getPointAlarming().setNotifyOnAcknowledge(
					new String("N"));
				pulseAccumPoint.getPointAlarming().setNotificationGroupID(
					new Integer(PointAlarming.NONE_NOTIFICATIONID));
				pulseAccumPoint.getPointUnit().setDecimalPlaces(new Integer(2));
				device.getDeviceLoadProfile().setLastIntervalDemandRate(
					new Integer(300));
		...
		if (success) {
			CTILogger.info(
				" MCT Device file was processed and inserted Successfully");
			getIMessageFrame().addOutput(
				"MCT Device file was processed and inserted Successfully");

			CTILogger.info(
				" " + addCount + " MCT Devices were added to the database");
			getIMessageFrame().addOutput(
				addCount + " MCT Devices were added to the database");
		} else
			getIMessageFrame().addOutput(
				" MCT Devices failed adding to the database");
		return success;
	}
	...
	private java.util.ArrayList readFile(String fileName) {
		...
		if (file.exists()) {
			try {
				java.io.RandomAccessFile fileReader =
