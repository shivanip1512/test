...
public class HECO_CustomerMonthlyBillingSettlementModel extends HECO_SettlementModelBase {
    // number of STATIC Columns.  Customers are columns, but are added dynamically
    protected final int NUMBER_COLUMNS = 21;
    ...
    protected void loadDataVector() {
      ...
      //loop (number of customers) for each customer's data to display
        for (int i = 0; i < (getCustomerIDS().length * size); i++) {
