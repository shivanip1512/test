...
public class CapControlStateComparisonModelTest {
    ...
    public void test_buildSQLStatement() throws Exception {
        ...
        assertEquals("Query: "
                ...
                + " left outer join (select * from yukonpaobject where type = 'CCFEEDER' ) yp2 on yp2.paobjectid = fb.feederid"
                + " left outer join ccfeedersubassignment sf on fb.feederid = sf.feederid"
                + " left outer join (select * from yukonpaobject where type = 'CCSUBBUS' ) yp3 on yp3.paobjectid = sf.substationbusid"
                + " left outer join ccsubstationsubbuslist ss on sf.substationbusid = ss.substationbusid"
                + " left outer join (select * from yukonpaobject where type = 'CCSUBSTATION' ) yp4 on yp4.paobjectid = ss.substationid"
                ...
                + " left outer join ccsubstationsubbuslist ssb on ssb.substationbusid = sf.substationbusid"
                + " left outer join ccsubareaassignment saa on saa.substationbusid = ssb.substationid"
                + " left outer join (select paobjectid, paoname from yukonpaobject where type = 'CCAREA' ) ca on ca.paobjectid = saa.areaid"
                + "  Arguments: []", 
            builder.getDebugSql());
