    ...
    public SOAPMessage process(SOAPMessage reqMsg, HttpSession session) {
        ...
        try {
            ...
            LiteStarsEnergyCompany energyCompany =
                StarsDatabaseCache.getInstance().getEnergyCompany(user.getEnergyCompanyID());

            StarsSearchCustomerAccount searchAccount = reqOper.getStarsSearchCustomerAccount();
            int searchByDefID =
                YukonSpringHook.getBean(YukonListDao.class).getYukonListEntry(searchAccount.getSearchBy().getEntryID()).getYukonDefID();
            ...
            boolean searchMembers =
                YukonSpringHook.getBean(RolePropertyDao.class).checkProperty(YukonRoleProperty.ADMIN_MANAGE_MEMBERS,
                    user.getYukonUser())
            ...
            if (searchByDefID == YukonListEntryTypes.YUK_DEF_ID_SEARCH_TYPE_ACCT_NO) {
                ...
                accountList =
                    energyCompany.searchAccountByAccountNumber(searchAccount.getSearchValue(), searchMembers, true);
                ...
                /* Search by last name */
                accountList =
                    energyCompany.searchAccountByLastName(searchAccount.getSearchValue(), searchMembers,
                        searchAccount.getPartialMatch());
            ...
            if (accountList == null || accountList.size() == 0) {
                StarsFailure failure =
                    StarsFactory.newStarsFailure(StarsConstants.FAILURE_CODE_OPERATION_FAILED,
                        "No customer account matching the search criteria");
                ...
                if (accountId != null) { // liteAcctInfo will only be loaded if exactly 1 account was found in
                    ...
                    ServletUtils.removeTransientAttributes(session);
                    session.setAttribute(ServletUtils.TRANSIENT_ATT_LEADING + ServletUtils.ATT_CUSTOMER_ACCOUNT_INFO,
                        starsAcctInfo);

                    resp.setStarsCustAccountInformation(starsAcctInfo);
                } else if (searchByDefID == YukonListEntryTypes.YUK_DEF_ID_SEARCH_TYPE_LAST_NAME) {
                    // Don't sort, the sort by has already been handled in the
                    // CustomerAccount.searchByPrimaryContactLastName query.
                    for (int i = 0; i < accountList.size(); i++) {
                        StarsBriefCustAccountInfo starsAcctInfo = new StarsBriefCustAccountInfo();
                        Integer accountId2 =
                            (Integer) (searchMembers ? ((Pair) accountList.get(i)).getFirst() : accountList.get(i));

                        starsAcctInfo.setAccountID(accountId2);
                        LiteStarsEnergyCompany company = energyCompany;
                        if (searchMembers) {
                            company = (LiteStarsEnergyCompany) ((Pair) accountList.get(i)).getSecond();
                        }
                        if (searchMembers) {
                            starsAcctInfo.setEnergyCompanyID(company.getLiteID());
                        }
                    ...
                    Map<LiteStarsEnergyCompany, List<Integer>> companyAcctTable =
                        new Hashtable<LiteStarsEnergyCompany, List<Integer>>();
                    ...
                    while (it.hasNext()) {
                        ...
                        if (searchByDefID == YukonListEntryTypes.YUK_DEF_ID_SEARCH_TYPE_LAST_NAME) {
                            // Do not sort the lastnames! This is now handled in the
                            // LiteStarsEnergyCompany.searchByPrimaryContactLastName query!!!
                            // Arrays.sort( accounts, LAST_NAME_CMP );
                        } else if (searchByDefID == YukonListEntryTypes.YUK_DEF_ID_SEARCH_TYPE_ADDRESS) {
                            // Do not sort the addresses! This is now handled in the
                            // LiteStarsEnergyCompany.searchByStreetAddress query!!!
                            /*
            ...
            try {
                respOper.setStarsFailure(StarsFactory.newStarsFailure(StarsConstants.FAILURE_CODE_OPERATION_FAILED,
                    "Failed to search for customer account"));
                return SOAPUtil.buildSOAPMessage(respOper);
            } catch (Exception e2) {
