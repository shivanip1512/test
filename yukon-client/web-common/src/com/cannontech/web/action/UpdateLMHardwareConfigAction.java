...
public class UpdateLMHardwareConfigAction implements ActionBase {
    ...
    public SOAPMessage process(SOAPMessage reqMsg, HttpSession session) {
        ...
        try {
            ...
            LiteLmHardwareBase liteHw =
                (LiteLmHardwareBase) inventoryBaseDao.getByInventoryId(updateHwConfig.getInventoryID());

            try {
                StarsUpdateLMHardwareConfigResponse resp =
                    updateLMHardwareConfig(updateHwConfig, liteHw, user.getUserID(), energyCompany);
            ...
            try {
                respOper.setStarsFailure(StarsFactory.newStarsFailure(StarsConstants.FAILURE_CODE_OPERATION_FAILED,
                    "Failed to update the hardware configuration."));
                return SOAPUtil.buildSOAPMessage(respOper);
            } catch (Exception e2) {
    ...
    public int parse(SOAPMessage reqMsg, SOAPMessage respMsg, HttpSession session) {
        try {
            ...
            StarsCustAccountInformation accountInfo =
                (StarsCustAccountInformation) session.getAttribute(ServletUtils.TRANSIENT_ATT_LEADING
                    + ServletUtils.ATT_CUSTOMER_ACCOUNT_INFO);

            parseResponse(accountInfo, operation.getStarsUpdateLMHardwareConfigResponse());

            StarsUpdateLMHardwareConfig updateCfg =
                SOAPUtil.parseSOAPMsgForOperation(reqMsg).getStarsUpdateLMHardwareConfig();
            if (updateCfg.getSaveConfigOnly()) {
                ...
                session.setAttribute(ServletUtils.ATT_CONFIRM_MESSAGE,
                    "Configuration command saved to batch successfully.");
            } else {
                session.setAttribute(ServletUtils.ATT_CONFIRM_MESSAGE,
                    "Configuration command has been sent out successfully.");
            }
     ...
     */
    public static void updateLMConfiguration(StarsLMConfiguration starsHwConfig, LiteLmHardwareBase liteHw,
    ...
    public static StarsUpdateLMHardwareConfigResponse updateLMHardwareConfig(
            StarsUpdateLMHardwareConfig updateHwConfig, LiteLmHardwareBase liteHw, int userID,
            LiteStarsEnergyCompany energyCompany) throws WebClientException, CommandCompletionException {
        ...
        if (liteHw.getAccountID() > 0) {
            ...
            ProgramEnrollmentService programEnrollmentService =
                YukonSpringHook.getBean("starsProgramEnrollmentService", ProgramEnrollmentService.class);
            hwsToConfig =
                programEnrollmentService.applyEnrollmentRequests(customerAccount, requests, liteHw, currentUser);
        ...
        for (int i = 0; i < hwsToConfig.size(); i++) {
            ...
            if (!updateHwConfig.getSaveConfigOnly()) {
                ...
                if (updateHwConfig.getSaveToBatch()) {
                    String commandType =
                        (toConfig) ? SwitchCommandQueue.SWITCH_COMMAND_CONFIGURE
                            : SwitchCommandQueue.SWITCH_COMMAND_DISABLE;
                    saveSwitchCommand(lHw, commandType, energyCompany);
                } else {
                    LmHardwareCommandService commandService =
                        YukonSpringHook.getBean("lmHardwareCommandService", LmHardwareCommandService.class);
                    if (toConfig) {
    ...
    public static void setStarsLMConfiguration(StarsLMConfiguration starsCfg, HttpServletRequest req)
        ...
        if (req.getParameter("XCOM_SPID") != null) {
            ...
            xcom.setServiceProvider(parseNumber(req.getParameter("XCOM_SPID"), 0, 65534, "SPID"));
            xcom.setGEO(parseNumber(req.getParameter("XCOM_GEO"), 0, 65534, 0, "GEO"));
            xcom.setSubstation(parseNumber(req.getParameter("XCOM_SUB"), 0, 65534, 0, "SUB"));
            xcom.setZip(parseNumber(req.getParameter("XCOM_ZIP"), 0, 16777214, 0, "ZIP"));
            xcom.setUserAddress(parseNumber(req.getParameter("XCOM_USER"), 0, 65534, 0, "USER"));
            ...
            if (splinters != null) {
                for (int i = 0; i < splinters.length; i++) {
                    parseNumber(splinters[i], 0, 254, 0, "Splinter");
            ...
            vcom.setUtility(parseNumber(req.getParameter("VCOM_Utility"), 0, 255, "Utility"));
            vcom.setSection(parseNumber(req.getParameter("VCOM_Section"), 1, 254, 0, "Section"));
            ...
            sa205.setSlot1(parseNumber(req.getParameter("SA205_Slot1"), 0, 4095, 0, "Slot Address"));
            sa205.setSlot2(parseNumber(req.getParameter("SA205_Slot2"), 0, 4095, 0, "Slot Address"));
            sa205.setSlot3(parseNumber(req.getParameter("SA205_Slot3"), 0, 4095, 0, "Slot Address"));
            sa205.setSlot4(parseNumber(req.getParameter("SA205_Slot4"), 0, 4095, 0, "Slot Address"));
            sa205.setSlot5(parseNumber(req.getParameter("SA205_Slot5"), 0, 4095, 0, "Slot Address"));
            sa205.setSlot6(parseNumber(req.getParameter("SA205_Slot6"), 0, 4095, 0, "Slot Address"));

            starsCfg.setSA205(sa205);
        } else if (req.getParameter("SA305_Utility") != null) {
            SA305 sa305 = new SA305();

            sa305.setUtility(parseNumber(req.getParameter("SA305_Utility"), 0, 15, "Utility"));
            sa305.setGroup(parseNumber(req.getParameter("SA305_Group"), 0, 63, 0, "Group"));
            sa305.setDivision(parseNumber(req.getParameter("SA305_Division"), 0, 63, 0, "Division"));
            sa305.setSubstation(parseNumber(req.getParameter("SA305_Substation"), 0, 1023, 0, "Substation"));
            ...
            sa305.setRateFamily(parseNumber(new Integer(rateFam).toString(), 0, 7, "Rate Family"));
            sa305.setRateMember(parseNumber(new Integer(rateMem).toString(), 0, 15, "Rate Member"));
    ...
    public static int parseNumber(String str, int lowerLimit, int upperLimit, String fieldName)
            throws WebClientException {
        ...
        try {
            ...
            if (value < lowerLimit || value > upperLimit) {
                throw new WebClientException("The value of '" + fieldName + "' must be between " + lowerLimit + " and "
                    + upperLimit);
            }
            return value;
