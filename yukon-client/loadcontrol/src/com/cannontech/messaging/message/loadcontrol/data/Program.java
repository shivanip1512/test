...
public abstract class Program implements ProgramMessageCreation, Data, Cloneable, YukonPao {
    ...
    private java.util.Vector<ProgramControlWindow> controlWindowVector = null;

    // contains objects of type GroupBase
    private List<GroupBase> loadControlGroupVector = null;

    // data not restored when sent/received
    private java.util.GregorianCalendar stoppedControlling = null;

    public Program clone() {
        Program clone = cloneKeepingLoadGroups();
        if (loadControlGroupVector != null) {
            clone.loadControlGroupVector = new Vector<GroupBase>();
            for (GroupBase group : loadControlGroupVector) {
    ...
    public Program cloneKeepingLoadGroups() {
        Program clone;
        try {
    ...
    public Program cloneUpdatingLoadGroup(GroupBase newGroup) {
        Program clone = cloneKeepingLoadGroups();
        if (loadControlGroupVector != null) {
            clone.loadControlGroupVector = new Vector<GroupBase>();
            for (GroupBase loadGroup : loadControlGroupVector) {
    ...
    public boolean equals(Object o) {
        return ((o != null) && (o instanceof Program) && ((Program) o).getYukonId().equals(getYukonId()));
    }
    ...
    public java.util.Vector<ProgramControlWindow> getControlWindowVector() {
    ...
    public List<GroupBase> getLoadControlGroupVector() {
        if (loadControlGroupVector == null)
            loadControlGroupVector = new Vector<GroupBase>(10);
    ...
    public static String getProgramStatusString(int status) {
        switch (status) {
            ...
            default:
                throw new RuntimeException("*** Unknown status(" + status + ") in getProgramStatusString(int) in : " +
    ...
    public void setControlWindowVector(Vector<ProgramControlWindow> newControlWindowVector) {
    ...
    public void setLoadControlGroupVector(List<GroupBase> newLoadControlGroupVector) {
    ...
    public boolean isRampingIn() {
        for (int i = 0; i < getLoadControlGroupVector().size(); i++) {
            Group grp = (Group) getLoadControlGroupVector().get(i);
            if (grp.isRampingIn())
                return true;
        }
