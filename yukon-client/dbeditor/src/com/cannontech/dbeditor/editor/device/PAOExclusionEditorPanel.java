		...
		java.awt.event.ActionListener
{
	...
	public Vector oldAvailableVector = null;
	public Vector oldExcludedVector = null;
	...
	/**
	 * connEtoC2:
	 * (JComboBoxRequeue.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * PAOExclusionEditorPanel.fireInputUpdate()V)
	 * @param arg1 java.awt.event.ActionEvent
	 */
	/* WARNING: THIS METHOD WILL BE REGENERATED. */
	...
	public Object getValue( Object val )
	{
		...
		Vector newExcludedVector = new Vector();
        Vector newAvailableVector = new Vector();
        
		for ( int i = 0; i < getAddRemovePanelPAOExcl().rightListGetModel()	.getSize(); i++ )
		{
			newExcludedVector.add( getAddRemovePanelPAOExcl().rightListGetModel().getElementAt(i));
		}
        
        for ( int i = 0; i < getAddRemovePanelPAOExcl().leftListGetModel().getSize(); i++ )
        {
            newAvailableVector.add( getAddRemovePanelPAOExcl().leftListGetModel().getElementAt(i));
        }
		...
		for ( int i = 0; i < oldExcludedVector.size(); i++ )
		{
			...
			{
				LiteYukonPAObject excludedItem = (LiteYukonPAObject) oldExcludedVector.elementAt(i);
				Integer id = excludedItem.getLiteID();
				
				// remove this exclusion from pao's vector
				for ( int j = 0; j < pao.getPAOExclusionVector().size(); j++ )
				{
					PAOExclusion exclusion = (PAOExclusion)pao.getPAOExclusionVector()	.elementAt( j );
		...
		for ( int i = 0; i < newExcludedVector.size(); i++ )
		{
			if ( !oldExcludedVector.contains( newExcludedVector.elementAt(i)) )
			{
				...
				LiteYukonPAObject litePAO = (LiteYukonPAObject) newExcludedVector.elementAt(i);
				
				// this device was not in our original list of excluded devices,
				// modify it and add to our list needing dbchangemsgs
				YukonPAObject excPAO = (YukonPAObject)LiteFactory.convertLiteToDBPersAndRetrieve( (LiteYukonPAObject)litePAO );
        ...
        {
			LiteYukonPAObject litePAO = (LiteYukonPAObject)newAvailableVector.elementAt(i);
			if ( !oldAvailableVector.contains(litePAO) )
			{
				...
				YukonPAObject unExcludedPAO = (YukonPAObject)LiteFactory.convertLiteToDBPersAndRetrieve((LiteYukonPAObject)litePAO);
                for ( int j = 0; j < unExcludedPAO.getPAOExclusionVector().size(); j++)
                {
                    PAOExclusion exclusion = (PAOExclusion)unExcludedPAO.getPAOExclusionVector().elementAt(j);
	...
	private void initialize()
	{
		try
		{
			...
			constraintsJLabelRequeue.gridx = 1;
			constraintsJLabelRequeue.gridy = 3;
			...
			constraintsJComboBoxRequeue.gridx = 2;
			constraintsJComboBoxRequeue.gridy = 3;
			...
			constraintsJComboBoxRequeue.insets = new java.awt.Insets(	3,
																		0,
																		8,
																		124 );
	...
	public void setValue( Object val )
	{
		...
		synchronized ( cache )
		{
			...
			for ( int i = 0; i < paos.size(); i++ )
			{
				...
				if ( litePAO.getCategory() == PAOGroups.getCategory( pao.getPAOCategory() ) && litePAO.getYukonID() != pao	.getPAObjectID()
