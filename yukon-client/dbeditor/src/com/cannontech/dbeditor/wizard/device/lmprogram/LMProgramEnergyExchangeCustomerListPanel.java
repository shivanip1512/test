...
public LMProgramEnergyExchangeCustomerListPanel() {
	super();
	initialize();
}
/**
...
public Object getValue(Object o) 
{
	...
	{
		com.cannontech.database.db.device.lm.LMEnergyExchangeCustomerList customer = new com.cannontech.database.db.device.lm.LMEnergyExchangeCustomerList();

		customer.setDeviceID( program.getPAObjectID() );
		customer.setCustomerOrder( new Integer(i+1) );
		customer.setLmCustomerDeviceID( new Integer(
...
public void setValue(Object o) 
{
	...
	{
		com.cannontech.database.db.device.lm.LMEnergyExchangeCustomerList group = (com.cannontech.database.db.device.lm.LMEnergyExchangeCustomerList)program.getLmProgramStorageVector().get(i);
		
		for( int j = 0; j < allItems.size(); j++ )
		{
			if( ((com.cannontech.database.data.lite.LiteYukonPAObject)allItems.get(j)).getYukonID() ==
				  group.getLmCustomerDeviceID().intValue() )
			{
