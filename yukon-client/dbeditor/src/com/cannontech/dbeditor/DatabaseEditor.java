...
import com.cannontech.core.dao.impl.DBDeletionDaoImpl;
...
public class DatabaseEditor implements
    ...
    public void viewMenuRefreshAction() {
        ...
        try {
        	...
        	if(holder != null && holder instanceof LiteBase) {
        		...
        		if(tempNode != null) {
        			TreeNode[] path = tempNode.getPath();
        			TreePath newPath = new TreePath(path);
        			treeViewPanel.selectLiteBase(newPath, (LiteBase)holder);
        		}
        	}
        	IServerConnection dispatchConnection = getConnToDispatch();
        	if( dispatchConnection.isValid() ) {
        	    String host = dispatchConnection.getHost();
        	    int port = dispatchConnection.getPort();
        		f.setTitle("Yukon Database Editor [Connected to Dispatch@" + host + ":" + port + "]" );
        		f.repaint();
        	} else {
    		...
    		public void internalFrameClosing(InternalFrameEvent e) {
    			...
    			if (frame.getContentPane() instanceof PropertyPanel) {
    				...
    				if( current != null && lastSelection != null && current.hasChanged() ) {
    					...
    					int c = javax.swing.JOptionPane.showConfirmDialog(getParentFrame(), "Do you want to save changes made to '" + frame.getOwnerNode() 
    ...
    private void displayAWizardPanel(JMenuItem item) {
    	...
    	} else if (item == coreCreateMenu.billingGroupMenuItem) { //new billing addition
    ...
    private boolean executeChangeObjectType(WizardPanelEvent event) {
    	...
    	DefaultMutableTreeNode node = getTreeViewPanel().getSelectedNode();
    	DBPersistent selectedObject = LiteFactory.createDBPersistent((LiteBase) node.getUserObject());
    
    	try {
    		Transaction<DBPersistent> t = Transaction.createTransaction(Transaction.RETRIEVE, selectedObject);
    	...
    	if (changeType) {
    		...
    		try {
    			...
    			if( selectedObject instanceof SmartMultiDBPersistent ) {
    				...
    				for( int i = 0; i < smarty.size(); i++ ) {
    					if( !smarty.getDBPersistent(i).equals(smarty.getOwnerDBPersistent()) ) {
    						generateDBChangeMsg( smarty.getDBPersistent(i), DBChangeMsg.CHANGE_TYPE_ADD );
    					}
    				}
    				selectedObject = smarty.getOwnerDBPersistent();
    			}			
    
    			Transaction<DBPersistent> t1 = Transaction.createTransaction(Transaction.UPDATE, selectedObject);
    			...
    			if(DeviceTypesFuncs.isMCT4XX(newType)) {
    				DBChangeMsg ptChange = new DBChangeMsg(0,
    ...
    public void executeChangeTypeButton_ActionPerformed(ActionEvent event) {
        ...
        if (node != null) {
    	  ...
    	  try {
    		 ...
    		 fireMessage( new MessageEvent(this, "Error changing type for " + node + ".  Error received:  " + e.getMessage(), MessageEvent.ERROR_MESSAGE));		 
    	  }
          if (userObject instanceof DeviceBase && deviceDefinitionService.isDeviceTypeChangeable((DeviceBase) userObject)) {
              showChangeTypeWizardPanel(new DeviceChangeTypeWizardPanel(userObject));
          } else if (userObject instanceof PointBase) {
    		 try {
    			DBDeleteResult delRes = new DBDeleteResult(((PointBase)userObject).getPoint().getPointID().intValue(), DBDeletionDaoImpl.POINT_TYPE);
    			byte deleteVal = DaoFactory.getDbDeletionDao().deletionAttempted( delRes );
    			if( deleteVal ==  DBDeletionDaoImpl.STATUS_ALLOW || deleteVal ==  DBDeletionDaoImpl.STATUS_CONFIRM ) {
    			   showChangeTypeWizardPanel(new PointChangeTypeWizardPanel(userObject));
    			} else {
                    JOptionPane.showConfirmDialog(getParentFrame(),("You cannot change this point '" 
    			        + node.toString() + "'" + delRes.getDescriptionMsg().toString()),
    				    "Unable to change",
    			...
    			CTILogger.error( e.getMessage(), e );
    			JOptionPane.showConfirmDialog(getParentFrame(), "Are you sure you want to change '" + node + "'?", "Confirm Delete", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
    		 }
    	  } else {
    		 JOptionPane.showMessageDialog(getParentFrame(), "Cannot currently change the type of this Object", "ChangeType Error", JOptionPane.INFORMATION_MESSAGE);
    	  }
       } else {
    	  JOptionPane.showMessageDialog(getParentFrame(), "You must select something to be changed", "ChangeType Error", JOptionPane.INFORMATION_MESSAGE);
       }
    }
    ...
    private void executeCopyButton_ActionPerformed(ActionEvent event) {
    	...
    	if( node != null ) {
    		...
    		if(toCopy instanceof DeviceBase && !(toCopy instanceof LMGroup)) {
    			...
    			showCopyWizardPanel( new LMProgramCopyWizardPanel(toCopy) );
            } else if(toCopy instanceof LMGroup) {
    			showCopyWizardPanel( new LMGroupCopyWizardPanel(toCopy) );
            } else if(toCopy instanceof LMScenario) {
    			showCopyWizardPanel( new LMScenarioCopyWizardPanel(toCopy) );
            } else if( toCopy instanceof PointBase ) {
    			showCopyWizardPanel( new PointCopyWizardPanel(toCopy, currentDatabase ) );
    			...
    			JOptionPane.showMessageDialog(getParentFrame(), "Cannot currently copy that type of Object", "Copy Error", JOptionPane.INFORMATION_MESSAGE);
    		}
    	} else {
    		JOptionPane.showMessageDialog(getParentFrame(), "You must select something to be copied", "Copy Error", JOptionPane.INFORMATION_MESSAGE);
    	}
    }
    ...
    private void deleteDBPersistent( DBPersistent deletable ) {
    	try {
    		Transaction<DBPersistent> t = Transaction.createTransaction(Transaction.DELETE, deletable);
    		deletable = t.execute();
    		...
    		CTILogger.error( e.getMessage(), e );
    		fireMessage(new MessageEvent(this, "Error deleting " + deletable + " from the database.  Error received:  " + e.getMessage(), MessageEvent.ERROR_MESSAGE));
    	}
    }
    		...
    		if( nodes != null && nodes.length >= 1 ) {
    			...
    			for( int i = 0; i < nodes.length; i++ ) {
    				...
    				try {
    				    ...
    				    CTILogger.error( e.getMessage(), e );
    					fireMessage( new MessageEvent(this, "Error retrieving " + nodes[i] + " from the database.  Error received:  " + e.getMessage(), MessageEvent.ERROR_MESSAGE));
    				}
                    ...
                    frame.setSize( EDITOR_FRAME_SIZE ); // sets the size of EVERY editor frame!!!!!!!
    ...
    public void executeFindButton_ActionPerformed(ActionEvent event) {
    	String value = JOptionPane.showInputDialog(getParentFrame(), "Name of the item: ", "Search", JOptionPane.QUESTION_MESSAGE );
    	if( value != null ) {
    		boolean found = getTreeViewPanel().searchFirstLevelString(value);
    		if( !found ) {
    			JOptionPane.showMessageDialog(getParentFrame(), "Unable to find your selected item", "Item Not Found", JOptionPane.INFORMATION_MESSAGE );
    		}
    	}
    }
    ...
    private void executePointOffsetLegend_ActionPerformed( ActionEvent event ) {
    	...
    	try {
    		...
    		OkCancelDialog diag = new OkCancelDialog( f, "Point Offset Legend", true, ptLegend );
    ...
    */
    ...
    private void generateDBChangeMsg( DBPersistent object, int changeType  ) {
    	if( object instanceof CTIDbChange ) {
    		DBChangeMsg[] dbChange = DefaultDatabaseCache.getInstance().createDBChangeMessages((CTIDbChange)object, changeType );
    		...
    		throw new IllegalArgumentException("Non " + CTIDbChange.class.getName() + " class tried to generate a " + DBChangeMsg.class.getName() 
    ...
    private JFrame getParentFrame() {
    	return (JFrame)CtiUtilities.getParentFrame(DatabaseEditor.this.getContentPane() );
    }
          ...
          if( getTreeViewPanel().getSelectedNodes().length > 1 ) {
    	      ...
    	      if (selectedNode != null) {
    	         ...
    	         if (selectedNode.getUserObject() instanceof LiteYukonPAObject) {
    	            ...
    	            if( (litYuk.getPaoClass() == PAOGroups.CLASS_CAPCONTROL && !DeviceTypesFuncs.isCapBankController(litYuk.getType())) 
    	                || litYuk.getType() == PAOGroups.LM_GROUP_RIPPLE
    	                || litYuk.getType() == PAOGroups.MACRO_GROUP
    	                || litYuk.getPaoClass() == DeviceClasses.LOADMANAGEMENT ) {
    	               getTreeNodePopupMenu().getJMenuItemChangeType().setEnabled(false);               
    	            } else if ( litYuk.getCategory() == PAOGroups.CAT_CUSTOMER || litYuk.getPaoClass() == DeviceClasses.SYSTEM) {
    	               getTreeNodePopupMenu().getJMenuItemChangeType().setEnabled(false);
    	               getTreeNodePopupMenu().getJMenuItemCopy().setEnabled(false);
    	            }
    ...
    private void readConfigParameters() {	
    	try {	 			
    		decimalPlaces = (new Integer(ClientSession.getInstance().getRolePropertyValue(TDCRole.DECIMAL_PLACES, "3")) ).intValue();
    	} catch( Exception e ) {
    		CTILogger.error( e.getMessage(), e );
    		decimalPlaces = 3;
    	}
    	...
    	try {
    		activateBilling = ClientSession.getInstance().getRolePropertyValue(BillingRole.WIZ_ACTIVATE, "FALSE").trim().equalsIgnoreCase("TRUE");
    	} catch( Exception e ) {
    		CTILogger.error( e.getMessage(), e );
    	}
    	...
    	try {
    		showCore = ClientSession.getInstance().getRolePropertyValue(DBEditorRole.DBEDITOR_CORE, "FALSE").trim().equalsIgnoreCase("TRUE");
    	} catch( Exception e ) {
    		CTILogger.error( e.getMessage(), e );
    	}
    	
    	try {
    		showLm = ClientSession.getInstance().getRolePropertyValue(DBEditorRole.DBEDITOR_LM, "FALSE").trim().equalsIgnoreCase("TRUE");
    	} catch( Exception e ) {
    		CTILogger.error( e.getMessage(), e );
    	}
    	
    	try {
    		showSystem = ClientSession.getInstance().getRolePropertyValue(DBEditorRole.DBEDITOR_SYSTEM, "FALSE").trim().equalsIgnoreCase("TRUE");
    	} catch( Exception e ) {
    		CTILogger.error( e.getMessage(), e );
    	}
    	//check whether the user should be able to see anything regarding logins in the dbeditor
    	try {
    		accessOfLoginNotAllowed = ClientSession.getInstance().getRolePropertyValue(DBEditorRole.PERMIT_LOGIN_EDIT, "TRUE").trim().equalsIgnoreCase("FALSE");
    	} catch( Exception e ) {
    		CTILogger.error( e.getMessage(), e );
    	}
    ...
    public boolean insertDBPersistent( DBPersistent newItem ) {
    	...
    	try {
    		...
    		CTILogger.error( e.getMessage(), e );
    		String messageString = "Error Inserting " + newItem + " into the database.  Error received:  " + e.getMessage().trim();
    		fireMessage(new MessageEvent(this, messageString, MessageEvent.ERROR_MESSAGE));
    	}
    	return success;
    }
    ...
    public void selectionPerformed(WizardPanelEvent event) {
    	...
    	if (event.getID() == WizardPanelEvent.FINISH_SELECTION) {
    		if( changingObjectType ) {
                ...
                selectInTree = !(newItem instanceof TOUSchedule || newItem instanceof Category) || (newItem instanceof TOUSchedule && currentDatabase == DatabaseTypes.SYSTEM_DB);
    	...
    	if( event.getID() == WizardPanelEvent.CANCEL_SELECTION || (event.getID() == WizardPanelEvent.FINISH_SELECTION && successfullInsertion)
    ...
    public void setDatabase(int whichDatabase) {
        ...
        long enableDeviceConfig = Long.parseLong(ClientSession.getInstance().getRolePropertyValue(DBEditorRole.OPTIONAL_PRODUCT_DEV, "0"), 16);
        DatabaseEditor.showDeviceConfiguration = (enableDeviceConfig & ClientRights.ENABLE_DEVICE_CONFIGURATION) > 0; 
    
        int length = (DatabaseEditor.showDeviceConfiguration || models == LM_MODELS || models == LM_MODELS_WITH_SA) ? models.length : models.length - 1;
    ...
    private void showCopyWizardPanel(WizardPanel wizard) {
    	...
    	this.desktopPane.add( f );
    	//a DBPersistent must be created from the Lite object so you can copy it
    	DBPersistent userObject = LiteFactory.createDBPersistent((LiteBase)getTreeViewPanel().getSelectedNode().getUserObject());
    	try {
    		Transaction<DBPersistent> t = Transaction.createTransaction(Transaction.RETRIEVE, userObject);
    		userObject = t.execute();
    	} catch( Exception e ) {
    ...
    private void updateConnectionStatus(IServerConnection conn) {
    	if( conn.isValid() ) {
    		...
    		if( owner != null ) {
    			owner.setTitle("Yukon Database Editor [Connected to Dispatch@" + conn.getHost() + ":" + conn.getPort() + "]" );
    		}
    	} else {
    		if( owner != null ) {
     ...
     */
    /* Returns true if the DB transaction executed successfully, else returns false. */
    private boolean updateDBPersistent(DBPersistent object) {
    	try {
    		Transaction<DBPersistent> t = Transaction.createTransaction( Transaction.UPDATE, object );
    		object = t.execute();
     ...
     */
    private void updateTreePanel(LiteBase lBase, int changeType) {
