...
public class FloatArrayConverter extends AbstractArrayConverter
{
   ...
   public FloatArrayConverter( float defaultValue )
   {
      super( true );
   ...
   public Object convert( Object array )
   {
      ...
      if( array.getClass()
         ...
         FloatConverter converter = isUseDefault() ? new FloatConverter( defaultValue )
               : new FloatConverter();
         if( dims == 1 ){
            for( int index = 0; index < length; index++ ){
               Array.set( result, index,
                     new Float( converter.convert( Array.get( array, index ) ) ) );
            }
         }else{
            for( int index = 0; index < length; index++ ){
