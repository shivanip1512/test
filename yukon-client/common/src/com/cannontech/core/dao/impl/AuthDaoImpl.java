        ...
        if (lContact != null) {
            ...
            if (lYukUser != null) {
                if (lYukUser.getUserID() == UserUtils.USER_NONE_ID) {
                    CTILogger.info("  Failed VOICE login because the YukonUser found was the (none) userid, Contact: "
                        + lContact.toString());
                } else if (lYukUser.getLoginStatus().isDisabled()) {
                    CTILogger.info("  Failed VOICE login because the YukonUser found is DISABLED, Contact: "
                        + lContact.toString());
                } else {
                    ...
                    for (int i = 0; i < pins.length; i++) {
                        ...
                        if (pinNotif.getNotification().equals(pin)) {
                            if (pinNotif.isDisabled()) {
                                CTILogger.info("  Failed VOICE login because the matching PIN found is DISABLED, Contact: "
                                    + lContact.toString());
                                break;
                            } else {
                    ...
                    CTILogger.info("  Failed VOICE login because an invalid PIN was entered or no PIN was found, Contact: "
                        + lContact.toString());
                }
            } else {
                CTILogger.info("  Failed VOICE login because no YukonUser was found for the Contact: "
                    + lContact.toString());
            }
        } else {
        ...
        if (StringUtils.isNotBlank(timeZoneStr)) {
            try {
                ...
                throw new BadConfigurationException(e.getMessage()
                    + ".  Invalid value in YukonRoleProperty Default TimeZone property.");
            }
        } else {
