...
public class CurtailmentEventNotifDaoImpl implements CurtailmentEventNotifDao,
    ...
    SimpleTableAccessTemplate<CurtailmentEventNotif> template;
    ...
    public CurtailmentEventNotif getForId(Integer id) {
        ...
        sql.append("select * from CCurtCENotif");
        sql.append("where CCurtCENotifID").eq(id);
        
        CurtailmentEventNotif result = yukonJdbcTemplate.queryForObject(sql, rowMapper);
        return result;
    }
    ...
    public List<CurtailmentEventNotif> getForEvent(CurtailmentEvent event) {
        SqlStatementBuilder sql = new SqlStatementBuilder();
        sql.append("select cen.* from CCurtCENotif cen");
        ...
        List<CurtailmentEventNotif> result = yukonJdbcTemplate.query(sql, rowMapper);
        return result;
    }

    @Override
    public List<CurtailmentEventNotif> getForEventAndReason(
        ...
        sql.append(  "join CCurtCEParticipant cep on cep.CCurtCEParticipantID = cen.CCurtCEParticipantID");
        sql.append("where cep.CCurtCurtailmentEventID").eq(event.getId());
        sql.append(  "and cen.Reason").eq(reason.name());
    ...
    public List<CurtailmentEventNotif> getScheduledNotifs() {
        SqlStatementBuilder sql = new SqlStatementBuilder();
        sql.append("select * from CCurtCENotif");
        sql.append("where State").eq(NotificationState.SCHEDULED.name());
    ...
    public void delete(CurtailmentEventNotif eventNotif) {
        SqlStatementBuilder sql = new SqlStatementBuilder();
        sql.append("delete from CCurtCENotif");
    ...
    private YukonRowMapper<CurtailmentEventNotif> rowMapper = new YukonRowMapper<CurtailmentEventNotif>() {
        public CurtailmentEventNotif mapRow(YukonResultSet rs) throws SQLException {
            ...
            CurtailmentEventParticipant participant = curtailmentEventParticipantDao.getForId(rs.getInt("CCurtCEParticipantID"));
    ...
    private FieldMapper<CurtailmentEventNotif> curtailmentEventNotifFieldMapper = new FieldMapper<CurtailmentEventNotif>() {
        public void extractValues(MapSqlParameterSource p, CurtailmentEventNotif curtailmentEventNotif) {
            ...
            p.addValue("State", curtailmentEventNotif.getState().name());
            p.addValue("Reason", curtailmentEventNotif.getReason().name());
