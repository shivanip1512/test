...
import com.cannontech.encryption.YukonEncryptDecryptUtils;
...
public class SmtpHelper {
    ...
    private void encryptSetting(List<String> settings, String key)
            throws IllegalBlockSizeException, BadPaddingException, UnsupportedEncodingException {
        settings.add(YukonEncryptDecryptUtils.encrypt(key.toString())
                .concat(separator)
                .concat(YukonEncryptDecryptUtils.encrypt(getCachedValue(key))));
    }
        ...
        try (Stream<String> lines = Files.lines(Paths.get(emailSettingsFile.toURI()), Charset.defaultCharset())) {
            lines.forEach(line -> {
                try {
                    ...
                    updateCachedValue(YukonEncryptDecryptUtils.decrypt(tokens[0]),
                            YukonEncryptDecryptUtils.decrypt(tokens[1]));
