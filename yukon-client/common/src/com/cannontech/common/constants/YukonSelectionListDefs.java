package com.cannontech.common.constants;

/**
 * @author yao
 *
 * To change this generated comment edit the template variable "typecomment":
 * Window>Preferences>Java>Templates.
 * To enable and disable the creation of type comments go to
 * Window>Preferences>Java>Code Generation.
 */
public interface YukonSelectionListDefs {

	public static final int YUK_LIST_ID_CONTACT_TYPE = 1;
	
	// List names are now only used in STARS
	public static final String YUK_LIST_NAME_CONTACT_TYPE = "ContactType";
	
	public static final String YUK_LIST_NAME_LM_CUSTOMER_EVENT = "LMCustomerEvent";
	public static final String YUK_LIST_NAME_LM_CUSTOMER_ACTION = "LMCustomerAction";
	public static final String YUK_LIST_NAME_INVENTORY_CATEGORY = "InventoryCategory";
	public static final String YUK_LIST_NAME_DEVICE_VOLTAGE = "DeviceVoltage";
	public static final String YUK_LIST_NAME_DEVICE_TYPE = "DeviceType";
	public static final String YUK_LIST_NAME_DEVICE_STATUS = "DeviceStatus";
	public static final String YUK_LIST_NAME_APPLIANCE_CATEGORY = "ApplianceCategory";
	public static final String YUK_LIST_NAME_CALL_TYPE = "CallType";
	public static final String YUK_LIST_NAME_SERVICE_TYPE = "ServiceType";
	public static final String YUK_LIST_NAME_SERVICE_STATUS = "ServiceStatus";
	public static final String YUK_LIST_NAME_SEARCH_TYPE = "SearchBy";
	public static final String YUK_LIST_NAME_MANUFACTURER = "Manufacturer";
	public static final String YUK_LIST_NAME_APP_LOCATION = "ApplianceLocation";
	public static final String YUK_LIST_NAME_DEVICE_LOCATION = "DeviceLocation";
	public static final String YUK_LIST_NAME_CHANCE_OF_CONTROL = "ChanceOfControl";
	public static final String YUK_LIST_NAME_TIME_OF_WEEK = "TimeOfWeek";
	public static final String YUK_LIST_NAME_QUESTION_TYPE = "QuestionType";
	public static final String YUK_LIST_NAME_ANSWER_TYPE = "AnswerType";
	public static final String YUK_LIST_NAME_THERMOSTAT_MODE = "ThermostatMode";
	public static final String YUK_LIST_NAME_THERMOSTAT_FAN_STATE = "ThermostatFanState";
	public static final String YUK_LIST_NAME_CUSTOMER_FAQ_GROUP = "CustomerFAQGroup";
	public static final String YUK_LIST_NAME_RESIDENCE_TYPE = "ResidenceType";
	public static final String YUK_LIST_NAME_CONSTRUCTION_MATERIAL = "ConstructionMaterial";
	public static final String YUK_LIST_NAME_DECADE_BUILT = "DecadeBuilt";
	public static final String YUK_LIST_NAME_SQUARE_FEET = "SquareFeet";
	public static final String YUK_LIST_NAME_INSULATION_DEPTH = "InsulationDepth";
	public static final String YUK_LIST_NAME_GENERAL_CONDITION = "GeneralCondition";
	public static final String YUK_LIST_NAME_COOLING_SYSTEM = "CoolingSystem";
	public static final String YUK_LIST_NAME_HEATING_SYSTEM = "HeatingSystem";
	public static final String YUK_LIST_NAME_NUM_OF_OCCUPANTS = "NumberOfOccupants";
	public static final String YUK_LIST_NAME_OWNERSHIP_TYPE = "OwnershipType";
	public static final String YUK_LIST_NAME_FUEL_TYPE = "FuelType";
	public static final String YUK_LIST_NAME_AC_TONNAGE = "ACTonnage";
	public static final String YUK_LIST_NAME_AC_TYPE = "ACType";
	public static final String YUK_LIST_NAME_WH_NUM_OF_GALLONS = "WHNumberOfGallons";
	public static final String YUK_LIST_NAME_WH_ENERGY_SOURCE = "WHEnergySource";
	public static final String YUK_LIST_NAME_WH_LOCATION = "WHLocation";
	public static final String YUK_LIST_NAME_DF_SWITCH_OVER_TYPE = "DFSwitchOverType";
	public static final String YUK_LIST_NAME_DF_SECONDARY_SOURCE = "DFSecondarySource";
	public static final String YUK_LIST_NAME_GEN_TRANSFER_SWITCH_TYPE = "GENTransferSwitchType";
	public static final String YUK_LIST_NAME_GEN_TRANSFER_SWITCH_MFG = "GENTransferSwitchMfg";
	public static final String YUK_LIST_NAME_GRAIN_DRYER_TYPE = "GrainDryerType";
	public static final String YUK_LIST_NAME_GD_BIN_SIZE = "GDBinSize";
	public static final String YUK_LIST_NAME_GD_ENERGY_SOURCE = "GDEnergySource";
	public static final String YUK_LIST_NAME_GD_HORSE_POWER = "GDHorsePower";
	public static final String YUK_LIST_NAME_GD_HEAT_SOURCE = "GDHeatSource";
	public static final String YUK_LIST_NAME_STORAGE_HEAT_TYPE = "StorageHeatType";
	public static final String YUK_LIST_NAME_HEAT_PUMP_TYPE = "HeatPumpType";
	public static final String YUK_LIST_NAME_HEAT_PUMP_SIZE = "HeatPumpSize";
	public static final String YUK_LIST_NAME_HP_STANDBY_SOURCE = "HPStandbySource";
	public static final String YUK_LIST_NAME_IRRIGATION_TYPE = "IrrigationType";
	public static final String YUK_LIST_NAME_IRR_HORSE_POWER = "IRRHorsePower";
	public static final String YUK_LIST_NAME_IRR_ENERGY_SOURCE = "IRREnergySource";
	public static final String YUK_LIST_NAME_IRR_SOIL_TYPE = "IRRSoilType";
	public static final String YUK_LIST_NAME_IRR_METER_LOCATION = "IRRMeterLocation";
	public static final String YUK_LIST_NAME_IRR_METER_VOLTAGE = "IRRMeterVoltage";
	public static final String YUK_LIST_NAME_OPT_OUT_PERIOD = "OptOutPeriod";
	public static final String YUK_LIST_NAME_INV_SEARCH_BY = "InvSearchBy";
	public static final String YUK_LIST_NAME_INV_SORT_BY = "InvSortBy";
	public static final String YUK_LIST_NAME_INV_FILTER_BY = "InvFilterBy";
	public static final String YUK_LIST_NAME_SO_SEARCH_BY = "SOSearchBy";
	public static final String YUK_LIST_NAME_SO_SORT_BY = "SOSortBy";
	public static final String YUK_LIST_NAME_SO_FILTER_BY = "SOFilterBy";
	public static final String YUK_LIST_NAME_RATE_SCHEDULE = "RateSchedule";
	
}
