...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:key name="list" match="pao" use="displayGroup/@value" />
  <xsl:template match="paoDefinitions">
    <html>
      <head>
        <title>Yukon Pao Definitions</title>
        <style type="text/css">
          <![CDATA[
  .pointTable {
  ...
  .paoBody {
    margin-left: 2em;
      ...
      <body>
        ...
        <xsl:for-each
          select="pao[generate-id(.)=generate-id(key('list', displayGroup/@value))]/displayGroup/@value">
          <xsl:sort />
          <h2>
            <xsl:value-of select="." />
          </h2>
