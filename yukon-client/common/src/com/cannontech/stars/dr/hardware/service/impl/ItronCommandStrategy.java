...
public class ItronCommandStrategy  implements LmHardwareCommandStrategy{
    ...
    public void sendCommand(LmHardwareCommand command) throws CommandCompletionException {
        ...
        try {
            ...
            case CONFIG:
            case IN_SERVICE:
                // we are not sending any in-service command if they do not come from enrollment
                // EXPECT_RESPONSE = true marks the command as command generated by doing enrollment 
                Object isEnrollment = command.getParams().get(LmHardwareCommandParam.EXPECT_RESPONSE);
                if(isEnrollment != null && (boolean) isEnrollment) {
                    itronCommunicationService.enroll(command.getDevice().getAccountID(),
                        (int) command.getParams().get(LmHardwareCommandParam.GROUP_ID));
                } else {
                    log.info(
                        "Attempting to send IN_SERVICE to Itron device, this command is not supported unless it is enrollment. Command:"
                            + command);
                }
                break;
            case OUT_OF_SERVICE:
                // we are not sending any out of service command if they do not come from un-enrollment
                // EXPECT_RESPONSE = true marks the command as command generated by doing un-enrollment 
                Object isUnenrollment = command.getParams().get(LmHardwareCommandParam.EXPECT_RESPONSE);
                if (isUnenrollment != null && (boolean) isUnenrollment) {
                    itronCommunicationService.unenroll(command.getDevice().getAccountID(),
                        (int) command.getParams().get(LmHardwareCommandParam.GROUP_ID));
                } else {
                    log.info(
                        "Attempting to send OUT_OF_SERVICE to Itron device, this command is not supported unless it is un-enrollment. Command:"
                            + command);
                }
                break;
