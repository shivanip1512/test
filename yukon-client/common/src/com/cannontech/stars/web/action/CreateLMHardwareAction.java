...
public class CreateLMHardwareAction implements ActionBase {
		...
		throws WebClientException
	{
		...
		try {
			com.cannontech.stars.database.db.hardware.InventoryBase invDB = new com.cannontech.stars.database.db.hardware.InventoryBase();
			com.cannontech.stars.database.data.hardware.LMHardwareBase hw = null;
			
			if (createHw.getLMHardware() != null) {
				hw = new com.cannontech.stars.database.data.hardware.LMHardwareBase();
			...
			if (liteAcctInfo != null) {
				if (liteInv instanceof LiteStarsLMHardware) {
                    ...
                    {
                        com.cannontech.stars.database.db.hardware.LMHardwareConfiguration config =
                            new com.cannontech.stars.database.db.hardware.LMHardwareConfiguration();
                        int appID = 0;
                        int appCatID = 0;
				...
				if (createHw.getDeviceStatus() != null) {
					...
					if (statusDefID != liteInv.getDeviceStatus()) {
						...
						if (statusDefID == YukonListEntryTypes.YUK_DEF_ID_DEV_STAT_AVAIL) {
							...
							event = new com.cannontech.stars.database.data.event.LMHardwareEvent();
