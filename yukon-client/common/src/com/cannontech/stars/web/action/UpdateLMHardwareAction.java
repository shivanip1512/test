		...
		try {
			...
			for (LiteLMHardwareEvent liteEvent : hwHist) {
				...
				if (liteEvent.getActionID() == installEntryID) {
					...
					{
						com.cannontech.stars.database.data.event.LMHardwareEvent event =
								(com.cannontech.stars.database.data.event.LMHardwareEvent) StarsLiteFactory.createDBPersistent( liteEvent );
						com.cannontech.stars.database.db.event.LMCustomerEventBase eventDB = event.getLMCustomerEventBase();
						if (updateHw.getInstallDate() != null)
