...
public final class DeviceTypesFuncs implements com.cannontech.database.data.pao.DeviceTypes {
    ...
    public static boolean isMCT440(int type) {
        switch (type) {
        case MCT440_2131B:
        case MCT440_2132B:
        case MCT440_2133B:
    ...
    @Deprecated
    /** use ChangeDeviceTypeService */
    public static Object changeType(String newType, Object val, DBPersistent extraObj, Vector extra410Objs,
            boolean loadProfileExists, boolean blinkCountExists, boolean totalKWhExists, boolean kWDemandExists,
            boolean voltageExists, boolean peakKWExists, boolean maxVoltsExists, boolean minVoltsExists,
        ...
        if (val instanceof CarrierBase && oldDevice instanceof CarrierBase) {
            ((CarrierBase) val).getDeviceCarrierSettings().setAddress(((CarrierBase) oldDevice).getDeviceCarrierSettings().getAddress());
            ((CarrierBase) val).getDeviceRoutes().setRouteID(((CarrierBase) oldDevice).getDeviceRoutes().getRouteID());
        } else if (val instanceof IGroupRoute && oldDevice instanceof IGroupRoute) {
            ((IGroupRoute) val).setRouteID(((IGroupRoute) oldDevice).getRouteID());
        } else if (val instanceof IDLCBase && oldDevice instanceof IDLCBase) {
            ((IDLCBase) val).getDeviceIDLCRemote().setAddress(((IDLCBase) oldDevice).getDeviceIDLCRemote().getAddress());
        }
        ...
        if (val instanceof MCT410IL) {
            if (loadProfileExists) {
                ...
                ((MCT410IL) val).getDeviceLoadProfile().setLoadProfileDemandRate(((MCTBase) oldDevice).getDeviceLoadProfile()
            ...
            /*
             * TODO: This process needs to be handled much more generally
             * instead of specifying the type and offset of each required point like it is now.
             */
            try {
                for (int j = 0; j < extra410Objs.size(); j++) {
                    Transaction.createTransaction(Transaction.UPDATE, ((DBPersistent) extra410Objs.elementAt(j))).execute();
                }

                if (!totalKWhExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createPulseAccumPoint("kWh",
                ...
                if (!blinkCountExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createPulseAccumPoint("Blink Count",
                                                                                     ((DeviceBase) val).getDevice().getDeviceID(),
                                                                                     YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                if (!loadProfileExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createDmdAccumPoint("kW-LP",
                                                                                   ((DeviceBase) val).getDevice().getDeviceID(),
                                                                                   YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                Transaction.createTransaction(Transaction.INSERT,
                                              PointFactory.createDmdAccumPoint("Voltage-LP",
                                                                               ((DeviceBase) val).getDevice().getDeviceID(),
                                                                               YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                if (!peakKWExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createDmdAccumPoint("Peak kW",
                                                                                   ((DeviceBase) val).getDevice().getDeviceID(),
                                                                                   YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                if (!maxVoltsExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createDmdAccumPoint("Max Volts",
                                                                                   ((DeviceBase) val).getDevice().getDeviceID(),
                                                                                   YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                if (!minVoltsExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createDmdAccumPoint("Min Volts",
                                                                                   ((DeviceBase) val).getDevice()
                                                                                                     .getDeviceID(),
                                                                                   YukonSpringHook.getBean(PointDao.class)
                                                                                                  .getNextPointId(),
                ...
                Transaction.createTransaction(Transaction.INSERT,
                                              PointFactory.createDmdAccumPoint("Frozen Peak Demand",
                                                                               ((DeviceBase) val).getDevice().getDeviceID(),
                                                                               YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                Transaction.createTransaction(Transaction.INSERT,
                                              PointFactory.createDmdAccumPoint("Frozen Max Volts",
                                                                               ((DeviceBase) val).getDevice().getDeviceID(),
                                                                               YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                Transaction.createTransaction(Transaction.INSERT,
                                              PointFactory.createDmdAccumPoint("Frozen Min Volts",
                                                                               ((DeviceBase) val).getDevice().getDeviceID(),
                                                                               YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                if (!kWDemandExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createDmdAccumPoint("kW",
                                                                                   ((DeviceBase) val).getDevice().getDeviceID(),
                                                                                   YukonSpringHook.getBean(PointDao.class).getNextPointId(),
                ...
                if (!voltageExists) {
                    Transaction.createTransaction(Transaction.INSERT,
                                                  PointFactory.createDmdAccumPoint("Voltage",
                                                                                   ((DeviceBase) val).getDevice().getDeviceID(),
                                                                                   YukonSpringHook.getBean(PointDao.class).getNextPointId(),
            ...
            for (LitePoint point : ltPoints) {
                if ((point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_LPROFILE_KW_DEMAND) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_LPROFILE_VOLTAGE_DEMAND) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_PEAK_KW_DEMAND) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_MAX_VOLT_DEMAND) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_MIN_VOLT_DEMAND) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_FROZEN_PEAK_DEMAND) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_FROZEN_MAX_VOLT) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_FROZEN_MIN_VOLT) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_KW_DEMAND) ||
                        (point.getPointType() == PointTypes.DEMAND_ACCUMULATOR_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_VOLTAGE_DEMAND) ||
                        (point.getPointType() == PointTypes.STATUS_POINT && point.getPointOffset() == PointOffsets.PT_OFFSET_TOTAL_KWH)) {
                    
                    Transaction deletePoint = Transaction.createTransaction(Transaction.DELETE, (LiteFactory.convertLiteToDBPers(point)));
                    try {
                        deletePoint.execute();
                    } catch (TransactionException e) {
                        CTILogger.error(e.getMessage(), e);
            ...
            try {
                if (extraObj != null) {
                    Transaction.createTransaction(Transaction.INSERT, (extraObj)).execute();
            ...
            return val;
        } else {// execute the actual command in the database
            try {
                Transaction t2 = Transaction.createTransaction(Transaction.ADD_PARTIAL, ((DBPersistent) val));
