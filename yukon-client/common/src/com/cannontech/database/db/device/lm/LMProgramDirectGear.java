...
public abstract class LMProgramDirectGear extends com.cannontech.database.db.DBPersistent implements DeviceListItem, LMProgramDirectGearDefines
{
	...
	{ 
		"GearName", "gearNumber", "ControlMethod", "MethodRate",
		"MethodPeriod", "MethodRateCount", "CycleRefreshRate", "MethodStopType",
		"ChangeCondition", "ChangeDuration", "ChangePriority", "ChangeTriggerNumber",
		"ChangeTriggerOffset", "PercentReduction", "GroupSelectionMethod",
		"MethodOptionType", "MethodOptionMax"
	};
...
{
	Object addValues[] = { getDeviceID(), getGearName(), getGearNumber(),
			getControlMethod(), getMethodRate(), getMethodPeriod(), getMethodRateCount(),
			getCycleRefreshRate(), getMethodStopType(), 
			getChangeCondition(), getChangeDuration(), getChangePriority(), 
			getChangeTriggerNumber(),	getChangeTriggerOffset(), getPercentReduction(),
			getGroupSelectionMethod(), getMethodOptionType(), getMethodOptionMax() };
...
public static final LMProgramDirectGear createGearFactory( String gearType )
{
	...
	{
		return new com.cannontech.database.data.device.lm.ThermostatSetbackGear();
	}
	else if( gearType.equalsIgnoreCase(THERMOSTAT_PRE_OPERATE) )
	{
		return new com.cannontech.database.data.device.lm.ThermostatPreOperateGear();
	}
	else
		throw new IllegalArgumentException("Unable to create DirectGear for type : " + gearType );
 ...
 */
public static final void deleteAllDirectGears(Integer deviceID, java.sql.Connection conn) throws java.sql.SQLException
{
 ...
 * @param stateGroup java.lang.Integer
 */
public static final LMProgramDirectGear[] getAllDirectGears(Integer deviceID, java.sql.Connection conn) throws java.sql.SQLException
{
	...
	String sql = com.cannontech.database.SqlUtils.createSqlString(
						"select ", SETTER_COLUMNS,
						tNames,
	...
	try
	{
		...
		while( rset.next() )
		{
			...
			LMProgramDirectGear gear = LMProgramDirectGear.createGearFactory( 
					rset.getString(3) );//"ControlMethod") );
			...
			gear.setMethodRate( new Integer(rset.getInt(4)) );//"MethodRate")) );
			gear.setMethodPeriod( new Integer(rset.getInt(5)) );//"MethodPeriod")) );
			gear.setMethodRateCount( new Integer(rset.getInt(6)) );//"MethodRateCount")) );
			gear.setCycleRefreshRate( new Integer(rset.getInt(7)) );//"CycleRefreshRate")) );
			gear.setMethodStopType( rset.getString(8) );//"MethodStopType") );
			gear.setChangeCondition( rset.getString(9) );//"ChangeCondition") );
			gear.setChangeDuration( new Integer(rset.getInt(10)) );//"ChangeDuration")) );
			gear.setChangePriority( new Integer(rset.getInt(11)) );//"ChangePriority")) );
			gear.setChangeTriggerNumber( new Integer(rset.getInt(12)) );//"ChangeTriggerNumber")) );
			gear.setChangeTriggerOffset( new Double(rset.getDouble(13)) );//"ChangeTriggerOffset")) );
			gear.setPercentReduction( new Integer(rset.getInt(14)) );//"PercentReduction")) );
			gear.setGroupSelectionMethod( rset.getString(15) );//"GroupSelectionMethod") );
			gear.setMethodOptionType( rset.getString(16) );//"MethodOptionType") );
			gear.setMethodOptionMax( new Integer(rset.getInt(17)) );//"MethodOptionMax")) );
 ...
 */
protected void setGroupSelectionMethod(java.lang.String newGroupSelectionMethod) {
...
{
	Object setValues[] = { getGearName(), getGearNumber(),
			getControlMethod(), getMethodRate(), getMethodPeriod(), getMethodRateCount(),
