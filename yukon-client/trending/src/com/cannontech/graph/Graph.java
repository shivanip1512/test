...
public synchronized com.jrefinery.chart.JFreeChart getFreeChart() 
{
	if( freeChart == null)
	{
		...
		freeChart = com.jrefinery.chart.ChartFactory.createTimeSeriesChart(
				"Default Test Chart", "Time", "Value", new com.jrefinery.data.TimeSeriesCollection(), true);

		//if( getCurrentGraphDefinition() != null)
			//freeChart = new TestFreeChart(getCurrentGraphDefinition()).getFreeChart();
	}
	return freeChart;
}
/**
	...
	for (int i = 0; i < currentModels.length; i++)
		for (int j = 0; currentModels[i] != null && j < currentModels[i].length; j++)
		{
			if( getModelType() ==  DATA_VIEW_MODEL)
			{
				...
				//freechart.showFreeChart(freechart);
			}
			else if( getModelType() == BAR_GRAPH_MODEL)
			{
				DataViewModel model = (DataViewModel) currentModels[i][j];
