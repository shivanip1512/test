 ...
 * that are plotted in a chart. If possible, use {@link org.jfree.chart.title.LegendTitle} rather than this
 * class.
 */
public class YukonStandardLegend extends DefaultOldLegend implements Serializable {
    ...
    protected Rectangle2D draw(Graphics2D g2, Rectangle2D available, boolean horizontal, boolean inverted,
        ...
        if (getTitle() != null && !getTitle().equals("")) {
            titleItem = new LegendItem(getTitle(), getTitle(), null, null, (Shape) null, Color.black);
        }
        ...
        if (horizontal) {
            double xstart = available.getX() + getMargin().calculateLeftOutset(availableWidth);
            double xlimit = available.getMaxX() - getMargin().calculateRightOutset(availableWidth);
            ...
            for (int i = 0; i < legendItems.getItemCount(); i++) {
                ...
                if (getRenderingOrder() == LegendRenderingOrder.STANDARD) {
                    ...
                    item =
                        createDrawableLegendItem(g2, legendItems.get(legendItems.getItemCount() - i - 1), xoffset,
                            totalHeight);
                } else {
            ...
            legendArea =
                new RoundRectangle2D.Double(0, 0, maxRowWidth, totalHeight, getBoundingBoxArcWidth(),
            ...
            for (int i = 0; i < legendItemsLength; i++) {
                ...
                if (getRenderingOrder() == LegendRenderingOrder.STANDARD) {
                    ...
                    drawableParts =
                        createAllDrawableLinesForItem(g2, legendItems.get(legendItemsLength - i - 1), 0, totalHeight,
                            maxWidth);
                } else {
                    // we're not supposed to get here, will cause
                    // NullPointerException
            ...
            // Create the bounding box
            legendArea =
                new RoundRectangle2D.Float(0, 0, (float) maxWidth, (float) totalHeight, getBoundingBoxArcWidth(),
                    getBoundingBoxArcHeight());
     ...
     */
    private Point2D createTranslationPointForHorizontalDraw(Rectangle2D available, boolean inverted,
            double maxRowWidth, double totalHeight) {
        // The yloc point is the variable part of the translation point
        // for horizontal legends xloc can be: left, center or right.
        double yloc =
            (inverted) ? available.getMaxY() - totalHeight - getMargin().calculateBottomOutset(available.getHeight())
                : available.getY() + getMargin().calculateTopOutset(available.getHeight());
        double xloc;
        if (isAnchoredToLeft()) {
            xloc = available.getX() + getMargin().calculateLeftOutset(available.getWidth());
        } else if (isAnchoredToCenter()) {
            xloc = available.getX() + available.getWidth() / 2 - maxRowWidth / 2;
        } else if (isAnchoredToRight()) {
            xloc = available.getX() + available.getWidth() - maxRowWidth - getChart().getPlot().getInsets().getLeft();
        } else {
    ...
    private Point2D createTranslationPointForVerticalDraw(Rectangle2D available, boolean inverted, double totalHeight,
        ...
        double xloc =
            (inverted) ? available.getMaxX() - maxWidth - getMargin().calculateRightOutset(available.getWidth())
                : available.getX() + getMargin().calculateLeftOutset(available.getWidth());
        double yloc;
        if (isAnchoredToTop()) {
            ...
            yloc =
                available.getY() + available.getHeight() - getChart().getPlot().getInsets().getBottom() - totalHeight;
        } else {
            throw new IllegalStateException("Unexpected Legend Anchor");
        }
        // Create the translation point
    ...
    private void drawLegendTitle(Graphics2D g2, DrawableLegendItem legendTitle) {
        if (legendTitle != null) {
            ...
            g2.setFont(getTitleFont());
            TextUtilities.drawAlignedString(legendTitle.getItem().getLabel(), g2,
    ...
    private void drawSeriesElements(Graphics2D g2, List items, Point2D translation, ChartRenderingInfo info) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            g2.setPaint(getItemPaint());
            g2.setFont(getItemFont());
            TextUtilities.drawAlignedString(item.getItem().getLabel(), g2, (float) item.getLabelPosition().getX(),
                (float) item.getLabelPosition().getY(), TextAnchor.CENTER_LEFT);

            if (entities != null) {
                Rectangle2D area =
                    new Rectangle2D.Double(translation.getX() + item.getX(), translation.getY() + item.getY(),
    ...
    private Rectangle2D calcRemainingDrawingArea(Rectangle2D available, boolean horizontal, boolean inverted,
        ...
        if (horizontal) {
            ...
            double yloc =
                (inverted) ? yy : yy + legendArea.getHeight()
                    + getMargin().calculateBottomOutset(available.getHeight());

            // return the remaining available drawing area
            return new Rectangle2D.Double(available.getX(), yloc, available.getWidth(), available.getHeight()
                - legendArea.getHeight() - getMargin().calculateTopOutset(available.getHeight())
                - getMargin().calculateBottomOutset(available.getHeight()));
        } else {
            // The remaining drawing area bounding box will have the same
            ...
            double xloc =
                (inverted) ? available.getX() : available.getX() + legendArea.getWidth()
                    + getMargin().calculateLeftOutset(available.getWidth())
                    + getMargin().calculateRightOutset(available.getWidth());

            // return the remaining available drawing area
            return new Rectangle2D.Double(xloc, available.getY(), available.getWidth() - legendArea.getWidth()
                - getMargin().calculateLeftOutset(available.getWidth())
                - getMargin().calculateRightOutset(available.getWidth()), available.getHeight());
        }
    }
    ...
    private List createAllDrawableLinesForItem(Graphics2D g2, LegendItem legendItem, double x, double y,
        ...
        LegendItem tmpItem =
            new LegendItem(prefix.trim(), legendItem.getLabel(), legendItem.getToolTipText(), legendItem.getURLText(),
                legendItem.isShapeVisible(), legendItem.getShape(), legendItem.isShapeFilled(),
                legendItem.getFillPaint(), legendItem.isShapeOutlineVisible(), legendItem.getOutlinePaint(),
                legendItem.getOutlineStroke(), legendItem.isLineVisible(), legendItem.getLine(),
                legendItem.getLineStroke(), legendItem.getLinePaint());
        ...
        do {
            ...
            tmpItem =
                new LegendItem(prefix.trim(), legendItem.getLabel(), legendItem.getToolTipText(),
                    legendItem.getURLText(), firstLine && legendItem.isShapeVisible(), legendItem.getShape(), firstLine
                        && legendItem.isShapeFilled(), legendItem.getFillPaint(), firstLine
                        && legendItem.isShapeOutlineVisible(), legendItem.getOutlinePaint(),
                    legendItem.getOutlineStroke(), firstLine && legendItem.isLineVisible(), legendItem.getLine(),
                    legendItem.getLineStroke(), legendItem.getLinePaint());
     ...
     * <P>
     * The marker box for each entry will be positioned next to the name of the specified series within the
     * legend area. The marker box will be square and 70% of the height of current font.
     *
    ...
    private DrawableLegendItem createDrawableLegendItem(Graphics2D graphics, LegendItem legendItem, double x, double y) {
        ...
        float width =
            (float) (item.getLabelPosition().getX() - x + fm.stringWidth(legendItem.getLabel()) + 0.5 * textAscent);
        ...
        if (legendItem.isLineVisible()) {
            ...
            if (item.getItem().isShapeVisible()) {
                ...
                AffineTransform t1 = AffineTransform.getScaleInstance(getShapeScaleX(), getShapeScaleY());
                Shape s1 = t1.createTransformedShape(marker);
                AffineTransform transformer =
                    AffineTransform.getTranslateInstance(xloc + (boxDim * 1.5), yloc + boxDim / 2);
            ...
            if (item.getItem().isShapeVisible()) {
                ...
                Shape s1 = t1.createTransformedShape(marker);
                AffineTransform transformer =
                    AffineTransform.getTranslateInstance(xloc + boxDim / 2, yloc + boxDim / 2);
                Shape s2 = transformer.createTransformedShape(s1);
                item.setMarker(s2);
            } else {
