...
#This is a modified version of vmclone2 found online. The major differences added by Eaton/Cooper are:
    ...
    # CustomizationWinOptions added.
    # IP addressing added.
    # Admininistrator login password added
...
sub get_customization_spec {
   ...
   my $utcClock = 1; #This is not in vmclone.xsd and thus is fixed
   my $domainusername;
   my $domainuserpassword;
   my $adminuserpassword = "cti123"; #This is not in vmclone.xsd and thus is fixed
   my $domain;
   my $fullname = "Eaton"; #This is not in vmclone.xsd and thus is fixed
   my $autoMode = "perServer";
   my $autoUsers = 5;
   my $organization_name = "Eaton";
   my $productId = "XXXX-XXXX-XXXX-XXXX-XXXX";
   my $customization_fixed_ip;
   my $ip;
   my @gateway;
   my @dnsServers;
   my $dnsDomain; #This has been commented out below thus it is unused
   my $subnet;
   my $primaryWINS;
   my $secondaryWINS;
   ...
   foreach (@cspec) {
      ...
      if ($_->findvalue('Domain-User-Name')) {
         $domainusername = $_->findvalue('Domain-User-Name');
      }
      if ($_->findvalue('Domain-User-Password')) {
         $domainuserpassword = $_->findvalue('Domain-User-Password');
      }
   ...
   my $domainpassword =
      CustomizationPassword->new(plainText=>"true", value=> $domainuserpassword );
      
   my $adminpassword =
      CustomizationPassword->new(plainText=>"true", value=> $adminuserpassword );

   my $cust_identification =
      CustomizationIdentification->new(domainAdmin => $domainusername,
   ...
   if (Opts::get_option('runonce')) {
      $customization_runonce = CustomizationGuiRunOnce->new(commandList => \@runonce);
   }

   # test for Linux or Windows customization
   if ( $custType eq "Win" && Opts::get_option('runonce')) {
     $cust_prep = 
   ...
   my $customizationOptions = 
      CustomizationWinOptions->new (changeSID=>1,
                                    deleteAccounts=>0);
