...
if (! Opts::get_option('vmhost') && ! Opts::get_option('resourcepool')) {
  die ( "Either vmname or resourcepool must be specified");
}
 
if (Opts::get_option('vmhost') && Opts::get_option('resourcepool')) {
  die ( "Only one of vmname or resourcepool can be specified at once");
}
...
sub clone_vm {
   ...
   if(@$vm_views) {
      foreach (@$vm_views) {
         ...
         # If we have a vmhost (host_name), use that to find a datastore
         ...
         if ($host_name) {
           ...
           if (not $resourcepool) { 
               Util::trace(0, "\nSpecified resource pool not available. Using host default.\n");
               $resourcepool = $comp_res_view->resourcePool; 
           }
           ...
           # If no hostname, we need to use the datastore name
             my $ds_name = Opts::get_option('datastore');
             if ($ds_name) {
               my $ds_view = Vim::find_entity_view(view_type => 'Datastore',
                                               filter => {'name' => $ds_name});

               $relocate_spec = VirtualMachineRelocateSpec->new(pool => $resourcepool, 
                 datastore => $ds_view->{mo_ref});
             } else {
               # If no datastore name, we use the default for the resource pool
               $relocate_spec = VirtualMachineRelocateSpec->new(pool => $resourcepool);
             }
          }
          ...
          if ($@) {
             if (ref($@) eq 'SoapFault') {
                if (ref($@->detail) eq 'FileFault') {
                   ...
                   Util::trace(0,
                   "The name already exists. Perhaps the VM exists but is not running.\n");
                }
         ...
         die "Execution failed, returning error to windows";
         }
      }
   }
   else {
...
#Gets the resource pool entity view from the pool's name.
