 ...
 */
void ApplyGroupControlStatusVerification(const long key, CtiPointSPtr pPoint, void* d)
{
       ...
       pVG->isDeviceGroupType(pPoint->getDeviceID()))
    {
        ...
        const CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(pPoint->getPointID());
        if( (INT)(pDyn->getValue()) == CONTROLLED && pDyn->getQuality() != ManualQuality )
        {
            ...
            if(!pVG->isPointInPendingControl(pPoint->getPointID()))
            {
                CtiTime now;
                ...
                {
                    {
                        ...
                        if(pPoint)
                        {
                            CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(pPoint->getPointID());
                               ...
                               && !(pDyn->getDispatch().getTags() & (TAG_DISABLE_CONTROL_BY_POINT | TAG_DISABLE_CONTROL_BY_DEVICE)))
                            {
                                ...
                                {
                                    const CtiTablePointAlarming alarming = PointMgr.getAlarming(pPoint->getID());
    ...
    case (CtiCommandMsg::PointTagAdjust):
        {
            if(Cmd->getOpArgList().size() >= 4)
            {
...

                {
                    ...
                    CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(pPt->getPointID());
    ...
    case (CtiCommandMsg::ResetControlHours):
        {
            ...
            try
            {
                ...
                for( ; iter != end; iter++ )
                {
                    CtiPointSPtr TempPoint = iter->second;
                    if( TempPoint &&
                        ...
                        isDeviceGroupType(TempPoint->getDeviceID()))
                    {
                        ...
                        CtiPointSPtr pControlPoint = PointMgr.getControlOffsetEqual( TempPoint->getDeviceID(), 1);     // This is the control status control point which keeps control in play.
    ...
    case (CtiCommandMsg::PointDataRequest):
        {
            ...
            try
            {
                ...
                for(i = 0; i < Cmd->getOpArgList().size(); i++ )
                {
                    ...
                    if(pPt)
                    {
                        const CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(pPt->getPointID());
...
INT CtiVanGogh::archivePointDataMessage(const CtiPointDataMsg &aPD)
{
    ...
    try
    {
        ...
        CtiPointSPtr TempPoint = PointMgr.getEqual(aPD.getId());

        if(TempPoint)      // We do know this point..
        {
            CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(TempPoint->getPointID());
...
void CtiVanGogh::processStalePoint(CtiPointSPtr pPoint, CtiDynamicPointDispatch* pDyn, int updateType, const CtiPointDataMsg &aPD, CtiMultiWrapper& wrap)
{
...
INT CtiVanGogh::archiveSignalMessage(const CtiSignalMsg& aSig)
{
    ...
    try
    {
...

        {
            // See if I know about this point ID
            CtiServerExclusion pmguard(_server_exclusion);
            CtiPointSPtr TempPoint = PointMgr.getEqual(aSig.getId());
                                                        ...
                                                        CtiMultiMsg_vec                         &Ord)
{
    ...
    try
    {
        if(pDat != NULL)
        {
            ...
            {
                CtiPointSPtr pTempPoint = PointMgr.getEqual(pDat->getId());

                if(pTempPoint)
                {
                    const CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(pTempPoint->getPointID());
...
BOOL CtiVanGogh::isPointDataNewInformation(const CtiPointDataMsg &Msg, const CtiDynamicPointDispatch *pDyn)
{
...
INT CtiVanGogh::postMOAUploadToConnection(CtiServer::ptr_type &CM, int flags)
{
    ...
    else if(pMulti != NULL && ((const CtiVanGoghConnectionManager *)CM.get())->isRegForAnyType())
    {
        ...
        for( ; itr != end; itr++ )
        {
            CtiPointSPtr TempPoint = itr->second;
            {
                const CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(TempPoint->getPointID());
...
void CtiVanGogh::refreshCParmGlobals(bool force)
{
    try
    {
        ...
        if( !stringCompareIgnoreCase(gConfigParms.getValueAsString("DISPATCH_SHUTDOWN_ON_THREAD_TIMEOUT"),"true") )
        {
...
INT CtiVanGogh::checkPointDataStateQuality(CtiPointDataMsg  *pData, CtiMultiWrapper &aWrap)
{
    ...
    if(pData != NULL)
    {
        CtiPointSPtr pPoint = PointMgr.getEqual(pData->getId());

        if(pPoint)      // We do know this point..
        {
            CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(pPoint->getPointID());
                        ...
                        if(pPoint->isNumeric())
                        {
                            ...
                            if( TriggerMgr.isAVerificationPoint(pPoint->getPointID()) )
                            {
                                ...
                                if( verificationPtr = TriggerMgr.getPointTriggerFromVerificationID(pPoint->getID()) )
                                {
                                    ...
                                    {
                                        const CtiDynamicPointDispatch *pCtrlDyn = PointMgr.getDynamic(pCtrlPt->getPointID());
...
INT CtiVanGogh::markPointNonUpdated(CtiPointSPtr point, CtiMultiWrapper &aWrap)
{
    ...
    {
        CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(point->getPointID());
        ...
        if( pDyn != NULL )
        {
            ...
            if(quality != NonUpdatedQuality)
            {
                ...
                {
                    const CtiTablePointAlarming alarming = PointMgr.getAlarming(point->getID());
...
{
    {
        ...
        if(signaltrx > SignalEvent)
        {
            ...
            CtiPointSPtr pPoint;
            {
                ...
                if(pPoint)
                {
                    ngid = PointMgr.getAlarming(pPoint->getID()).getNotificationGroupID();
                }
            }
    ...
    if(point->isStatus())    // OK, we are indeed a status point.
    {
        ...
        if(pDyn != NULL)     // We must know about the point!
        {
            ...
            for( alarm = 0; alarm < CtiTablePointAlarming::invalidstatusstate; alarm++ )
            {
                ...
                if( PointMgr.getAlarming(point->getID()).alarmOn( alarm ) ) // we are set to alarm on this condition (category > SignalEvent).
                {
                    switch(alarm)
                    {
...
void CtiVanGogh::VGAppMonitorThread()
{
    ...
    if(!bGCtrlC)
    {
        ...
        CtiPointSPtr pPt;
        const CtiDynamicPointDispatch *pDynPt;
        CtiTime compareTime;
...
int CtiVanGogh::checkNumericReasonability(CtiPointDataMsg *pData, CtiMultiWrapper &aWrap, CtiPointNumericSPtr pointNumeric, CtiDynamicPointDispatch *pDyn, CtiSignalMsg *&pSig )
{
    int alarm = NORMAL;
    string text;
            ...
            if(limits.highLimit < MAX_HIGH_REASONABILITY)  // Is the reasonability reasonable?
            {
                ...
                if(val > limits.highLimit)
                {
                    ...
                    if(!_signalManager.isAlarmed(pointNumeric->getID(), CtiTablePointAlarming::highReasonability) && !pDyn->isConditionActive(alarm))
                    {
                        ...
                        const CtiTablePointAlarming alarming = PointMgr.getAlarming(pointNumeric->getID());
...
void CtiVanGogh::checkNumericRateOfChange(int alarm, CtiPointDataMsg *pData, CtiMultiWrapper &aWrap, CtiPointNumericSPtr pointNumeric, CtiDynamicPointDispatch *pDyn, CtiSignalMsg *&pSig )
{
    bool balarm = false;
...
void CtiVanGogh::checkNumericLimits(int alarm, CtiPointDataMsg *pData, CtiMultiWrapper &aWrap, CtiPointNumericSPtr pointNumeric, CtiDynamicPointDispatch *pDyn, CtiSignalMsg *&pSig )
{
    string text;
    ...
    try
    {
        const CtiTablePointLimit limit = PointMgr.getPointLimit(pointNumeric->getPointID(), limitnumber);
...
void CtiVanGogh::checkStatusState(int alarm, CtiPointDataMsg *pData, CtiMultiWrapper &aWrap, CtiPointSPtr point, CtiDynamicPointDispatch *pDyn, CtiSignalMsg *&pSig )
{
    string action;
    double val = pData->getValue();
    ...
    // if( (pDyn->getValue() != pData->getValue() || pDyn->getQuality() != pData->getQuality()))
    {
        ...
        {
            if( PointMgr.getAlarming(point->getID()).getAlarmCategory(stateverify) > SignalEvent)
            {
 ...
 */
void CtiVanGogh::checkChangeOfState(int alarm, CtiPointDataMsg *pData, CtiMultiWrapper &aWrap, CtiPointSPtr point, CtiDynamicPointDispatch *pDyn, CtiSignalMsg *&pSig )
{
    #if 0
    string action;
    double val = pData->getValue();
...
void CtiVanGogh::tagSignalAsAlarm( CtiPointSPtr point, CtiSignalMsg *&pSig, int alarm, CtiPointDataMsg *pData )
{
    ...
    {
        UINT tags = 0;
        const CtiDynamicPointDispatch *pDyn = PointMgr.getDynamic(point->getPointID());
...
void CtiVanGogh::activatePointAlarm(int alarm, CtiMultiWrapper &aWrap, CtiPointSPtr point, CtiDynamicPointDispatch *&pDyn, bool activate )
{
    ...
    if(pSigActive != NULL)
    {
        ...
        else if( !activate && ((pDyn->getDispatch().getTags() & SIGNAL_MANAGER_MASK) || !PointMgr.getAlarming(point->getID()).getNotifyOnClear()) )
        {
            sigtags |= TAG_REPORT_MSG_BLOCK_EXTRA_EMAIL;
        }
...
void CtiVanGogh::deactivatePointAlarm(int alarm, CtiMultiWrapper &aWrap, CtiPointSPtr point, CtiDynamicPointDispatch *&pDyn )
{
    ...
    if(pSigActive != NULL)
    {
        ...
        if( (pDyn->getDispatch().getTags() & SIGNAL_MANAGER_MASK) || !PointMgr.getAlarming(point->getID()).getNotifyOnClear() )
        {
            sigtags |= TAG_REPORT_MSG_BLOCK_EXTRA_EMAIL;
        }
...

    {
        ...
        if(pSigNew)
        {
            ...
            if(pDyn != NULL)
            {
                ...
                const CtiTablePointAlarming &alarming = PointMgr.getAlarming(pPt->getID());
        ...
        try
        {
            ...
            if(pMulti)
            {
...

                {
                    ...
                    CtiPointSPtr pPt = PointMgr.getEqual(id);
...
void CtiVanGogh::checkStatusCommandFail(int alarm, CtiPointDataMsg *pData, CtiMultiWrapper &aWrap, CtiPointSPtr point, CtiDynamicPointDispatch *pDyn, CtiSignalMsg *&pSig )
{
    UINT tags = pDyn->getDispatch().getTags();
...
void CtiVanGogh::loadStalePointMaps(int pointID)
{
    if( pointID == 0 )
    {
        ...
        for( ; iter != end; iter++ )
        {
             ...
             if( tempPoint && PointMgr.hasProperty(iter->first, CtiTablePointProperty::STALE_ALARM_TIME) )
             {
                 //so we have the alarm time, lets get it and be happy!
                 int alarmTime = PointMgr.getProperty(iter->first, CtiTablePointProperty::STALE_ALARM_TIME);
                ...
                do
                {
                    ...
                    if( checkTimeIter != _expirationSet.end() )
                    {
                        ...
                        if( checkTimeIter->time < CtiTime::now() )
                        {
                            ...
                            if( point && PointMgr.hasProperty(checkTimeIter->pointID, CtiTablePointProperty::STALE_ALARM_TIME) && updatedIter != _pointUpdatedTime.end() )
                            {
                                unsigned int alarmTime = PointMgr.getProperty(checkTimeIter->pointID, CtiTablePointProperty::STALE_ALARM_TIME);
                                ...
                                else
                                {
                                    ...
                                    if( !_signalManager.isAlarmed(point->getPointID(), alarm) && !pDyn->isConditionActive(alarm))
                                    {
                                        ...
                                        const CtiTablePointAlarming &alarming = PointMgr.getAlarming(point->getPointID());
