...
struct test_ConfigManager : ConfigManager
{
	...
	Config::DeviceConfigSPtr fetchConfig(const long deviceID, const DeviceTypes deviceType) override
	{
		return config;
	}
};
...
public:
	...
	{
		_oldConfigManager = std::exchange(gConfigManager, std::make_unique<test_ConfigManager>(config));
	}
...
extern const std::string testProgram;  //  defined below

}

static const std::string MeterProgramming_TestProgramGuid = "CBF44FB5-FEBB-451B-9E97-3504A902D0E1";

struct test_MeterProgrammingManager : MeterProgramming::MeterProgrammingManager
{
	ErrorOr<RawProgram> loadRawProgram(const std::string guid) override
	{
		if( guid == MeterProgramming_TestProgramGuid )
		{
			return RawProgram{
				{ testProgram.begin(), testProgram.end() },
	...
	Bytes convertRawProgram(const RawProgram& raw, const std::string guid) override
	{
		if( guid == MeterProgramming_TestProgramGuid )
		{
			return MeterProgramming::MeterProgrammingManager::convertRawProgram(raw, guid);
		}
		return raw.program;  //  No conversion, disregard GUID and password, just return the raw bytes.
	}
	std::string getAssignedGuid(const Cti::RfnIdentifier rfnIdentifier) override
	{
		return MeterProgramming_TestProgramGuid;
	}
...
struct Override_MeterProgrammingManager
{
	std::unique_ptr<Cti::MeterProgramming::MeterProgrammingManager> oldMeterProgrammingManager;

	Override_MeterProgrammingManager() {
		oldMeterProgrammingManager = std::exchange(Cti::MeterProgramming::gMeterProgrammingManager, std::make_unique<Cti::Test::test_MeterProgrammingManager>());
	}
	~Override_MeterProgrammingManager() {
