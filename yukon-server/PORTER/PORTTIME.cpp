...
INT SendTime(VOID)
{
   ...
   for(; ++itr_prt ;)
   {
      CtiPort *PortRecord = itr_prt.value();

      /* check for a remote port */
      if(PortRecord->getName()(0) == '@') continue;
      if(PortRecord->isInhibited()) continue;
      ...
      if(PortRecord->getProtocol() == IDLC)
      {
         ...
         if(RemoteRecord != NULL) // It existed
         {
            ...
            if(PortRecord->writeQueue(OutMessage->EventCode,
                                      sizeof (*OutMessage),
                                      (char *) OutMessage,
                                      OutMessage->Priority,
                              PortThread))
         ...
         else
         {
            ...
            for(; ++itr_dev ;)
            {
               RemoteRecord = itr_dev.value();

               if(RemoteRecord->getPortID() != PortRecord->getPortID())
               {
                  continue;
               }
      ...
      for(; ++itr_dev ;)
      {
         ...
         if(!RemoteRecord->isInhibited() && RemoteRecord->getPortID() == PortRecord->getPortID())
         {
            if(RemoteRecord->getAddress() == 0xffff)
            {
