#define BOOST_AUTO_TEST_MAIN "Test GenReply"
...
{
    if(gConfigParms.isTrue("CCU711_SEQUENCING_FIX"))
    {
    ...
    else
    {
        ...
        /* NSADJ = false, SequencingBroken = true, SequencesMatch = true
           ...
           We expect a NORMAL return. Sequencing broken should remain true.
           Request value should be accepted from the reply as 2 this time. */
        BOOST_CHECK_EQUAL(pInfo.SequencingBroken, true);
        BOOST_CHECK_EQUAL(pInfo.RemoteSequence.Request, 2);
        BOOST_CHECK_EQUAL(pInfo.RemoteSequence.Reply, 4);
        ...
        /* NSADJ = true, SequencingBroken = true, SequencesMatch = false
           ...
           We expect a NORMAL to occur here as well.                    */
        BOOST_CHECK_EQUAL(pInfo.SequencingBroken, true);
        BOOST_CHECK_EQUAL(pInfo.RemoteSequence.Request, 2);
        BOOST_CHECK_EQUAL(pInfo.RemoteSequence.Reply, 4);
        BOOST_CHECK_EQUAL(status, NORMAL);
        ...
        /* NSADJ = true, SequencingBroken = false, SequencesMatch = true
           ...
           Can't get back on track, CPARM isn't enabled!*/
        BOOST_CHECK_EQUAL(pInfo.SequencingBroken, true); // Wasn't able to be flipped!
        BOOST_CHECK_EQUAL(pInfo.RemoteSequence.Request, 2);
        BOOST_CHECK_EQUAL(pInfo.RemoteSequence.Reply, 4);
        BOOST_CHECK_EQUAL(status, NORMAL);
        ...
        // We need to manually set SequencingBroken back to false for the tests,
        // since we weren't able to flip back to false in the GenReply call because
        // of the CPARM not being present.
        pInfo.SequencingBroken = false;
