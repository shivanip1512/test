        ...
        case ProtocolWrapNone:
            {
                ...
                if(!status)
                {
                    ...
                    case TYPE_CCU710:
                        {
                            //  if we're looking for more than just the two ACKs, we have to check to see if we're getting two NAK-Ws back,
                            //    in which case we won't get any Emetcon words back
                            //  Otherwise, we can fall through to the general CCU-710/RTU-DAVIS case below
                            if( OutMessage->InLength > 2 )
                            {
                                //  read the ACK bytes
                                ...
                                unsigned short nack1 = 0;
                                unsigned short nack2 = 0;

                                if( !(status = Port->inMess(trx, Device, traceList)) &&
                                    !(status = NackTst(InMessage->Buffer.InMessage[0], &nack1, OutMessage->Remote)) &&
                                    !(status = NackTst(InMessage->Buffer.InMessage[1], &nack2, OutMessage->Remote)) )
                                {
                                    //  copied shamelessly from DoProcessInMessage()
                                    if( nack1 )
                                    {
                                        {
                                            CtiLockGuard<CtiLogger> doubt_guard(dout);
                                    ...
                                    else if( nack2 && !(OutMessage->MessageFlags) )
                                    {
                                        if( (OutMessage->EventCode & BWORD) &&
                                    ...
                                    else
                                    {
                                        ...
                                        //  move the buffers and counts ahead of the ACKs
                                        trx.setInBuffer(InMessage->Buffer.InMessage + 2);
                                        trx.setInCountExpected(OutMessage->InLength - 2);

                                        //  if the first part has arrived, the second part is here too - we don't need a timeout
                                        trx.setInTimeout(0);
                                        trx.setMessageStart(false);
                                        trx.setMessageComplete();

                                        status = Port->inMess(trx, Device, traceList);

                                        //  move the buffer and counts back to reflect both reads
                                        trx.setInBuffer(InMessage->Buffer.InMessage);
                                        //  if we're in here, the ACK read succeeded - add that back to the counts
                                        trx.setInCountExpected(trx.getInCountExpected() + 2);
                                        trx.setInCountActual(trx.getInCountActual() + 2);
                                    }
                                }
