...
struct MessageFactoryRegister
{
    MessageFactoryRegister()
    {
        ...
        g_messageFactory.registerSerializer <::CtiCommErrorHistoryMsg,     Thrift::CommErrorHistory>     ( &serialize, &deserialize, "CommErrorHistory" );
...
{
    MessagePtr<Thrift::CommErrorHistory>::type omsg( new Thrift::CommErrorHistory );

    omsg->__set__baseMessage            ( *serialize( static_cast<const ::CtiMessage&>(imsg) ));
    omsg->__set__commErrorId            ( imsg.getCommErrorId() );
    omsg->__set__paoId                  ( imsg.getPAOId() );
    omsg->__set__dateTime               ( CtiTimeToMilliseconds( imsg.getDateTime() ));
    omsg->__set__errorType              ( imsg.getErrorType() );
    omsg->__set__errorNumber            ( imsg.getErrorNumber() );
    omsg->__set__command                ( imsg.getCommand() );
    omsg->__set__outMessage             ( imsg.getOutMessage() );
    omsg->__set__inMessage              ( imsg.getInMessage() );
...
{
    MessagePtr<::CtiCommErrorHistoryMsg>::type omsg( new ::CtiCommErrorHistoryMsg );

    static_cast<::CtiMessage&>(*omsg)   = *deserialize( imsg._baseMessage );
    omsg->setCommErrorId                ( imsg._commErrorId );
    omsg->setPAOId                      ( imsg._paoId );
    omsg->setDateTime                   ( MillisecondsToCtiTime( imsg._dateTime ));
    omsg->setErrorType                  ( imsg._errorType );
    omsg->setErrorNumber                ( imsg._errorNumber );
    omsg->setCommand                    ( imsg._command );
    omsg->setOutMessage                 ( imsg._outMessage );
    omsg->setInMessage                  ( imsg._inMessage );
...
IM_EX_MSG MessagePtr<Thrift::PointRegistration>::type serialize( const ::CtiPointRegistrationMsg& imsg )
{
    ...
    omsg->__set__pointList              ( std::vector<int32_t>( imsg.getPointList().begin(), imsg.getPointList().end() ));
...
IM_EX_MSG MessagePtr<::CtiPointRegistrationMsg>::type deserialize( const Thrift::PointRegistration& imsg )
{
    ...
    omsg->setPointList                  ( std::vector<LONG>( imsg._pointList.begin(), imsg._pointList.end() ));
