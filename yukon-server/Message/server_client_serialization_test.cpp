...
void main(void)
{
    ...
    auto_ptr<CtiServerConnection> serverConn;

    for(;!bGCtrlC;)
    {
        if( listenerConn.verifyConnection() != NORMAL )
        {
            listenerConn.establishConnection();
        }

        // will wait here until a new client connection is accepted.
        if( listenerConn.acceptClient() == NORMAL )
        {

            serverConn.reset( new CtiServerConnection( listenerConn ));
...

            {
                ...
                ServerClientTestSequence<TestCase<CtiCommErrorHistoryMsg>> seq( * serverConn );
                seq.Run();
            }
            ...
            cout << CtiTime() << " Test completed." << endl;
