...
struct TestCase<CtiMessage> : public TestCaseBase<CtiMessage>
{
    ...
    void Populate()
    {
        ...
        GenerateRandom( imsg.MessagePriority, 0, 0x10 );
...
-----------------------------------------------------------------------------*/
template<>
struct TestCase<CtiCommErrorHistoryMsg> : public TestCase<CtiMessage>
{
    void Create()
    {
        _imsg.reset( new CtiCommErrorHistoryMsg );
        _omsg.reset( new CtiCommErrorHistoryMsg );
    }
    ...
    void Populate()
    {
        ...
        CtiCommErrorHistoryMsg &imsg = dynamic_cast<CtiCommErrorHistoryMsg&>(*_imsg);

        GenerateRandom( imsg._commErrorId );
        GenerateRandom( imsg._paoId );
        GenerateRandom( imsg._dateTime );
        GenerateRandom( imsg._errorType );
        GenerateRandom( imsg._errorNumber );
        GenerateRandom( imsg._command );
        GenerateRandom( imsg._outMessage );
        GenerateRandom( imsg._inMessage );
    }
    ...
    void Compare()
    {
        ...
        CtiCommErrorHistoryMsg &imsg = dynamic_cast<CtiCommErrorHistoryMsg&>(*_imsg),
                               &omsg = dynamic_cast<CtiCommErrorHistoryMsg&>(*_omsg);

        CompareMember( "_commErrorId",          imsg._commErrorId,         omsg._commErrorId );
        CompareMember( "_paoId",                imsg._paoId,               omsg._paoId );
        CompareMember( "_dateTime",             imsg._dateTime,            omsg._dateTime );
        CompareMember( "_errorType",            imsg._errorType,           omsg._errorType );
        CompareMember( "_errorNumber",          imsg._errorNumber,         omsg._errorNumber );
        CompareMember( "_command",              imsg._command,             omsg._command );
        CompareMember( "_outMessage",           imsg._outMessage,          omsg._outMessage );
        CompareMember( "_inMessage",            imsg._inMessage,           omsg._inMessage );
    }
};
...
struct TestCase<CtiDBChangeMsg> : public TestCase<CtiMessage>
{
    ...
    void Populate()
    {
        ...
        GenerateRandom( imsg._typeofchange );
    }
...
struct TestCase<CtiMultiMsg> : public TestCase<CtiMessage>
{
    ...
    typedef CtiCommErrorHistoryMsg message1_t;
    typedef CtiLMControlHistoryMsg message2_t;

    TestCaseItem<message0_t> _tc_message0;
    TestCaseItem<message1_t> _tc_message1;
    TestCaseItem<message2_t> _tc_message2;
    ...
    void Compare()
    {
        ...
        if( omsg._bag.size() == 3 )
        {
            ...
            if( !_tc_message2.Compare( (message2_t*)omsg._bag[2] )) reportMismatch( "_bag[2]", _tc_message2._failures );
        }
        else
        {
...
struct TestCase<CtiPointDataMsg> : public TestCase<CtiMessage>
{
    ...
    void Populate()
    {
        ...
        GenerateRandom( imsg._id );
        GenerateRandom( (int&)(imsg._type), 0, (int)InvalidPointType ); // invalid point is excluded
        GenerateRandom( imsg._quality,      0, (int)EstimatedQuality + 1 );
        GenerateRandom( imsg._tags );
        GenerateRandom( imsg._attrib );
        GenerateRandom( imsg._limit );
        GenerateRandom( imsg._value );
        GenerateRandom( imsg._time );
        GenerateRandom( imsg._millis, 0, 1000 );
...
struct TestCase<CtiSignalMsg> : public TestCase<CtiMessage>
{
    ...
    void Populate()
    {
        ...
        GenerateRandom( imsg._tags );
        GenerateRandom( imsg._condition );
        GenerateRandom( imsg._signalMillis, 0 , 1000 );
