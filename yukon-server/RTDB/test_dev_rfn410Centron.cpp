...
BOOST_AUTO_TEST_CASE( test_config_notification )
{
    ...
    BOOST_CHECK_EQUAL(overrideDynamicPaoInfoManager.dpi->dirtyEntries[-1].size(), 62);

    BOOST_CHECK_EQUAL(dpiLong(PI::Key_RFN_LcdCycleTime), 7);
    BOOST_CHECK_EQUAL(dpiLong(PI::Key_RFN_LcdDisplayDigits), 5);

    BOOST_CHECK_EQUAL(dpiString(PI::Key_DisplayItem01), "4");
    BOOST_CHECK_EQUAL(dpiString(PI::Key_DisplayItem02), "8");
    BOOST_CHECK_EQUAL(dpiString(PI::Key_DisplayItem03), "12");
