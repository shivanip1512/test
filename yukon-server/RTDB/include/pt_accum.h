...
public:
   ...
   //The string below was generated by the above getSQL call
   void getSQL(string &sql, LONG pointID, LONG paoID) const
   {
      sql = "SELECT Point.pointid, Point.pointname, Point.pointtype, Point.paobjectid, Point.stategroupid, Point.pointoffset, "
            "Point.serviceflag, Point.alarminhibit, Point.pseudoflag, Point.archivetype, Point.archiveinterval, "
            "PointUnit.uomid, PointUnit.decimalplaces, PointUnit.decimaldigits, UnitMeasure.calctype, "
            "PointAccumulator.multiplier, PointAccumulator.dataoffset "

            "FROM Point, PointUnit, UnitMeasure, PointAccumulator "
            "WHERE Point.pointid = PointUnit.pointid AND PointUnit.uomid = UnitMeasure.uomid AND Point.pointid = PointAccumulator.pointid";

      if(pointID != 0)
      {
         sql += " AND Point.pointid = " + CtiNumStr(pointID);
      }
      if(paoID != 0)
      {
         sql += " AND Point.paobjectid = " + CtiNumStr(paoID);
      }
   }
