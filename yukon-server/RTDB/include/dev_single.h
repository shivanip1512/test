...
protected:
    ...
    */
        ...
        CtiDeviceSingle::ReturnMsgList &returnMsgs )
    {
        std::string msg( "Config " + setting + " did not match. Config: " +
            ...
            ", Device: " +
            ( deviceSetting.is_initialized() ? ( deviceSetting.get() ? "True" : "False" ) : "Uninitialized" ) );
    ...
    */
        ...
        CtiDeviceSingle::ReturnMsgList &returnMsgs )
    {
        std::string msg( "Config " + setting + " did not match. Config: " +
            ...
            ", Device: " +
            ( deviceSetting.is_initialized() ? std::to_string( deviceSetting.get() ) : "Uninitialized" ) );
    ...
    */
        ...
        CtiDeviceSingle::ReturnMsgList &returnMsgs )
    {
        std::string msg( "Config " + setting + " did not match. Config: " +
            ...
            ", Device: " +
            ( deviceSetting.is_initialized() ? std::string( CtiNumStr( deviceSetting.get(), 1 ) ) : "Uninitialized" ) );
    ...
    */
        ...
        CtiDeviceSingle::ReturnMsgList &returnMsgs )
    {
        ...
        std::string config = "(" + join( configSetting |
            transformed( static_cast<std::string( *)( unsigned long )>( std::to_string ) ),
            ", " ) + ")";

        std::string device;
        if( deviceSetting.is_initialized() )
        {
            device = "(" + join( deviceSetting.get() |
                transformed( static_cast<std::string( *)( unsigned long )>( std::to_string ) ),
                ", " ) + ")";
        }
        else
