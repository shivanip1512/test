...
public:
    ...
    void reportControlStart(int isshed, int shedtime, int reductionratio, RWTPtrSlist< CtiMessage >  &vgList, RWCString cmd = RWCString("") )
    {
        ...
        if(pControlStatus != 0)
        {
            CtiLMControlHistoryMsg *hist = new CtiLMControlHistoryMsg ( pControlStatus->getDeviceID(),
            ...
            if(pControlStatus->isPseudoPoint())
            {
                ...
                CtiPointDataMsg *pData = new CtiPointDataMsg( pControlStatus->getPointID(), (DOUBLE)(isshed), NormalQuality, StatusPointType, (isshed == CONTROLLED ? RWCString(getName() + " controlling") : RWCString(getName() + " restoring")));
            ...
            if(isshed == CONTROLLED && shedtime > 0)
            {
                ...
                CtiPointDataMsg *pData = new CtiPointDataMsg( pControlStatus->getPointID(), (DOUBLE)UNCONTROLLED, NormalQuality, StatusPointType, RWCString(getName() + " restoring (delayed)"), TAG_POINT_DELAYED_UPDATE);
