...
BOOST_AUTO_TEST_CASE( test_config_notification )
{
    ...
    BOOST_CHECK_EQUAL(overrideDynamicPaoInfoManager.dpi->dirtyEntries.begin()->second.size(), 63);

    BOOST_CHECK_EQUAL(dpiLong(PI::Key_RFN_LcdCycleTime), 7);
    BOOST_CHECK_EQUAL(dpiLong(PI::Key_RFN_LcdDisplayDigits), 5);
    BOOST_CHECK_EQUAL(dpiLong(PI::Key_RFN_LcdDisconnectDisplayDisabled), 0);
