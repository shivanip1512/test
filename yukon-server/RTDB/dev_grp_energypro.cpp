...
string CtiDeviceGroupEnergyPro::getSQLCoreStatement() const
{
    static const string sqlCore =  "SELECT YP.paobjectid, YP.category, YP.paoclass, YP.paoname, YP.type, YP.disableflag, "
                                   ...
                                   "WHERE upper (YP.type) = 'ENERGYPRO GROUP' AND YP.paobjectid = ECV.lmgroupid AND "
                                   "YP.paobjectid = DV.deviceid";

    return sqlCore;
}

void CtiDeviceGroupEnergyPro::DecodeDatabaseReader(Cti::RowReader &rdr)
{
