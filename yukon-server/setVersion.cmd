...
rem version.external.filenameSafe
    ...
    if "%%p" == "version.patchname" (
        set my_version_patchname=%%q
    )
)
...
if "%updateVersion%" == "1" (
  ...
  echo #define BUILD_VERSION %my_version_maj%.%my_version_min%.%my_build_release_number% ^(build %my_version_build%^) %my_version_patchname% >>%versionFileName%
