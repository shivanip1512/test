...
void LoadScannableDevices(void *ptr)
{
    ...
    if(pChg == NULL || (pChg->getDatabase() == ChangePAODb && resolvePAOCategory(pChg->getCategory()) == PAO_CATEGORY_DEVICE) || (pChg->getDatabase() == ChangePAODb && resolvePAOCategory(pChg->getCategory()) == PAO_CATEGORY_ROUTE) )
    {
        try
        {
            RWRecursiveLock<RWMutexLock>::LockGuard  dev_guard(ScannerDeviceManager.getMux());       // Protect our iteration!
