...
CtiFDR_TextImport::CtiFDR_TextImport() :
    ...
    _threadReadFromFile(Cti::WorkerThread::Function(&CtiFDR_TextImport::threadFunctionReadFromFile, this).name("readFromFile"))
{
...
bool CtiFDR_TextImport::readConfig()
{
    ...
    {
        setFileImportBaseDrivePath("c:\\yukon\\server\\import");
    }
...
bool CtiFDR_TextImport::translateSinglePoint(CtiFDRPointSPtr & translationPoint, bool sendList)
{
    ...
    std::string translationDrivePath;
    std::string translationFilename;
    std::string translationFolderName;
    ...
    for (int x = 0; x < translationPoint->getDestinationList().size(); x++)
    {
        ...
        if ( !pointID.empty() )
        {
            ...
            if (!tempString1.empty())
            {
                ...
                CtiToLower(translationFolderName);

                if ((translationDrivePath = Cti::matchRegex(translationFolderName, "([A-Z]|[a-z]):\\\\")).empty())
                {
                    translationDrivePath = getFileImportBaseDrivePath();
                    CtiToUpper(translationDrivePath);
                } else
                ...
                if ( !tempString1.empty() )
                {
                    ...
                    CtiToLower(translationFilename);
...
std::string& CtiFDR_TextImport::getFileImportBaseDrivePath()
{
    return _fileImportBaseDrivePath;
}

std::string& CtiFDR_TextImport::setFileImportBaseDrivePath(std::string importBase)
{
