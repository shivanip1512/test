...
BOOST_AUTO_TEST_CASE(test_lmobjects_honeywell_group)
{
    ...
    const std::size_t   object_size = sizeof LMGroupHoneywell;
    char                object_buffer[object_size];

    std::fill(object_buffer, object_buffer + object_size, 0xAA);

    // custom deleter to call only the destructor for the honeywell group allocated by placement new
    struct HoneywellGroupDeleter
    {
        void operator()(LMGroupHoneywell * group)
        {
            group->~LMGroupHoneywell();
        }
    };

    boost::shared_ptr<LMGroupHoneywell>
        group(new (object_buffer) LMGroupHoneywell(reader), HoneywellGroupDeleter());
...
BOOST_AUTO_TEST_CASE(test_lmobjects_honeywell_cycle_gear)
{
    ...
    boost::scoped_ptr<HoneywellCycleGear>  gear(new HoneywellCycleGear(reader));
