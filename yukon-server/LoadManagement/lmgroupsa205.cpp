...
CtiRequestMsg* CtiLMGroupSA205::createTimeRefreshRequestMsg(LONG refreshRate, LONG shedTime, int priority) const
{
    ...
    dout << RWTime() << " - createTimeRefreshRequestMsg() not implemented for sa205 LM Groups " << __FILE__ << " at:" << __LINE__ << endl;
    return NULL;
}
...
{
    CtiLockGuard<CtiLogger> logger_guard(dout);
    dout << RWTime() << " - createRotationRequestMsg() not implemented for sa205 LM Groups " << __FILE__ << " at:" << __LINE__ << endl;
    return NULL;
}
...
{
    CtiLockGuard<CtiLogger> logger_guard(dout);
    dout << RWTime() << " - createMasterCycleRequestMsg() not implemented for sa205 LM Groups " << __FILE__ << " at:" << __LINE__ << endl;
    return NULL;
