...
---------------------------------------------------------------------------*/    
DOUBLE CtiLMProgramCurtailment::reduceProgramLoad(DOUBLE loadReductionNeeded, ULONG currentPriority, RWOrdered controlAreaTriggers, ULONG nowInSeconds, CtiMultiMsg* multiPilMsg, CtiMultiMsg* multiDispatchMsg)
{
    RWRecursiveLock<RWMutexLock>::LockGuard  guard(_mutex);
...
---------------------------------------------------------------------------*/
BOOL CtiLMProgramCurtailment::handleManualControl(CtiMultiMsg* multiPilMsg, CtiMultiMsg* multiDispatchMsg)
{
    RWRecursiveLock<RWMutexLock>::LockGuard  guard(_mutex);
...
void CtiLMProgramCurtailment::notifyCustomersOfStop(CtiMultiMsg* multiDispatchMsg)
{
    ...
    if( _lmprogramcurtailmentcustomers.entries() > 0 )
    {
        ...
        for(ULONG i=0;i<_lmprogramcurtailmentcustomers.entries();i++)
        {
            ...
            RWCString emailBody;
            if( getRunStatus() == CtiLMProgramCurtailment::CanceledRunStatus )
            {
                emailBody = getCanceledMsg();
            }
            else if( getRunStatus() == CtiLMProgramCurtailment::StoppedEarlyRunStatus )
            {
