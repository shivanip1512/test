...
private:
    ...
    void checkDispatch(LONG secondsFrom1901);
    void checkPIL(LONG secondsFrom1901);
    void registerForPoints(const RWOrdered& controlAreas);
    void parseMessage( RWCollectable *message, LONG secondsFrom1901 );
    void pointDataMsg( long pointID, double value, unsigned quality, unsigned tags, RWTime& timestamp, LONG secondsFrom1901 );
    void porterReturnMsg( long deviceId, RWCString commandString, int status, RWCString resultString, LONG secondsFrom1901 );
    void signalMsg( long pointID, unsigned tags, RWCString text, RWCString additional, LONG secondsFrom1901 );
