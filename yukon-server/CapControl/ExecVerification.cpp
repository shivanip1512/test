...
void VerificationExecutor::startVerification()
{
    ...
    //Note: Currently supporting only subbus as per legacy code:
    //Find the device from the id.
    CapControlType type = store->determineTypeById(_deviceId);
    if (type != SubBus)
    {
        CtiLockGuard<CtiLogger> logger_guard(dout);
        dout << CtiTime() << "Error: Attempted to start verification on an unsupported device type: " << type << endl;
        return;
    }

    CtiCCAreaPtr currentArea = NULL;
    CtiCCSubstationPtr substation = NULL;
    CtiCCSubstationBusPtr subBus = store->findSubBusByPAObjectID(_deviceId);

    if (subBus == NULL)
    {
        CtiLockGuard<CtiLogger> logger_guard(dout);
        dout << CtiTime() << "Error: SubstationBus with id, " << _deviceId << ", was not found." << endl;
        dout << CtiTime() << "Verification was not started." << endl;
        return;
    }
    ...
    {
        CtiLockGuard<CtiLogger> logger_guard(dout);
        dout << CtiTime() <<  " - Verification Not Enabled on SubBus: "<< subBus->getPaoName() <<" due to SubBus DisableFlag"<< endl;
        return;
    }

    if (subBus->getStrategy()->getUnitType() != ControlStrategy::None)
    {
        CtiLockGuard<CtiLogger> logger_guard(dout);
        dout << CtiTime() <<  " - Verification Not Enabled on SubBus: "<< subBus->getPaoName() <<" due to SubBus Strategy = " << ControlStrategy::NoControlUnit << endl;
        return;
    }

    substation = store->findSubstationByPAObjectID(subBus->getParentId());
    if (substation == NULL)
    {
        CtiLockGuard<CtiLogger> logger_guard(dout);
        dout << CtiTime() << "Error: Parent Substation to Subbus " << subBus->getPaoName() << " was not found." << endl;
        dout << CtiTime() << "Verification was not started." << endl;
        return;
    }

    if (substation->getDisableFlag())
    {
        if (_CC_DEBUG & CC_DEBUG_STANDARD)
        {
            CtiLockGuard<CtiLogger> logger_guard(dout);
            dout << CtiTime() <<  " - Verification Not Enabled on SubBus: "<<subBus->getPaoName()
                <<" due to Substation: "<<substation->getPaoName() <<" DisableFlag"<< endl;
        }
        return;
    }

    currentArea = store->findAreaByPAObjectID(substation->getParentId());

    if (currentArea == NULL)
    {
        CtiLockGuard<CtiLogger> logger_guard(dout);
        dout << CtiTime() << "Error: Parent Area to Substation " << substation->getPaoName() << " was not found." << endl;
        dout << CtiTime() << "Verification was not started." << endl;
        return;
    }
    ...
    if (!subBus->getVerificationFlag())
    {
        ...
        {
            CtiLockGuard<CtiLogger> logger_guard(dout);
            dout << CtiTime() << " - Verification Start Message received from client for sub: " << subBus->getPaoName() << " (" << subBus->getPaoId() << ")" << endl;
        }

        CtiFeeder_vec& ccFeeders = subBus->getCCFeeders();
        for (long j = 0; j < ccFeeders.size(); j++)
        {
            CtiCCFeeder* currentFeeder = (CtiCCFeeder*)ccFeeders.at(j);
            CtiCCCapBank_SVector& ccCapBanks = currentFeeder->getCCCapBanks();
            ...
            for(long k = 0; k < ccCapBanks.size(); k++)
            {
                ...
                if (currentCapBank->getControlStatus() == CtiCCCapBank::OpenPending ||
                    currentCapBank->getControlStatus() == CtiCCCapBank::ClosePending)
                {
                    subBus->setWaitToFinishRegularControlFlag(true);
                    string text =  "Verification will be delayed for Sub: ";
                    text += subBus->getPaoName();
                    text += ", Cap Bank: ";
                    text += currentCapBank->getPaoName();
                    text += " is being controlled.";
                    ...
                    CtiCapController::getInstance()->getCCEventMsgQueueHandle().write(new CtiCCEventLogMsg(0, SYS_PID_CAPCONTROL, spAreaId, areaId, stationId, subBus->getPaoId(), 0, capControlEnableVerification, subBus->getEventSequence(), 1, text, "cap control"));
        ...
        subBus->setVerificationFlag(true);
        subBus->setVerificationStrategy(_verifyType);
        subBus->setVerificationDisableOvUvFlag(_disableOvUv);
        subBus->setCapBankInactivityTime(_inactiveTime);
        subBus->setBusUpdatedFlag(true);

        string text = subBus->getVerificationString();
        text += " Starting";
        string additional("Bus: ");
        additional += subBus->getPaoName();
        if (_LOG_MAPID_INFO)
        {
            additional += " MapID: ";
        ...
        if (subBus->getVerificationDisableOvUvFlag())
        {
            CtiCCExecutorFactory::createExecutor(new ItemCommand(CapControlCommand::AUTO_DISABLE_OVUV, subBus->getPaoId()))->execute();
        }

        ItemCommand* cmdMsg = new ItemCommand(CapControlCommand::DISABLE_SUBSTATION_BUS, subBus->getPaoId());
        cmdMsg->setUser(_userName);
    ...
    if (subBus->getPerformingVerificationFlag() && _subVerificationMsg->getStrategy() < subBus->getVerificationStrategy())
    {
        subBus->setOverlappingVerificationFlag( true );
        subBus->setVerificationStrategy(_subVerificationMsg->getStrategy());
        subBus->setVerificationDisableOvUvFlag(_subVerificationMsg->getDisableOvUvFlag());
        subBus->setCapBankInactivityTime(_subVerificationMsg->getInactivityTime());
        subBus->setBusUpdatedFlag(true);
