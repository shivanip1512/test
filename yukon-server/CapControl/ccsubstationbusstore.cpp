...
/*----------------------------------------------------------------------------
  ...
  data shows up.
----------------------------------------------------------------------------*/
bool CtiCCSubstationBusStore::findSpecialAreaByPointID(long point_id, map< long, CtiCCSpecialPtr >::iterator &begin, map< long, CtiCCSpecialPtr >::iterator &end)
{
...
bool CtiCCSubstationBusStore::findAreaByPointID(long point_id, map< long, CtiCCAreaPtr >::iterator &begin, map< long, CtiCCAreaPtr >::iterator &end)
{
...
bool CtiCCSubstationBusStore::findSubstationByPointID(long point_id, map< long, CtiCCSubstationPtr >::iterator &begin, map< long, CtiCCSubstationPtr >::iterator &end)
{
...
bool CtiCCSubstationBusStore::findSubBusByPointID(long point_id, map< long, CtiCCSubstationBusPtr >::iterator &begin, map< long, CtiCCSubstationBusPtr >::iterator &end)
{
...
bool CtiCCSubstationBusStore::findFeederByPointID(long point_id, map< long, CtiCCFeederPtr >::iterator &begin, map< long, CtiCCFeederPtr >::iterator &end)
{
...
bool CtiCCSubstationBusStore::findCapBankByPointID(long point_id, map< long, CtiCCCapBankPtr >::iterator &begin, map< long, CtiCCCapBankPtr >::iterator &end)
{
...
pair<map<long,long>::iterator,map<long,long>::iterator> CtiCCSubstationBusStore::getSubsWithAltSubID(int altSubId)
{
...
{
    map< long, long >::iterator iter = _altsub_sub_idmap.find(altSubId);
...
void CtiCCSubstationBusStore::doAMFMThr()
{
    ...
    if( !stringCompareIgnoreCase(amfm_interface,m3iAMFMInterfaceString) )
    {
        ...
        if( amFmDbDll != "none" && amFmDbName != "none" && amFmDbUser != "none" && amFmDbPassword != "none" )
        {
            ...
            setDatabaseParams(1,amFmDbDll,amFmDbName,amFmDbUser,amFmDbPassword);
...
bool CtiCCSubstationBusStore::UpdateFeederBankListInDB(CtiCCFeeder* feeder)
{
...

    {
        ...
        if (conn.isValid())
        {
            ...
            RWDBInserter dbInserter = ccFeederBankListTable.inserter();
            ...
            return dbInserter.status().isValid();
        }
    }
    return false;
}
...
bool CtiCCSubstationBusStore::UpdateFeederSubAssignmentInDB(CtiCCSubstationBus* bus)
{
...

    {
        ...
        if (conn.isValid())
        {
            ...
            RWDBInserter dbInserter = ccFeederSubAssignmentTable.inserter();
...
{
    {
        ...
        if (conn.isValid())
        {
            ...
            RWDBInserter dbInserter = ccEventLogTable.inserter();
            dbInserter << logId
                         << msg->getPointId()
...

                    {
                        ...
                        if (subBusId > 0)
                        {
                            selector.where(yukonPAObjectTable["paobjectid"]==capControlSubstationBusTable["substationbusid"] &&
                                     (yukonPAObjectTable["paobjectid"] == subBusId || capControlSubstationBusTable["AltSubID"] == subBusId ));
                        }
                        else
                            selector.where(yukonPAObjectTable["paobjectid"]==capControlSubstationBusTable["substationbusid"]);
                        ...
                        {
                            map <long, long>::iterator iter = altsub_sub_idmap->begin();
                            while (iter != altsub_sub_idmap->end())
                            {
                                ...
                                if (paobject_subbus_map->find(dualBusId) != paobject_subbus_map->end())
                                {
                                    ...
                                    if (paobject_subbus_map->find(currentCCSubstationBus->getAltDualSubId()) != paobject_subbus_map->end())
                                    {
                                        ...
                                        if (!stringCompareIgnoreCase(currentCCSubstationBus->getStrategy()->getControlUnits(),ControlStrategy::KVarControlUnit) ||
                                            !stringCompareIgnoreCase(currentCCSubstationBus->getStrategy()->getControlUnits(),ControlStrategy::PFactorKWKVarControlUnit) ||
                                            !stringCompareIgnoreCase(currentCCSubstationBus->getStrategy()->getControlUnits(),ControlStrategy::PFactorKWKQControlUnit) )
                                        {
    ...
    try
    {
        ...
        try
        {
            ...
            while (!pointIds->empty())
            {
                ...
                if (ptCount > 1)
                {
                    map< long, CtiCCSubstationPtr >::iterator iter1 = _pointid_station_map.lower_bound(pointid);
        ...
        if (areaToDelete != NULL)
        {
            try
            {
                ...
                while (!pointIds->empty())
                {
                    ...
                    if (ptCount > 1)
                    {
                        map< long, CtiCCAreaPtr >::iterator iter1 = _pointid_area_map.lower_bound(pointid);
        ...
        if (spAreaToDelete != NULL)
        {
            ...
            try
            {
                ...
                while (!pointIds->empty())
                {
                    ...
                    if (ptCount > 1)
                    {
                        map< long, CtiCCSpecialPtr >::iterator iter1 = _pointid_specialarea_map.lower_bound(pointid);
    ...
    try
    {
        ...
        try
        {
            ...
            while (!pointIds->empty())
            {
                ...
                if (ptCount > 1)
                {
                    map< long, CtiCCSubstationBusPtr >::iterator iter1 = _pointid_subbus_map.lower_bound(pointid);
        ...
        try
        {
            ...
            while (_altsub_sub_idmap.find(subBusId) != _altsub_sub_idmap.end())
            {
                ...
                if (tempSub != NULL)
                {
                    if (!stringCompareIgnoreCase(tempSub->getStrategy()->getControlUnits(), ControlStrategy::KVarControlUnit))
                    {
                        LONG pointid = subToDelete->getCurrentVarLoadPointId();
                        _pointid_subbus_map.erase(pointid);
                        tempSub->getPointIds()->remove(pointid);
                    }
                    else if (!!stringCompareIgnoreCase(tempSub->getStrategy()->getControlUnits(), ControlStrategy::VoltsControlUnit))
                    {
            ...
            {
                map <long, long>::iterator iter = _altsub_sub_idmap.begin();
...
void CtiCCSubstationBusStore::deleteFeeder(long feederId)
{
    ...
    try
    {
        ...
        while (!pointIds->empty())
        {
            ...
            if (ptCount > 1)
            {
                map< long, CtiCCFeederPtr >::iterator iter1 = _pointid_feeder_map.lower_bound(pointid);
        ...
        if (capBankToDelete != NULL)
        {
          //  capBankToDelete->dumpDynamicData();
            ...
            while (!pointIds->empty())
            {
                ...
                if (ptCount > 1)
                {
                    map< long, CtiCCCapBankPtr >::iterator iter1 = _pointid_capbank_map.lower_bound(pointid);
            ...
            if( _CC_DEBUG & CC_DEBUG_DELETION )
            {
                ...
                {
                    map< long, CtiCCCapBankPtr >::iterator iter = _pointid_capbank_map.begin();
            ...
            if ( conn.isValid() )
            {
...

                {
                    ...
                    while ( rdr() )
                    {
                        ...
                        map< long, CtiCCCapBankPtr >::iterator capBeginIter, capEndIter;
