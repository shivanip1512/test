...
struct TestCase<CtiCCServerResponse> : public TestCase<CtiMessage>
{
    ...
    void Populate()
    {
        ...
        GenerateRandom( imsg._responseType );
...
struct TestCase<CtiCCCapBankStatesMsg> : public TestCase<CapControlMessage>
{
    ...
    void Populate()
    {
        ...
        for( int item_nbr=0; item_nbr < STATE_NBR; item_nbr++ )
        {
            imsg._ccCapBankStates->push_back( new CtiCCState );
            TestCaseItem<CtiCCState> tc_item;
            tc_item.Populate(*(imsg._ccCapBankStates->end()));
        }
    }
};
...
struct TestCase<CtiCCArea> : public TestCase<CapControlPao>
{
    void Populate()
    {
        ...
        {
            CtiCCAreaBase &imsg = reinterpret_cast<CtiCCAreaBase&>(*_imsg);
        ...
        {
            CtiCCArea &imsg = reinterpret_cast<CtiCCArea&>(*_imsg);
...
struct TestCase<CtiCCGeoAreasMsg> : public TestCase<CapControlMessage>
{
    ...
    void Populate()
    {
        ...
        for( int item_nbr=0; item_nbr < AREA_NBR; item_nbr++ )
        {
            imsg._ccGeoAreas->push_back( new CtiCCArea );
            TestCaseItem<CtiCCArea> tc_item;
            tc_item.Populate(*(imsg._ccGeoAreas->end()));
        }
    }
};
...
struct TestCase<CtiCCSpecialAreasMsg> : public TestCase<CapControlMessage>
{
    ...
    void Populate()
    {
        ...
        CtiCCSpecialAreasMsg &imsg = reinterpret_cast<CtiCCSpecialAreasMsg&>(*_imsg);

        for( int item_nbr=0; item_nbr < SPECIAL_NBR; item_nbr++ )
        {
            imsg._ccSpecialAreas->push_back( new CtiCCSpecial );
            TestCaseItem<CtiCCSpecial> tc_item;
            tc_item.Populate(*(imsg._ccSpecialAreas->end()));
        }
    }
};
...
struct TestCase<CtiCCCapBank> : public TestCase<CapControlPao>
{
    void Populate()
    {
        ...
        CtiCCCapBank &imsg = reinterpret_cast<CtiCCCapBank&>(*_imsg);
...
struct TestCase<CtiCCFeeder> : public TestCase<CapControlPao>
{
    ...
    void Populate()
    {
        ...
        CtiCCFeeder &imsg = reinterpret_cast<CtiCCFeeder&>(*_imsg);

        GenerateRandom( imsg._parentId );

        // From base ControlStrategy
        GenerateRandom( imsg.getStrategy()->_maxDailyOperation );
        GenerateRandom( imsg.getStrategy()->_maxOperationDisableFlag );
        ...
        // imsg.getStrategy()->getControlUnits() );

        GenerateRandom( imsg._decimalPlaces );
        GenerateRandom( imsg._peakTimeFlag );

        //imsg.getStrategy()->getPeakLag() );
        //imsg.getStrategy()->getOffPeakLag() );
        //imsg.getStrategy()->getPeakLead() );
        //imsg.getStrategy()->getOffPeakLead() );
        ...
        // imsg.getStrategy()->getPeakPFSetPoint() );
        // imsg.getStrategy()->getOffPeakPFSetPoint() );
        // imsg.getStrategy()->getControlMethod() );
        ...
        // careful here, this is a sorted vector
        for( int item_nbr=0; item_nbr < BANK_NBR; item_nbr++ )
        {
...
struct TestCase<CtiCCSubstationBus> : public TestCase<CapControlPao>
{
    ...
    void Populate()
    {
        ...
        CtiCCSubstationBus &imsg = reinterpret_cast<CtiCCSubstationBus&>(*_imsg);
        ...
        GenerateRandom( imsg._usePhaseData );
        GenerateRandom( imsg._primaryBusFlag );
        GenerateRandom( imsg._altDualSubId );
        GenerateRandom( imsg._dualBusEnable );

        {
            Controllable &item = reinterpret_cast<Controllable&>(*_imsg);
            GenerateRandom( item._strategyId );
        }

        for( int item_nbr=0; item_nbr < FEEDER_NBR; item_nbr++ )
        {
            imsg._ccfeeders.push_back( new CtiCCFeeder );
...
struct TestCase<CtiCCSubstationBusMsg> : public TestCase<CapControlMessage>
{
    ...
    void Populate()
    {
        ...
        for( int item_nbr=0; item_nbr < SUBSTATION_BUS_NBR; item_nbr++ )
        {
            imsg._ccSubstationBuses->push_back( new CtiCCSubstationBus );
            TestCaseItem<CtiCCSubstationBus> tc_item;
            tc_item.Populate(*(imsg._ccSubstationBuses->end()));
        }
    }
};
...
struct TestCase<CtiCCSubstation> : public TestCase<CapControlPao>
{
    void Populate()
    {
        ...
        CtiCCSubstation &imsg = reinterpret_cast<CtiCCSubstation&>(*_imsg);
        ...
        GenerateRandom( imsg._voltReductionControlId );
        GenerateRandom( imsg._recentlyControlledFlag );
        GenerateRandom( imsg._childVoltReductionFlag );
    }
};
...
struct TestCase<CtiCCSubstationsMsg> : public TestCase<CapControlMessage>
{
    ...
    void Populate()
    {
        ...
        for( int item_nbr=0; item_nbr < SUBSTATION_NBR; item_nbr++ )
        {
            imsg._ccSubstations->push_back( new CtiCCSubstation );
            TestCaseItem<CtiCCSubstation> tc_item;
            tc_item.Populate(*(imsg._ccSubstations->end()));
        }
    }
};
...
struct TestCase<Cti::CapControl::VoltageRegulator> : public TestCase<CapControlPao>
{
    void Populate()
    {
        ...
        Cti::CapControl::VoltageRegulator &imsg = reinterpret_cast<Cti::CapControl::VoltageRegulator&>(*_imsg);
...
struct TestCase<ChangeOpState> : public TestCase<ItemCommand>
{
    ...
    void Populate()
    {
        ...
        GenerateRandom( imsg._opStateName );
    }
};
